Ext.define('MOST.view.popup.ChassisMultiPopupController', {
	extend: 'MOST.view.foundation.BaseViewController',

	requires: [
	],

	alias: 'controller.chassismultipopup',	
	...
	onUpdate: function(){
		...
		store.each(function(record,idx){
			if(record.data.chkChassisMulti === 1) {
				if(storeTotal > cnt ){
					if(setChkPtyCdValue === ''){
						setChkPtyCdValue = record.get("plateNo")
					} else {
						setChkPtyCdValue += "," + record.get("plateNo")
					}
	...
	onChassisMultiCheckChange: function(chkbox, rowIdx, checked, record, e, eOpts){
		var me = this;
		var chassisMultiListiPopup = me.getStore(me.MAIN_STORE_NAME );
		var refs = me.getReferences();
		...
		chassisMultiListiPopup.commitChanges();
	},
	
	onChassisAllCheck: function(chk, newValue, oldValue, eOpts) {
		var me = this;
		var store = me.getStore(me.MAIN_STORE_NAME);
