...
public class WHCheckExportDao extends BaseDao implements IWHCheckExportDao {
	...
	public DataItemList selectInvLocList(SearchWHCheckExportParm parm) throws DaoException {
		try {
			return unifiedDao.getItems("whCheckExport.selectInvLocList", parm);
		} catch (Exception e) {
			throw new DaoException(e);
		}
	}
	...
	public void insertShutOutJobItems(DataItemList items) throws DaoException {
		try{
			setNewVersion(items);
			unifiedDao.insertItems(null,"whCheckExport.insertShutOutJobItems", items);
			setVersion(items);
		}catch(Exception e){
			throw new DaoException(e);
		}
	...
	@Override
	public void insertShutOutInvLocationItems(DataItemList items) throws DaoException {
		try{
			setNewVersion(items);
			unifiedDao.insertItems(null,"whCheckExport.insertShutOutInvLocationItems", items);
			setVersion(items);
		}catch(Exception e){
			throw new DaoException(e);
		}
