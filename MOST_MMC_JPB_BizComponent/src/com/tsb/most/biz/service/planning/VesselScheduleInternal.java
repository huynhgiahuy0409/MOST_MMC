...
//		returnItem.setUploadItems((ArrayList<FileUploadItem>) tempList.getCollection());
	...
	public DataItemList insertConfirmationSlipItem(InsertItemsBizParm parm) throws BizException {
		...
		if (ltConfirmCheck.size() < 1) {
			...
			insertItemList.setCollection(masterItem.getItems());
	...
	public DataItemList selectTankerSubmissionItems(SearchVesselScheduleParm parm) throws BizException {
		return vesselScheduleInternalDao.selectTankerSubmissionItems(parm);
	}
	...
	public DataItemList processTankerSubmissionItems(UpdateItemsBizParm parm) throws BizException {
		...
		List<VesselScheduleItem> updateList = items.getCollection();
		...
		// insert Tanker Submission
		if (insertItems.size() > 0) {
		    InsertItemsBizParm insertParm = new InsertItemsBizParm();
		    insertParm.setInsertItems(insertItems);
		    
		    vesselScheduleInternalDao.insertTankerSubmissionItems(insertParm);
		}

		// update Tanker Submission
		if (updateItems.size() > 0) {
		    UpdateItemsBizParm updateParm = new UpdateItemsBizParm();
		    updateParm.setUpdateItems(updateItems);
		    vesselScheduleInternalDao.updateTankerSubmissionItems(updateParm);
		}

		// delete Tanker Submission
		if (deleteItems.size() > 0) {
		    DeleteItemsBizParm deleteParm = new DeleteItemsBizParm();
		    deleteParm.setDeleteItems(deleteItems);
		    vesselScheduleInternalDao.deleteTankerSubmissionItems(deleteParm);
		}
