...
//				blDao.updateOriginalBlStatus(updateItems);
	...
	public DataItemList updateItems(UpdateItemsBizParm parm) throws BizException {
		...
		for(BLItem item: (ArrayList<BLItem>)itemList.getCollection()) {
			...
			ArrayList<BLItem> splitItems = (ArrayList<BLItem>) blDao.selectSplitWgtChk(blParm).getCollection();
			if(splitItems.size() > 0) {
				int pkgQty = 0;
				double mt= 0;
				double m3= 0;
				for(BLItem splitItem: splitItems) {
					pkgQty += Integer.valueOf(splitItem.getSplitPkgQty());
					mt += Double.valueOf(splitItem.getSplitCgWgt());
					m3 += Double.valueOf(splitItem.getSplitCgVol());
				}
				BLItem orgItem = (BLItem) item.clone();
				orgItem.setSplitCgVol(String.valueOf(m3));
