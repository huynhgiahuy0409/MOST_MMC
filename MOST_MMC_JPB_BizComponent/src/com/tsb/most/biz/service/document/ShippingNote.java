...
public class ShippingNote extends MOSTBaseService implements IShippingNote {
	...
	public DataItemList insertItems(InsertItemsBizParm parm) throws BizException {
		...
		manifestParm.setScn(item.getScn());
	...
	public void insertPkgDetailData(ShippingNoteItem item)throws BizException {
	...
	public DataItemList updateItems(UpdateItemsBizParm parm) throws BizException {
		...
		if (dgs != null && dgs.size() > 0) {
			...
			if (item.getVslCallId().equalsIgnoreCase("NonCallId")) {
				...
				itemDG.setCallSeq(Integer.toString(calendar.get(Calendar.YEAR)));
				itemDG.setVslCd(DAOProcessType.INSERT);
				itemDG.setCallYear("1000");
			}
			...
			String dgSeq = processDGDeclarationItemsInt(dgCudParm);
			if (dgSeq != null && dgSeq.length() > 0)
				item.setDgSeq(dgSeq);
	...
	private String processDGDeclarationItemsInt(UpdateBizParm<DGDeclarationItem> parm) throws BizException {
		DGDeclarationItem item = (DGDeclarationItem) parm.getDataItem();
		String newDgSeq = "";
		
		if (item.getCrudDG().equals(DAOProcessType.INSERT)) {
			SearchDGDeclarationParm dgParm = new SearchDGDeclarationParm();
			List dgNoList = dgDeclarationDao.getDGDeclarationNoList(dgParm).getCollection();
