...
public class RehandleCargoHandlingOut extends MOSTBaseService implements IRehandleCargoHandlingOut{
	...
	public DataItemList selectCargoRhdlHandlingOutList(SearchCargoHandlingOutParm parm) throws BizException {
		return cargoHandlingOutDao.selectCargoRhdlHandlingOutList(parm);
	}
	...
	public DataItemList updateCargoRhdlHandlingOutItem(UpdateItemsBizParm parm) throws BizException {
		...
		CargoHandlingOutItem jobGateItem = null;// Gate Out jobItem

		CargoHandlingOutItem invLocItem = null;
		CargoHandlingOutItem delvItem = null;
		CargoHandlingOutItem balItem = null;
		...
		if (masterItem.getCatgCd().equals("E") || masterItem.getCatgCd().equals("S")) { // if 3 //
			...
			/** *************** Import ************** */
			/** ** Start Import ********************************* */
		} else if (masterItem.getCatgCd().equals("I") || masterItem.getCatgCd().equals("T")) { // Import Case
		...
		if (insertDelvItems.getInsertItems() != null && insertDelvItems.getInsertItems().size() > 0) {
			for (int i = 0; i < insertDelvItems.getInsertItems().size(); i++) {
				...
				// Insert processing
				 ...
				 cargoHandlingOutDao.insertCargoHOArrvDelvItems(tmpArrvDelvItems);
			}
		}
