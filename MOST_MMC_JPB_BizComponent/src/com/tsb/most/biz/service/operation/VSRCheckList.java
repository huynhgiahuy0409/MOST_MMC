...
import com.tsb.most.biz.dao.planning.IVOperationDeployDao;
...
public class VSRCheckList extends MOSTBaseService implements IVSRCheckList {
	...
	public DataItemList selectVSRCheckList(SearchVSRCheckListParm parm) throws BizException {
		...
		if (("null".equalsIgnoreCase(parm.getVslCallID())
				...
				("null".equalsIgnoreCase(parm.getScn())
				|| "".equalsIgnoreCase(parm.getScn()))) {
...
//			}
	...
	public DataItemList getVSRDetailHHT(SearchVSRCheckListParm parm) throws BizException {
    	...
    	{
    		if(item.getDivCd().equals("FL"))
    		{
    			MegaFList.add(item);
    		}
    		else if(item.getDivCd().equals("ME"))
    		{
    			MegaMEList.add(item);
    		}
    		else if(item.getDivCd().equals("TRL"))
    		{
    			MegaTRList.add(item);
    		}
    		else if(item.getDivCd().equals("PC"))
    		{
    			MegaPCList.add(item);
    		}
    	}
