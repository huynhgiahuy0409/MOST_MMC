...
public class CargoLoading extends MOSTBaseService implements ICargoLoading {
	...
	public DataItemList updateCargoLoadingItems(UpdateItemsBizParm parm) throws BizException {
		...
		if("BV".equals(masterItem.getJobPurpCd())) {
			...
			//Start-SHUT-OUT/ DAMAGE
			double shuMt = masterItem.getShuMt();
			double shuM3 = masterItem.getShuM3();
			int shuQty = masterItem.getShuQty();
			boolean isShutOut = shuQty > 0 || shuMt > 0 || shuM3 > 0;	
			
			if (item.getOpDelvTpCd().equals(CodeConstant.MT_DELVTP_I)) {
				if(isShutOut) {
					CargoLoadingItem shuJobItem = (CargoLoadingItem) item.clone();
					shuJobItem = (CargoLoadingItem) item.clone();
					shuJobItem.setJobPurpCd(CodeConstant.MT_JOBPURP_AW);
					shuJobItem.setJobTpCd(CodeConstant.MT_JOBTP_RC);
					shuJobItem.setStat(CodeConstant.MT_JOBSTATCD_COM);
					shuJobItem.setDmgYn("N");
					shuJobItem.setShuYn("Y");
					shuJobItem.setJobCoCd(CodeConstant.INVLOC_WH_TP_SHUTOUT);
					shuJobItem.setLoadMt(item.getLoadMt());
					shuJobItem.setLoadM3(item.getLoadM3());
					shuJobItem.setLoadQty(item.getLoadQty());
					shuJobItem.setToLocId(shuJobItem.getShuLocId());
					insertJobItems.add(shuJobItem);					
					
					ArrayList<WhConfigurationItem> invLocShuItems = (ArrayList<WhConfigurationItem>) shuJobItem.getShutItems();
					if (invLocShuItems != null && invLocShuItems.size() > 0) {						
						
						for (int j = 0; j < invLocShuItems.size(); j++) {
							CargoLoadingItem cargoInvLocItem;
							cargoInvLocItem = (CargoLoadingItem) shuJobItem.clone();
							
							WhConfigurationItem whconfItem = (WhConfigurationItem) invLocShuItems.get(j);
							
							cargoInvLocItem.setLocArea(shuJobItem.getShuLocId());
			...
			if (insertInvLocItems.size() > 0) {
				cargoLoadingDao.insertCargoInvLocationItems(insertBalItems);
			}
