...
public class CargoMovement extends MOSTBaseService implements ICargoMovement {
	...
	/**
	 * selectPBZB10Movement
	 * MMC_JPB_OPR-020 Movement
	 * Validate PBZB10 function
	 * */
	@Override
	public DataItemList selectPBZB10Movement(SearchCargoMovementParm parm) throws BizException {
		DataItemList pbzb10Inf = new DataItemList();
		parm.setDocId(OperationConstant.MT_PBZB10);
		
		if (parm.getSearchType().equals("IM")) {
            parm.setDocNo(parm.getCgNo());
        	pbzb10Inf = cargoMovementDao.selectImportPBZB10Inf(parm);
        	
        } else if (parm.getSearchType().equals("EX")) {
        	parm.setDocNo(parm.getMfDocId());
        	pbzb10Inf = cargoMovementDao.selectExportPBZB10Inf(parm);
        }
	...
	public DataItemList selectCargoMovement(SearchCargoMovementParm parm) throws BizException {
        ...
        if (pbzb10Inf.size() > 0) {
        	CargoMovementItem pbzb10InfItem = (CargoMovementItem) pbzb10Inf.getCollection().get(0);
        	int zb10balQty = pbzb10InfItem.getZb10BalPkgQty();
			double zb110balWgt = pbzb10InfItem.getZb10BalWgt();
			double zb110balMsrmt = pbzb10InfItem.getZb10BalMsrmt();

			returnItem.setZb10BalPkgQty(zb10balQty < 0 ? 0 : zb10balQty);
			returnItem.setZb10BalWgt(zb110balWgt < 0 ? 0 : zb110balWgt);
			returnItem.setZb10BalMsrmt(zb110balMsrmt < 0 ? 0 : zb110balMsrmt);
        }
	...
	/**
	 * 20250205.
	 * add function [processCargoMovementItem] replace [updateCargoMovementItems]
	 * process Movement by SN (multiple GRs)
	 * */
	@Override
	public void processCargoMovementItem(UpdateItemsBizParm parm) throws BizException {
		...
		mvItem = (CargoMovementItem) masterItem.clone();
		...
		mstParm.setCgNo(mvItem.getCgNo());
		mstParm.setVslCallId(mvItem.getVslCallId());
		jobGroupNo = cargoMasterDao.selectJobGroupNo(mstParm);

		mvItem.setJobGroup(jobGroupNo);
    	...
    	if (mvItem.getFmLoc() != null && mvItem.getToLoc() != null && !mvItem.getFmLoc().equals("")
			...
			for (int j = 0; j < mvItem.getWhItems().size(); j++) {
				...
				if (CodeConstant.INVLOC_WH_TP_NORMAL.equals(whconfItem.getWhTpCd())) {
					...
					tmpCgNo = whconfItem.getCgNo();
					sumQty = sumQty + Integer.parseInt(whconfItem.getPkgQty());
					sumMt = sumMt + (whconfItem.getWgt());
					sumM3 = sumM3 + (whconfItem.getMsrmt());

					List<CargoMovementItem> chkList = ((List<CargoMovementItem>) insertItems.getCollection()).stream()
							.filter(itm -> itm.getCgNo().equals(whconfItem.getCgNo())).collect(Collectors.toList());

					if (chkList != null && chkList.size() > 0) {
						//If exits -> cumulative qty, wgt, msrmt
						CargoMovementItem crrItem = chkList.get(0);
						chkList.get(0).setCgNo(tmpCgNo);
						chkList.get(0).setPkgQty(crrItem.getPkgQty() + Integer.parseInt(whconfItem.getPkgQty()));
						chkList.get(0).setWgt(crrItem.getWgt() + whconfItem.getWgt());
						chkList.get(0).setMsrmt(crrItem.getMsrmt() + whconfItem.getMsrmt());
						
					} else {
						//If not exits -> add new job
						CargoMovementItem jbItm = (CargoMovementItem) jobItem.clone();

						jbItm.setCgNo(tmpCgNo);
						jbItm.setPkgQty(Integer.parseInt(whconfItem.getPkgQty()));
						jbItm.setWgt(whconfItem.getWgt());
						jbItm.setMsrmt(whconfItem.getMsrmt());

						insertItems.add(jbItm);
					}
				}
			}
    	}
	...
	private void movementInvLoc(DataItemList insertInvLocItems, DataItemList insertMinusInvLocItems, CargoMovementItem jobItem, WhConfigurationItem whconfItem) {
		...
		addLocItem.setCgNo(whconfItem.getCgNo());
		addLocItem.setWhLocTp(whconfItem.getLocTpCd());
		addLocItem.setRefNo(whconfItem.getRefNo());
		...
		minusItem.setCgNo(whconfItem.getCgNo());
		minusItem.setWhLocTp(whconfItem.getToLocTpCd());
		minusItem.setRefNo(whconfItem.getRefNo());
