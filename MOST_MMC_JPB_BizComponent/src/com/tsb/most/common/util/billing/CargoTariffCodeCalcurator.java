...
public class CargoTariffCodeCalcurator extends AbstractTariffCodeCalculator{

	public CargoTariffCodeCalcurator(TariffCodeGeneratorItem targetItem, DataItemList payerList) {
	...
	public void executeCalcurate() {
		...
		for (int i = 0; i < invoiceList.size(); i++) {
			...
			if (trfItem.getTrfTpCd().equals(BillingConstant.TRF_TP_HG)
					|| trfItem.getTrfTpCd().equals(BillingConstant.TRF_TP_HE)) {
				//Handling Goods at Dry/Break bulk Terminal
				...
				if (trfItem.getIvUnit1() != null) {
					unit1 = new BigDecimal(targetItem.getCgWgt());
