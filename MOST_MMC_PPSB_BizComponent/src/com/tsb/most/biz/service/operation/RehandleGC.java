...
//        rehandleGCDao.insertCargoRehandlingItems(parm.getTxTraceinfo(), items);
	...
	public DataItemList processCargoRehandlingItem(UpdateItemsBizParm parm) throws BizException {
		...
		if("C".equalsIgnoreCase(rehandlItem.getRhdlMode())) {		
	        ...
	        for (int i = 0; i < arrCgNo.length; i++) {
	        	...
	        	// Check NextSN exists:
	        	RehandleGCItem rehandlItemTemp = (RehandleGCItem) rehandlItem.clone();
	        	rehandlItemTemp.setNxMfDocId(rehandlItemTemp.getBkNo());
	        	DataItemList snCheckList = this.selectShipgNoteItems(rehandlItemTemp);
	        	
	        	SearchRehandleGCParm rhdlGCParm = new SearchRehandleGCParm();
	        	rhdlGCParm.setVslCallId(rehandlItem.getVslCallId());
	        	rhdlGCParm.setOrgRefNo(rehandlItem.getOrgRefNo());
	        	rhdlGCParm.setMfDocId(rehandlItem.getMfDocId());
	        	rhdlGCParm.setNxVslCallId(rehandlItem.getNxVslCallId());
	        	rhdlGCParm.setNxRefNo(rehandlItem.getNxRefNo());
	        	rhdlGCParm.setNxMfDocId(rehandlItem.getNxMfDocId());
	        	DataItemList rhdlGCDtlList = this.selectCargoRehandlingDetailList(rhdlGCParm);
	        	if ((snCheckList != null && snCheckList.size() > 0)) {
	        		ShippingNoteItem snItem = (ShippingNoteItem) snCheckList.get(0).clone();
	        		if ("E".equalsIgnoreCase(snItem.getCatgCd())) {
        				return createExceptionItems(600, "existsCargoId", Arrays.asList(snItem));
        			} 
	        		if ((rhdlGCDtlList != null && rhdlGCDtlList.size() > 0)) {
	        			this.updateRehandleItems(rehandlItem);
	        		}else {
	        			rehandlItem.setGeneratedSn(snItem);
    ...
    private void insertRehandleItems (RehandleGCItem rehandlItem) throws BizException {
    	...
    	rhdCgParm.setNxMfDocId(rehandlItem.getBkNo());
    	...
    	if("C".equalsIgnoreCase(rehandlItem.getRhdlMode())) {    		
    		...
    		if(rehandlItem.getGeneratedSn() == null){
    			snItem = this.buildShippingNote(rehandlItem); // auto
    			grItem = this.buildGR(snItem);
    			rehandlItem.setNxCgNo(grItem.getGdsRecvNo());
    			rehandlItem.setNxCgNo(grItem.getGdsRecvNo());
    			rehandlItem.setNxMfDocId(rehandlItem.getBkNo());
    			...
    			rehandlItem.setNxCgNo(rehandlItem.getNxGrNo());
    		}
    ...
    private void insertCargoMstItems(RehandleGCItem rhdlItem) throws BizException{
    	...
    	cgMstItem.setMfDocId(cgMstItem.getBkNo());
    ...
    private DataItemList createInvJob(RehandleGCItem rehandlItem) throws BizException{
		...
		nxJobItem.setScn(rehandlItem.getNxScn());
		...
		nxJobItem.setMfDocId(rehandlItem.getBkNo());
		 if("R".equalsIgnoreCase(rehandlItem.getRhdlMode())) {
    ...
    private DataItemList selectShipgNoteItems(RehandleGCItem rehandlItem) {
    	...
    	parm.setMfDocId(rehandlItem.getNxMfDocId());
    	try {
    ...
    private ShippingNoteItem buildShippingNote(RehandleGCItem rehandlItem) throws BizException  {
    	...
    	if("T".equalsIgnoreCase(rehandlItem.getCaTgCd())
    		...
    		snItem.setScn(rehandlItem.getNxScn());
    		...
    		snItem.setMfDocId(rehandlItem.getBkNo());
