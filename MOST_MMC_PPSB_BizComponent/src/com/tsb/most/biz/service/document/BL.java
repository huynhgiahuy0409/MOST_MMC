...
public class BL extends MOSTBaseService implements IBL {
	...
	public DataItemList insertItems(InsertItemsBizParm parm) throws BizException {
		...
		BLItem manifestItem = (BLItem) itemList.getCollection().get(0);
		if (manifestItem == null) {
			return null;
		}
		insertManifest((BLItem) itemList.getCollection().get(0));
	...
	public DataItemList updatePackageItems(UpdateItemsBizParm parm) throws BizException {
		...
		if(insertedPkgList != null && insertedPkgList.size() > 0) {
			ArrayList<BLItem> pkgItems  = (ArrayList<BLItem>) insertedPkgList.getCollection();
	...
	public DataItemList insertPkgDetailData(BLItem item)throws BizException {
		DataItemList itemList = new DataItemList();
		ArrayList<BLItem> pkgArr = item.getPkgItems();
		blDao.deletePackageItems(item);
	
		BLItem blItemParm = new BLItem();
		blItemParm.setOrgBlNo(item.getBlNo());
		blItemParm.setVslCallId(item.getVslCallId());
		blItemParm.setOldMfDocId(item.getMfDocId());
		
		
		for(int i=0; i<pkgArr.size(); i++) {
				
			if(!DAOProcessType.DELETE.equalsIgnoreCase(pkgArr.get(i).getAction())) {
				BLItem cudItem = (BLItem) pkgArr.get(i).clone();
		...
		blItemParm.setWgt(item.getWgt());
		blItemParm.setVol(item.getVol());
		blItemParm.setPkgQty(item.getPkgQty());
		blItemParm.setEachWgt(item.getEachWgt());
		blItemParm.setEachVol(item.getEachVol());
		
		UpdateItemsBizParm blUpdateitemBizParm = new UpdateItemsBizParm();
		DataItemList blDataItemList = new DataItemList();
		blDataItemList.add(blItemParm);
		blUpdateitemBizParm.setUpdateItems(blDataItemList);
		blDao.updateOriginalBlWgt(blUpdateitemBizParm);
