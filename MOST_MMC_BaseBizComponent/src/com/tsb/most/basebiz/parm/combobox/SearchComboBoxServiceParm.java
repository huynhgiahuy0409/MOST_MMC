package com.tsb.most.basebiz.parm.combobox;

import java.util.List;

import com.tsb.most.framework.bizparm.BaseBizParm;

public class SearchComboBoxServiceParm extends BaseBizParm {
	
	private String reqType;
	private String lcd;
	private String lcdNm;
	private String lcdDesc;
	private String mcd;
	private String mcdNm;
	private String scd;
	private String scdNm;
	private String scdUse;
	private String scdLgv;
	private String scdVal;
	private String tyCd;
	private String searchType; //ROLE:staff role
	private String unno;
	private String classes;	
	private String col1;
	private String col2;
	private String col3;
	private String col4;
	private String col5;
	private String divCd;
	private String pkgTypeCd;
	private String locDivCd;
	private String berthTp;
	private String berthCd;
	private String locCd;
	private String cd;
    private String cdNm;
	
	private String[] comboList;
	
	private String cgTp;
	
	private String eqTpCd;

	private String shftMethCd;
	private String shftDivCd;
	
	private String vslCallId;
	private String scn;
	private String mfDocNo; // master BL No
	private String blNo;
	private String ptnrCd;
	private String ptnrType;
	private String shipgNoteNo;
	private String authority;
	private String catgCd;
	
	private String eqDivCd;
	private String workYmd;
	
	private String staffType;
	private String shftId;
	private String cmdtGrp;
	private String cmdtCd;
	
	private String grNo;
	private String sDoNo;
	
	private String vslManifestNo;
	private String reqPos;
	private String opType;
	
	private String scrId;
	private String mfDocId;
	private String blNoSnNo;
	private String ssrTpCd;
	private String isMega;
	
	private String masterBL;
	private String bookingNo;
	private String cgTpCd;
	private String isGear;
	private String isCargoJob;
	//added by Tim 20/03/2024
	private String isPortCrane;
	
	// Roster Group list
	private String groupNm;
	private String groupCd;
	
	private String opeTp;
	private String fwAgent;
	private String estArrvDtFrom;
	private String estArrvDtTo;
	private String category;
	private String snNo;
	
	private String issuedDate;
	
	private String cs2YN;
	//s-OPR-015: Gate Operation – Modification
	private String lorryNo;
	private String driverId;
	//e-OPR-015: Gate Operation – Modification
	
	private String eqTpCdList;
	private String isVorScreen;
	
	
	public String getIsVorScreen() {
		return isVorScreen;
	}
	public void setIsVorScreen(String isVorScreen) {
		this.isVorScreen = isVorScreen;
	}
	public void setEqTpCdList(String eqTpCdList) {
		this.eqTpCdList = eqTpCdList;
	}
	
	public String getEqTpCdList() {
		return eqTpCdList;
	}

	public String getLorryNo() {
		return lorryNo;
	}

	public void setLorryNo(String lorryNo) {
		this.lorryNo = lorryNo;
	}

	public String getDriverId() {
		return driverId;
	}

	public void setDriverId(String driverId) {
		this.driverId = driverId;
	}

	public String getCs2YN() {
		return cs2YN;
	}

	public void setCs2YN(String cs2yn) {
		cs2YN = cs2yn;
	}

	public String getCmdtCd() {
		return cmdtCd;
	}

	public void setCmdtCd(String cmdtCd) {
		this.cmdtCd = cmdtCd;
	}

	public String getSnNo() {
		return snNo;
	}

	public void setSnNo(String snNo) {
		this.snNo = snNo;
	}

	public String getCategory() {
		return category;
	}

	public void setCategory(String category) {
		this.category = category;
	}

	public String getEstArrvDtFrom() {
		return estArrvDtFrom;
	}

	public void setEstArrvDtFrom(String estArrvDtFrom) {
		this.estArrvDtFrom = estArrvDtFrom;
	}

	public String getEstArrvDtTo() {
		return estArrvDtTo;
	}

	public void setEstArrvDtTo(String estArrvDtTo) {
		this.estArrvDtTo = estArrvDtTo;
	}

	public String getFwAgent() {
		return fwAgent;
	}

	public void setFwAgent(String fwAgent) {
		this.fwAgent = fwAgent;
	}

	public String getReqType() {
		return reqType;
	}

	public void setReqType(String reqType) {
		this.reqType = reqType;
	}

	public String getLcd() {
		return lcd;
	}

	public void setLcd(String lcd) {
		this.lcd = lcd;
	}

	public String getLcdNm() {
		return lcdNm;
	}

	public void setLcdNm(String lcdNm) {
		this.lcdNm = lcdNm;
	}

	public String getLcdDesc() {
		return lcdDesc;
	}

	public void setLcdDesc(String lcdDesc) {
		this.lcdDesc = lcdDesc;
	}

	public String getMcd() {
		return mcd;
	}

	public void setMcd(String mcd) {
		this.mcd = mcd;
	}

	public String getMcdNm() {
		return mcdNm;
	}

	public void setMcdNm(String mcdNm) {
		this.mcdNm = mcdNm;
	}

	public String getScd() {
		return scd;
	}

	public void setScd(String scd) {
		this.scd = scd;
	}

	public String getScdNm() {
		return scdNm;
	}

	public void setScdNm(String scdNm) {
		this.scdNm = scdNm;
	}

	public String getScdUse() {
		return scdUse;
	}

	public void setScdUse(String scdUse) {
		this.scdUse = scdUse;
	}

	public String getScdLgv() {
		return scdLgv;
	}

	public void setScdLgv(String scdLgv) {
		this.scdLgv = scdLgv;
	}

	public String getScdVal() {
		return scdVal;
	}

	public void setScdVal(String scdVal) {
		this.scdVal = scdVal;
	}

	public String getTyCd() {
		return tyCd;
	}

	public void setTyCd(String tyCd) {
		this.tyCd = tyCd;
	}

	public String getSearchType() {
		return searchType;
	}

	public void setSearchType(String searchType) {
		this.searchType = searchType;
	}

	public String getUnno() {
		return unno;
	}

	public void setUnno(String unno) {
		this.unno = unno;
	}

	public String getClasses() {
		return classes;
	}

	public void setClasses(String classes) {
		this.classes = classes;
	}

	public String getCol1() {
		return col1;
	}

	public void setCol1(String col1) {
		this.col1 = col1;
	}

	public String getCol2() {
		return col2;
	}

	public void setCol2(String col2) {
		this.col2 = col2;
	}

	public String getCol3() {
		return col3;
	}

	public void setCol3(String col3) {
		this.col3 = col3;
	}

	public String getCol4() {
		return col4;
	}

	public void setCol4(String col4) {
		this.col4 = col4;
	}

	public String getCol5() {
		return col5;
	}

	public void setCol5(String col5) {
		this.col5 = col5;
	}

	public String getDivCd() {
		return divCd;
	}

	public void setDivCd(String divCd) {
		this.divCd = divCd;
	}

	public String getPkgTypeCd() {
		return pkgTypeCd;
	}

	public void setPkgTypeCd(String pkgTypeCd) {
		this.pkgTypeCd = pkgTypeCd;
	}

	public String[] getComboList() {
		return comboList;
	}

	public void setComboList(String[] comboList) {
		this.comboList = comboList;
	}

	public String getLocDivCd() {
		return locDivCd;
	}

	public void setLocDivCd(String locDivCd) {
		this.locDivCd = locDivCd;
	}

	public String getBerthTp() {
		return berthTp;
	}

	public void setBerthTp(String berthTp) {
		this.berthTp = berthTp;
	}

	public String getBerthCd() {
		return berthCd;
	}

	public void setBerthCd(String berthCd) {
		this.berthCd = berthCd;
	}

	public String getLocCd() {
		return locCd;
	}

	public void setLocCd(String locCd) {
		this.locCd = locCd;
	}
	
	public String getCd() {
		return cd;
	}

	public void setCd(String cd) {
		this.cd = cd;	
	}
	
	public String getCdNm() {
		return cdNm;
	}
	
	public void setCdNm(String cdNm) {
		this.cdNm = cdNm;
	}
	
	public String getCgTp() {
		return cgTp;
	}

	public void setCgTp(String cgTp) {
		this.cgTp = cgTp;
	}

	public String getEqTpCd() {
		return eqTpCd;
	}

	public void setEqTpCd(String eqTpCd) {
		this.eqTpCd = eqTpCd;
	}

	public String getShftMethCd() {
		return shftMethCd;
	}

	public void setShftMethCd(String shftMethCd) {
		this.shftMethCd = shftMethCd;
	}

	public String getShftDivCd() {
		return shftDivCd;
	}

	public void setShftDivCd(String shftDivCd) {
		this.shftDivCd = shftDivCd;
	}

	public String getVslCallId() {
		return vslCallId;
	}

	public void setVslCallId(String vslCallId) {
		this.vslCallId = vslCallId;
	}

	public String getMfDocNo() {
		return mfDocNo;
	}

	public void setMfDocNo(String mfDocNo) {
		this.mfDocNo = mfDocNo;
	}

	public String getBlNo() {
		return blNo;
	}

	public void setBlNo(String blNo) {
		this.blNo = blNo;
	}

	public String getPtnrCd() {
		return ptnrCd;
	}

	public void setPtnrCd(String ptnrCd) {
		this.ptnrCd = ptnrCd;
	}

	public String getPtnrType() {
		return ptnrType;
	}

	public void setPtnrType(String ptnrType) {
		this.ptnrType = ptnrType;
	}

	public String getShipgNoteNo() {
		return shipgNoteNo;
	}

	public void setShipgNoteNo(String shipgNoteNo) {
		this.shipgNoteNo = shipgNoteNo;
	}

	public String getAuthority() {
		return authority;
	}

	public void setAuthority(String authority) {
		this.authority = authority;
	}

	public String getCatgCd() {
		return catgCd;
	}

	public void setCatgCd(String catgCd) {
		this.catgCd = catgCd;
	}

	public String getEqDivCd() {
		return eqDivCd;
	}

	public void setEqDivCd(String eqDivCd) {
		this.eqDivCd = eqDivCd;
	}

	public String getWorkYmd() {
		return workYmd;
	}

	public void setWorkYmd(String workYmd) {
		this.workYmd = workYmd;
	}

	public String getStaffType() {
		return staffType;
	}

	public void setStaffType(String staffType) {
		this.staffType = staffType;
	}

	public String getShftId() {
		return shftId;
	}

	public void setShftId(String shftId) {
		this.shftId = shftId;
	}

	public String getCmdtGrp() {
		return cmdtGrp;
	}

	public void setCmdtGrp(String cmdtGrp) {
		this.cmdtGrp = cmdtGrp;
	}

	public String getGrNo() {
		return grNo;
	}

	public void setGrNo(String grNo) {
		this.grNo = grNo;
	}

	public String getsDoNo() {
		return sDoNo;
	}

	public void setsDoNo(String sDoNo) {
		this.sDoNo = sDoNo;
	}

	public String getVslManifestNo() {
		return vslManifestNo;
	}

	public void setVslManifestNo(String vslManifestNo) {
		this.vslManifestNo = vslManifestNo;
	}

	public String getReqPos() {
		return reqPos;
	}

	public void setReqPos(String reqPos) {
		this.reqPos = reqPos;
	}

	public String getOpType() {
		return opType;
	}

	public void setOpType(String opType) {
		this.opType = opType;
	}

	public String getScrId() {
		return scrId;
	}

	public void setScrId(String scrId) {
		this.scrId = scrId;
	}

	public String getMfDocId() {
		return mfDocId;
	}

	public void setMfDocId(String mfDocId) {
		this.mfDocId = mfDocId;
	}

	public String getBlNoSnNo() {
		return blNoSnNo;
	}

	public void setBlNoSnNo(String blNoSnNo) {
		this.blNoSnNo = blNoSnNo;
	}

	public String getSsrTpCd() {
		return ssrTpCd;
	}

	public void setSsrTpCd(String ssrTpCd) {
		this.ssrTpCd = ssrTpCd;
	}

	public String getMasterBL() {
		return masterBL;
	}

	public void setMasterBL(String masterBL) {
		this.masterBL = masterBL;
	}

	public String getBookingNo() {
		return bookingNo;
	}

	public void setBookingNo(String bookingNo) {
		this.bookingNo = bookingNo;
	}

	public String getIsMega() {
		return isMega;
	}

	public void setIsMega(String isMega) {
		this.isMega = isMega;
	}

	public String getCgTpCd() {
		return cgTpCd;
	}

	public void setCgTpCd(String cgTpCd) {
		this.cgTpCd = cgTpCd;
	}

	public String getIsGear() {
		return isGear;
	}

	public void setIsGear(String isGear) {
		this.isGear = isGear;
	}

	public String getIsCargoJob() {
		return isCargoJob;
	}

	public void setIsCargoJob(String isCargoJob) {
		this.isCargoJob = isCargoJob;
	}

	public String getGroupNm() {
		return groupNm;
	}

	public void setGroupNm(String groupNm) {
		this.groupNm = groupNm;
	}

	public String getGroupCd() {
		return groupCd;
	}

	public void setGroupCd(String groupCd) {
		this.groupCd = groupCd;
	}

	public String getIsPortCrane() {
		return isPortCrane;
	}

	public void setIsPortCrane(String isPortCrane) {
		this.isPortCrane = isPortCrane;
	}

	public String getOpeTp() {
		return opeTp;
	}

	public void setOpeTp(String opeTp) {
		this.opeTp = opeTp;
	}
	

	public String getIssuedDate() {
		return issuedDate;
	}

	public void setIssuedDate(String issuedDate) {
		this.issuedDate = issuedDate;
	}

	public String getScn() {
		return scn;
	}

	public void setScn(String scn) {
		this.scn = scn;
	}
	
}
