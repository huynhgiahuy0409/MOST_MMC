...
public class TokenAuthenticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication auth) throws AuthenticationException {
		...
		if(((TokenAuthentication)auth).getType().equalsIgnoreCase(AuthConstant.TOKEN_TYPE_BEARER)){
			...
			credentialParm.setBranchCode(((TokenAuthentication)auth).getBranchCode());
			...
			if(item == null){
				...
				if(expiredCheck != null && expiredCheck.equals("Y")) {
					...
					if(auth instanceof TokenAuthentication){
						...
						item.setBranchCode(credentialParm.getBranchCode());
						
						UpdateItemsBizParm parm = new UpdateItemsBizParm();
						
						parm.setDataItem(item);
						parm.setBranchCode(item.getBranchCode());
