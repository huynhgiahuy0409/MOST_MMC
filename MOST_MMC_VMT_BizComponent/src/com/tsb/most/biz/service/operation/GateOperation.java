...
public class GateOperation extends MOSTBaseService implements IGateOperation {
	...
	public DataItemList selectGrInfo(SearchGateOperationParm parm) throws BizException {
		return gateOperationDao.selectGrInfo(parm);
	}
	...
	public DataItemList selectBlDoInfo(SearchGateOperationParm parm) throws BizException {
		return gateOperationDao.selectBlDoInfo(parm);
	}
...
//		}
	...
	public DataItemList selectGCGateOutItems(SearchGateOperationParm parm) throws BizException {
		...
		//Select to detect QR Code:
		DataItemList qrResult = gateOperationDao.selectQRTruckAssigedDetail(parm);
		GateOperationItem qrTruckItem = null;
		if(qrResult.size() > 0) {
			qrTruckItem = (GateOperationItem) qrResult.getCollection().get(0);
		} else {
			throw new BizException("QR_NOT_FOUND");
		}
		
		//Set more parameter:
		parm.setVslCallId(qrTruckItem.getVslCallId());
		parm.setBlNo(qrTruckItem.getBlNo());
		parm.setShipgNoteNo(qrTruckItem.getShipgNoteNo());
		parm.setLorryNo(qrTruckItem.getLorryNo());
		
		return gateOperationDao.selectGCGateOutItems(parm);
	}
