...
public class CargoLoading extends MOSTBaseService implements ICargoLoading {
	...
	public DataItemList selectCargoLoadingHatchList(SearchOperationSettingParm parm) throws BizException {
		...
		parm.setCgTpCd(CodeConstant.MT_CGTP_BBK);
	...
	public DataItemList updateCargoLoadingItems(UpdateItemsBizParm parm) throws BizException {
		...
		if("BV".equals(masterItem.getJobPurpCd())) {
			...
			CargoLoadingItem waJobItem = null;

			String prevJobGroupNo = null;
			...
			prevJobGroupNo = item.getJobGroup();
			...
			if (item.getOpDelvTpCd().equals(CodeConstant.MT_DELVTP_D)) {
				...
				if ((item.getCgTpCd().equals(CodeConstant.MT_CGTP_BBK) && !(item.getLoadMt() == 0 && item.getLoadQty() == 0))) {
					...
					if (flagWV) {
						/**
						 * READ ALL JOB BELONG TO GROUP AND PROCESS:
						 * */
						SearchCargoLoadingParm sParm = new SearchCargoLoadingParm();
						sParm.setVslCallId(item.getVslCallId());
						sParm.setCgNo(item.getCgNo());
						sParm.setJobGroup(prevJobGroupNo);
						DataItemList jobResList = cargoLoadingDao.selectWAJobList(sParm);
						ArrayList<CargoLoadingItem> jobArray = (ArrayList<CargoLoadingItem>) jobResList.getCollection();
						if(jobResList.size() == 0) {
							throw new BizException("No_Job");
						}
						
						for (CargoLoadingItem cargoLoadingItem : jobArray) {
							jobItem = (CargoLoadingItem) item.clone();
							...
							jobItem.setShuYn("N");
							jobItem.setJobCoCd(CodeConstant.INVLOC_WH_TP_NORMAL);
							jobItem.setCgNo(cargoLoadingItem.getCgNo());
							jobItem.setLoadMt(cargoLoadingItem.getLoadMt());
							jobItem.setLoadM3(cargoLoadingItem.getLoadM3());
							jobItem.setLoadQty(cargoLoadingItem.getLoadQty());
							jobItem.setPrevJobNo(cargoLoadingItem.getJobNo());
