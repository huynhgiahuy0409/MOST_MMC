Ext.define('MOST.view.popup.ShiftGroupMultiPopupController', {
						...
						var shiftGroupGrid = Ext.create('Ext.container.Container', {
							...
							items: [
								...
								},
								{
									...
									listeners: {
										celldblclick: 'onDblClickShiftGroup',
									},
									columns: {
										defaults: {
	...
	onDblClickShiftGroup: function (element, td, cellIndex, record, tr, rowIndex, e, eOpts) {
		var me = this;
		var staffListOfGroup = me.getStore('staffListOfGroup');

		staffListOfGroup.load({
			params: {
				shftGrpCd: record.get('groupCd'),
			},
			callback: function (records, operation, success) {
				if (success) {
					if (records.length > 0) {
