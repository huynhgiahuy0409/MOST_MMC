Ext.define('MOST.view.billing.ImportReconcileForLiquidController', {
	extend: 'MOST.view.foundation.BaseViewController',

	requires: [
	],

	alias: 'controller.importreconcileforliquid',
	...
	onLoad: function(){
		...
		var searchParm = Ext.create('MOST.model.billing.SearchImportReconcileForLiquidParm');
		...
		var importReconcilePackageTypeList = me.getStore('importReconcilePackageTypeList');
		var operationTypeCombo = me.getStore('operationTypeCombo');
		
		cargoTypeCombo.load();
		importReconcilePackageTypeList.load();
	...
	onGridComboRenderer: function(val, cell){
		...
		if(cell.column.dataIndex == 'cgTpCd'){
			...
			codeComboStore = me.getViewModel().getStore('importReconcilePackageTypeList');
		}
	...
	onSearch: function() {
		...
		var statusStore = me.getStore('importReconcileForLiquidStatusList');
		var mfStore = me.getStore(me.MF_STORE_NAME);
		var certStore = me.getStore(me.CERT_STORE_NAME);
			...
			callback: function(records, operation, success) {
				if (success) {
					if (records && records.length <= 0) {
						...
						me.onSearchOutturnCertificateList();
					}
				}
			}
		});
	...
	onSave: function(){
		...
		var statusStore = me.getStore('importReconcileForLiquidStatusList');
		var masterItem = Ext.create('MOST.model.billing.ImportReconcileForLiquid');
		var statusItem = Ext.create('MOST.model.billing.ImportReconcileForLiquid');
...
//			}
		...
		for(var i=0;i<arrItems.length;i++){
			if(arrItems[i].cgTpCd == null || arrItems[i].cgTpCd ==""){
				MessageUtil.warning('warning_msg', 'importexportreconcile_no_select_cargotype');
				return null;
			}
		}
...
//		To perform the save logic only when modified
		if(masterItem.dirty||arrItems.length>0){
			...
			updateParm.getProxy().url = MOST.config.Locale.getRestApiDestUrl() + '/v1/importreconcileforliquid/list';
	...
	onExportExcelPdfWithServer : function(gridNameString,isExcel) {
		...
		searchBizParm.classID = 'com.tsb.most.biz.parm.billing.SearchImportReconcileForLiquidParm';
		searchBizParm.serviceID = 'MOST.importReconcileForLiquid.selectImportReconcileForLiquidList';
	...
	onSetVerifyStatus: function(){
		...
		var store = me.getStore('importReconcileForLiquidStatusList');
		var searchParm = me.getViewModel().get('theSearch');
		var params = me.createParam(searchParm);
		params['vslCallId'] = StringUtil.toUpperCase(searchParm.get("vslCallId"));
		params['scn'] = StringUtil.toUpperCase(searchParm.get("scn"));
	...
	onSearchOutturnCertificateList: function() {
		var me = this;
     	var refs = me.getReferences();
		var store = me.getStore(me.CERT_STORE_NAME);
		var searchParm = me.getViewModel().get('theSearch');
		var params = me.createParam(searchParm);
		params['vslCallId'] = StringUtil.toUpperCase(searchParm.get("vslCallId"));
		params['scn'] = StringUtil.toUpperCase(searchParm.get("scn"));
