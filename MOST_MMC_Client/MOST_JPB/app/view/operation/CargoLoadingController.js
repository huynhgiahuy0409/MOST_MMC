const MT_CGTP_DBN = 'CodeConstants.MT_CGTP_DBN';
...
Ext.define('MOST.view.operation.CargoLoadingController', {
	...
	LIQUID_CD_LIST: ['LQN', 'LQE', 'LQD'], //LIQUID CARGO
	...
	onChangeShuAmout: function (ctl, newValue, oldValue, eOpts) {
		var me = this;
		var refs = me.getReferences();
		var detailItem = me.getViewModel().get('theDetail');
		var cgTpCd = detailItem.get('cgTpCd');
		...
		var loadaAvMt = detailItem.get('loadAvMt');
		var loadAvM3 = detailItem.get('loadAvM3');
		var shuQty = detailItem.get('shuQty');
		var shuMt = detailItem.get('shuMt');
		var shuM3 = detailItem.get('shuM3');
		
		if (cgTpCd === CodeConstants.MT_CGTP_BBK) {
			if (ctl.reference == 'ctlCgLdShutOutQty') {
				if(newValue != null && newValue > 0) {
					var eachMt = detailItem.get('projectCargo') == 'Y' ? detailItem.get('snMt') / detailItem.get('snQty') : detailItem.get('eachMt');
					var eachM3 = detailItem.get('projectCargo') == 'Y' ? detailItem.get('snM3') / detailItem.get('snQty') : detailItem.get('eachM3');

					shuMt = Number((eachMt * newValue).toFixed(3));
					shuM3 = Number((eachM3 * newValue).toFixed(3));

					refs.ctlCgLdShutOutMt.suspendEvents();
					refs.ctlCgLdShutOutM3.suspendEvents();
					detailItem.set('shuMt', shuMt);
					detailItem.set('shuM3', shuM3);
					refs.ctlCgLdShutOutMt.resumeEvents();
					refs.ctlCgLdShutOutM3.resumeEvents();

				} else if (newValue != null || newValue == 0 ) {
					detailItem.set('shuMt', 0);
					detailItem.set('shuM3', 0);
				}
			} 
		}
		//If change Amount after set Allocation => has to Re-Allocation with new Amount
		if(oldValue != null && (detailItem.get('shuLocId') != '' || (me.shuWhItems !=null && me.shuWhItems.length > 0))){
			detailItem.set('shuLocId', '');
			me.shuWhItems = new Array();
		}
	},
	...
	onSave: function () {
		...
		//LORRY NO
		let isTspTpLorry = detailItem.get('tsptTpCd') == CodeConstants.CGMST_TSPT_TP_LR;
		let isGV = refs.ctlRadioGV.getValue();
		let isAV = refs.ctlRadioAV.getValue();
		let lorryNo = detailItem.get('lorryNo');
		let isNullorEmptyLorryNo = StringUtil.isNullorEmpty(lorryNo);
		let cgTpCd = detailItem.get('cgTpCd');
		let dryBulkList = ['DBN', 'DBE', 'DBN'];

		//OPR-006 Operation Loading/Discharging Confirmation with Conveyor Mode.
		if (isNullorEmptyLorryNo) {
			if (!dryBulkList.includes(cgTpCd) && isGV && isTspTpLorry) {
			...
			if (me.LIQUID_CD_LIST.includes(cgTpCd)) {
		...
		//Check Shut out Cargo:
		var shuQty = detailItem.get('shuQty');
		var shuMt = detailItem.get('shuMt');
		var shuM3 = detailItem.get('shuM3');
		var shuLocId = detailItem.get('shuLocId');
		var isShutOutCargo = shuQty > 0 || shuMt > 0 || shuM3 > 0;

		if (isShutOutCargo && (shuLocId == null || me.shutItems.length == 0)) {
			MessageUtil.warning('warning_msg', 'confirmloading_loading_shutout_location_msg');
			return;
		}
		me.prevSaveCheck();
	},
	...
	/**
	 * OPR-025 Shut out Cargo process
	 * @param controlName
	 * @returns
	 * call WH Popup Set Location for Shut out Cargo
	*/
	onAllocationShutOut: function (controlName) {
		var me = this;
		var refs = me.getReferences();
		var detailItem = me.getViewModel().get('theDetail');
		var shuQty = detailItem.get('shuQty');
		...
		} else if (me.DBK_CD_LIST.includes(cgTpCd)) {
			if (shuMt <= 0 && shuM3 <= 0) {
				MessageUtil.warning('warning_msg', 'confirmhandlingin_amount_mt_zero');
				return;
			}
		}

		var selection = Ext.create('MOST.model.operation.CargoHandlingIn', {
			vslCallId: detailItem.get('vslCallId'),
			whTpCd: 'S',
	...
	setDetailInitialize: function (masterItem) {
		...
		if (recvData.get('conveyerCheck')) {
			detailItem.set('tsptTpCd', CodeConstants.CGMST_TSPT_TP_CV);
		}
		...
		if (recvData.get('jobNo')) {//Load from WA Job
			//Amount Indirect:
			detailItem.set('loadAvQty', detailItem.get('projectCargo') == 'Y' ? 0 : recvData.get('yardTruckQty'));
			detailItem.set('loadAvMt', detailItem.get('projectCargo') == 'Y' ? 0 : recvData.get('yardTruckMt'));
			detailItem.set('loadAvM3', detailItem.get('projectCargo') == 'Y' ? 0 : recvData.get('yardTruckM3'));
		...
		refs.ctlLoadGvMt.setEditable(cgTpCd !== CodeConstants.MT_CGTP_BBK);
		refs.ctlLoadGvM3.setEditable(cgTpCd !== CodeConstants.MT_CGTP_BBK);

		refs.ctlLoadGaMt.setEditable(cgTpCd !== CodeConstants.MT_CGTP_BBK);
		refs.ctlLoadGaM3.setEditable(cgTpCd !== CodeConstants.MT_CGTP_BBK);

		refs.ctlLoadAvMt.setEditable(cgTpCd !== CodeConstants.MT_CGTP_BBK);
		refs.ctlLoadAvM3.setEditable(cgTpCd !== CodeConstants.MT_CGTP_BBK);

		refs.ctlLoadBvMt.setEditable(cgTpCd !== CodeConstants.MT_CGTP_BBK);
		refs.ctlLoadBvM3.setEditable(cgTpCd !== CodeConstants.MT_CGTP_BBK);

		if (me.LIQUID_CD_LIST.includes(cgTpCd)) {
			refs.ctlLoadGvMt.setEditable(true);
			refs.ctlLoadGvM3.setEditable(true);
		}
	...
	afterSetCodePopupData: function (xtype, targetControl, returnValue) {
		...
		if (targetControl === 'refYardTruck' || targetControl === 'ctlConfirmLoadingLorryNo') {
		...
		else if(targetControl == 'ctlCgLdShuLocId') {
			...
			me.shutItems = whItems;
			me.isShuWhAllocation = true;
		}
	},
	...
	operationSeting: function (startDtStr, endDtStr) {
		...
		if (recvData.get('cgTpCd') === MT_CGTP_DBN) {
	...
	save: function () {
		...
		var docMT = detailItem.get('snMt');
		var docQty = detailItem.get('snQty');
		var docM3 = detailItem.get('snM3');
		var loadingMT = detailItem.get('loadMt');
		var loadingQty = detailItem.get('loadQty');
		var loadingM3 = detailItem.get('loadM3');

		var isM3LoadExceeded = (actM3Load + loadingM3) > docM3 && docM3 > 0;
		var isMTLoadExceeded = (actMTLoad + loadingMT) > docMT && docMT > 0;
		var isQtyLoadExceeded = (actQtyLoad + loadingQty) > docQty && docQty > 0;
		var isFinalOperation = detailItem.get('fnlOpeYn') === 'N';

		var isCgTpDBN = detailItem.get('cgTpCd') === CodeConstants.MT_CGTP_DBN;
		var isCgTpLqd = me.LIQUID_CD_LIST.includes(detailItem.get('cgTpCd'));

		//Block Liquid Cargo overloading:
		if (isCgTpLqd && (isM3LoadExceeded || isMTLoadExceeded)) {
			MessageUtil.warning('warning_msg', 'confirmloading_liquid_over_msg');
			return;
		}

		if (loadingMT == 0 && loadingM3 == 0 && me.LIQUID_CD_LIST.includes(detailItem.get('cgTpCd'))) {
			MessageUtil.warning('warning_msg', 'confirmloading_liquid_zero_msg');
			return;
		}

		//Break Bulk and Dry Bulk Cargo over loading:
		if ((isM3LoadExceeded || isMTLoadExceeded || isQtyLoadExceeded) && isFinalOperation) {
			if (isCgTpDBN) {
	...
	setLoadingItems: function () {
		...
		detailItem.set('shutItems', me.shutItems);
	},
	...
	checkInDirectBalanceAmt: function () {
		...
		if (detailItem.get('cgTpCd') && detailItem.get('cgTpCd') === CodeConstants.MT_CGTP_BBK) {
			if ((!loadQty || loadQty == 0) && (!shuQty || shuQty == 0)) {
				MessageUtil.warning('warning_msg', 'confirmloading_qty_zero_msg');
				return false;
			}
			if (loadQty + shuQty > balQty) {
			...
			if(loadMt + shuQty > balMt || loadM3 + shuM3 > balM3 || loadQty + shuQty > balQty){
