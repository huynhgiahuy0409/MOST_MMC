Ext.define('MOST.view.operation.CargoDischargingController', {
	...
	onLoad: function(){
		...
		if (recvData.data.projectCargo == 'Y') {
			...
			refs.ctlConfirmDischargingLoadQty.setDisabled(true);
...
//			refs.ctlConfirmDischargingAprBargeM3.setValue(0);
	...
	setDetailInitialize:function(masterItem){
		...
		if(masterItem.get('items') != null && masterItem.get('items').length > 0){
			...
			//Set by Cargo Type:
			if(me.LIQUID_CD_LIST.includes(detailItem.get('cgTpCd'))){//Check Liquid Cargo
				refs.ctlIndirectOperation.setDisabled(true);
			}
...
//						},
	...
	prevSaveCheck : function(){
		...
		//Re-assign SCN
		if(StringUtil.isNullorEmpty(detailItem.get('scn'))){
		...
		if(refs.ctlDirectOperation.checked
			...
			//Check Liquid Cargo
			if(me.LIQUID_CD_LIST.includes(detailItem.get('cgTpCd'))){
				if(detailItem.get('tsptTpCd') === 'PL'){
					
				}

				//Check Liquid Cargo only for Direct Operation
				if(!refs.ctlDirectOperation.checked){
					MessageUtil.warning('warning_msg', 'confirmdischarging_liquid_block'); 
					return;
				}

				if(Number(detailItem.get('loadQty')) > Number(detailItem.get('balQty'))
					|| Number(detailItem.get('loadMt')) > Number(detailItem.get('balMt'))
					|| Number(detailItem.get('loadM3')) > Number(detailItem.get('balM3'))){  
						
					MessageUtil.warning('warning_msg', 'confirmdischarging_exceed_liquid_amount'); 
					return;
				}
...
//			}
	...
	onPassedBillingDocumentHoldValidation: function() {
		...
		var refs = me.getReferences();
		var detailItem = me.getViewModel().get('theDetail');
		var isDirect = refs.ctlDirectOperation.checked;
		var isIndirect = refs.ctlIndirectOperation.checked;
	
		/**
		 * OPR-005.
		 * Direct Discharging: check Custom release
		 * Indirect Discharging: check ZB1 release
		*/
		if (isDirect) {
			// Perform customs hold validation if domesticChk is 'N' and custom hold check is 'Y'
