Ext.define('MOST.view.operation.GateOperationController', {
	...
	onLoad: function(){
		...
		refs.refGateInGCLorryField.refs.ctlField.setReadOnly(true);		
	...
	onTabChange : function(tabPanel, newCard, oldCard, eOpts) {
		...
		switch(tabTitle) {
			...
			case 'GeneralCargoGateOut':
				me.tabMode = 'gateOut';
				me.setEditableUserControl('refGateInGr');
				refs.ctlGateOutLorryField.refs.ctlField.setReadOnly(true);
				break;
			case 'ROROGateOut':
				me.tabMode = 'gateOut';
	...
	onMasterTabChange : function(tabPanel, newCard, oldCard, eOpts) {
		...
		me.getViewModel().setData({gateInGC: null});
	...
	saveGateInCargo: function(){
		...
		var gateInGC = me.getViewModel().get('gateInGC');
		var grid = me.lookupReference('refGateInCargo');
		var insertItem = grid.getSelection() == null ? null : grid.getSelection()[0];
		...
		gcGateOutstore.load({
			callback: function(records, operation, success) {
				if (success) {
					for(var i = 0; i < gcGateOutstore.data.length; i++){			
						if(insertItem.get('lorryNo') == gcGateOutstore.data.items[i].data.lorryNo){
							isInTerminal = true;
							break;				
						}
					}
					...
					if(refs.refGateInGCTruck.value == true && (gateInGC.get('lorryNo') == null || gateInGC.get('lorryNo') == '')){
						MessageUtil.error('warning_msg','gateoprations_truckNo_inquiry_msg');
						return;
					}
					
					if(refs.refGateInGCDriver.value == true && (gateInGC.get('driverID') == null || gateInGC.get('driverID') == '')){
						MessageUtil.error('warning_msg','gateoprations_driverId_inquiry_msg');
						return;
					}
					...
					updateParm.save({
						success: function(records,success) {
							MessageUtil.saveSuccess();
							grid.getStore().remove(insertItem);
						}
					});
	...
	onDamage_clickHandler: function(){
		...
		me.openCodePopup('app-damagecheckregistration', 'Damage Check', me.prevData);
	},
	...
	onGridGOREClick:function(){
		var me = this;
		var refs = me.getReferences();
		var grid = me.lookupReference('refGateInCargo');
		var selection = grid.getSelection() == null ? null : grid.getSelection()[0];
		var theGateInGc = me.getViewModel().get('gateInGC');
		
		if(selection == null) return;
		
		if(theGateInGc != null){			
			if(theGateInGc.driverId == undefined){
				var driverId = theGateInGc.get('driverId');
				var driverNm = theGateInGc.get('driverNm');
				var liscNo = theGateInGc.get('liscNo');
				var expdate = theGateInGc.get('expdate');
				var tsptr = theGateInGc.get('tsptr');
				var tsptCompNm = theGateInGc.get('tsptCompNm');
			}else{
				var driverId = theGateInGc.driverId;
				var driverNm = theGateInGc.driverNm;
				var liscNo = theGateInGc.liscNo;
				var expdate = theGateInGc.expdate;
				var tsptr = theGateInGc.tsptr;
				var tsptCompNm = theGateInGc.tsptCompNm;
			}
			...
			me.getViewModel().setData({gateInGC: selection});
		}else{
			me.getViewModel().setData({gateInGC: selection});
		}
	},
...
//			if(selection.get("secondWgtDt") == '' || selection.get("secondWgtDt") == null){
//				MessageUtil.warning('portsafetyconfirmation_validcheck_gate_mandatory_title', "printCir_gateoperation");
//				return;
//			}
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'refGateInRoRoLorryField'){
			...
			if(returnValue){
				me.getViewModel().setData({gateInGC: null});
				me.getViewModel().setData({gateInGC: returnValue.item});
				//refs.refGateInGCLorryField.setValue(returnValue.code);
				refs.ctlGateInGCDriverIdField.setValue(returnValue.item.get('driverId'));
				refs.refGateInDriverName.setValue(returnValue.item.get('driverName'));
				refs.refGateInLicenseNo.setValue(returnValue.item.get('licenseNo'));
				refs.refGateInExpiryDate.setValue(returnValue.item.get('licenseExpired'));
				refs.refGateInTransporter.setValue(returnValue.item.get('transportCd'));
				refs.refGateInTransporterName.setValue(returnValue.item.get('transportName'));
				var gateInGCStore = me.getStore('gateInCargo');
				gateInGCStore.load({
					params : {
						lorryNo: returnValue.code
		...
		// GateOut afterSetCodePopupData =============================================================
			...
			if (returnValue) {
				...
				refs.ctlGateOutDriverIdField.setValue(returnValue.item.get('driverId'));
				refs.txtGateOutDriverName.setValue(returnValue.item.get('driverName'));
				refs.txtGateOutLicenseNo.setValue(returnValue.item.get('licenseNo'));
				refs.txtGateOutExpiryDate.setValue(returnValue.item.get('licenseExpired'));
				refs.txtGateOutTransporter.setValue(returnValue.item.get('transportCd'));
				refs.txtGateOutTransporterName.setValue(returnValue.item.get('transportName'));
