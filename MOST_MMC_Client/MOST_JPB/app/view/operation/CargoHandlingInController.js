Ext.define('MOST.view.operation.CargoHandlingInController', {
	...
	onChangeHandlingInShuAmt: function(ctl, newValue, oldValue, eOpts){
		var me = this;
		var refs = me.getReferences();
		var detailItem = me.getViewModel().get('theDetail');
		...
		if (cgTpCd === CodeConstants.MT_CGTP_BBK) {
			if (ctl.reference == 'ctlConfirmHandlingInShuQty') {
				if (newValue != null && newValue > 0) {
					var noEachMt = detailItem.get('eachMt') == 0 || detailItem.get('eachMt') == undefined || detailItem.get('eachMt') == null;
					var noEachM3 = detailItem.get('eachM3') == 0 || detailItem.get('eachM3') == undefined || detailItem.get('eachM3') == null;

					var eachMt = !noEachMt ? detailItem.get('eachMt') : detailItem.get('snMt') / detailItem.get('snQty');
					var eachM3 = !noEachM3 ? detailItem.get('eachM3') : detailItem.get('snM3') / detailItem.get('snQty');

					shuMt = Number((eachMt * newValue).toFixed(3));
					shuM3 = Number((eachM3 * newValue).toFixed(3));

					refs.ctlConfirmHandlingInShuMt.suspendEvents(true);
					refs.ctlConfirmHandlingInShuM3.suspendEvents(true);

					detailItem.set('shuMt', shuMt);
					detailItem.set('shuM3', shuM3);

					refs.ctlConfirmHandlingInShuMt.resumeEvent();
					refs.ctlConfirmHandlingInShuM3.resumeEvent();

				} else if (newValue == undefined || newValue != null || newValue == 0) {
		...
		//If change Amount after set Allocation => has to Re-Allocation with new Amount
		if(oldValue != null && (detailItem.get('shuLocId') != '' || me.shuWhItems.length > 0 )){
	...
	onWarehouseAllocation : function(controlName){
		...
		if(controlName === 'ctlConfirmHandlingInLocId'){
		...
		else if(controlName === 'ctlConfirmHandlingInShuLocId'){
			if(detailItem.get('cgTpCd') != null && detailItem.get('cgTpCd') == CodeConstants.MT_CGTP_LQN){
				if(	refs.ctlConfirmHandlingInShuM3.getValue() <= 0){
				...
				if(	refs.ctlConfirmHandlingInShuMt.getValue() <= 0){
			...
			selection = Ext.create('MOST.model.operation.CargoHandlingIn', {
				...
				hdlInDt: detailItem.get('hdlInEndDt'),
				cgNo: refs.ctlBlGr.getValue(),
				grMt: refs.ctlConfirmHandlingInShuMt.getValue(),
				grM3: refs.ctlConfirmHandlingInShuM3.getValue(),
				grQty: refs.ctlConfirmHandlingInShuQty.getValue(),
				catgCd : detailItem.get('catgCd'),
				eachMt : detailItem.get('eachWgt'),
				eachM3 : detailItem.get('eachMsrmt'),
				cgTpCd: detailItem.get('cgTpCd')
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl == 'ctlHILorryNo') {
			...
			if (returnValue) {
				whItems = new Array();
				for (var i = 0; i < returnValue.data.whConfigurationMap.data.items.length; i++) {
					var shuInvLocItem = returnValue.data.whConfigurationMap.data.items[i];
					var handlingItem = Ext.create('MOST.model.configuration.WhConfiguration', {
						locTpCd: shuInvLocItem.data.locTpCd,
						whId: shuInvLocItem.data.whId,
						locId: shuInvLocItem.data.locId,
						vslCallId: shuInvLocItem.data.vslCallId,
						cgNo: shuInvLocItem.data.cgNo,
						wgt: shuInvLocItem.data.wgt,
						msrmt: shuInvLocItem.data.msrmt,
						pkgQty: shuInvLocItem.data.pkgQty,
						whTpCd: shuInvLocItem.data.whTpCd,
						whTpCdNm: shuInvLocItem.data.whwhTpCdNmId,
						spCaCoCd: ''
					});
					whItems.push(handlingItem.data);
					//me.whType = shuInvLocItem.data.locTpCd;
				}
...
//			detailItem.set('whBalQty', detailItem.get('balQty') - detailItem.get('accuSumQty'));
			...
			var rmQty = detailItem.get('pkgQty') - detailItem.get('accuSumQty');
			detailItem.set('pkgQty', rmQty < 0 ? 0 : rmQty);
	...
	amtVal : function(){
		...
		var shuMt = detailItem.get('shuMt');
		var shuM3 = detailItem.get('shuM3');
		var isShutOutCargo = shuQty > 0 || shuMt > 0 || shuM3 > 0 || detailItem.get('shuLocId') != '';

		
		//BBK: just input Qty => ato calculate MT M3 by eachWgt eachM3:
		if(cgTpCd && cgTpCd === CodeConstants.MT_CGTP_BBK){
			if(actNorQty <= 0){
...
//			}
		...
		//LQD
		if(cgTpCd && cgTpCd === CodeConstants.MT_CGTP_LQN){
			if(actNorM3 <= 0){
...
//		}
	...
	cudFunction : function(){
		...
		detailItem.set('shuWhItems', me.shuWhItems);
	...
	//MOST_MMC_JPB.OPR-005
	onCustomsHoldValidation : function(){
		var me = this;
		var detailItem = me.getViewModel().get('theDetail');
		var store = me.getStore('validationCheck');
		var skipCustomHold = Token.getCustomHoldChk() === 'N';
			...
			callback: function(records, operation, success) {
				if (success) {
					if(records.length === 0 || (records[0].get('isValidated') !== 'Y')){
						MessageUtil.warning('warning_msg', 'confirmhandlingin_custom_block');
						return;
