Ext.define('MOST.view.operation.ShiftingDoubleBankingController', {
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'txtJpvc'){
            ...
            if(returnValue){
                ...
                if(!StringUtil.isNullorEmpty(returnValue.item.get('vslCallId'))){
                        ...
                        me.jpvcData = returnValue.item.data;
                        me.getViewModel().setData({vslDetail:returnValue.item});
                        me.onSearch();
                }else {
		...
		}  else if(targetControl === 'refSTSPkgType'){
			refs.refSTSPkgType.setValue(returnValue.codeName);
		}
	...
	onStsGridClick:function(dv, record, index, item, e){
		...
		if(selection == null) return;
		refs.refStsCargoType.setDisabled(false)
	...
	onSelectOprMode:function(ele, rec, idx){
		var me = this;
		var refs = me.getReferences();
		var oprMode = refs.refStsOprMode.getValue()
		if(oprMode){
			refs.refStsCargoType.setDisabled(false);
		}else{
			refs.refStsCargoType.setDisabled(true);
		}
		refs.refStsCargoType.setValue()
	},
...
//		}				
	...
	onShiftingPositionSelect: function(){
		var me = this;
		var refs = me.getReferences();
		var shiftingPosition = refs.refCobShftPosition.getValue();
		var enableRefs = []
		var disableRefs = []
		var emptyRefs = []
		if(shiftingPosition && shiftingPosition === 'AB'){
			enableRefs.push('refVslShifting2ndATA')
		}else{
			disableRefs.push('refVslShifting2ndATA')
			emptyRefs.push('refVslShifting2ndATA')
		}
		enableRefs.forEach(item => refs[item].setDisabled(false))
		disableRefs.forEach(item => refs[item].setDisabled(true))
		emptyRefs.forEach(item => refs[item].setValue(''))
	},
...
//		refs.ctl3ndAtu.setDisabled(isAtu);
	...
	onAddDoubleBankingClick: function(){
		...
		if((refs.ctl3ndJpvc.getValue() != null && refs.ctl3ndJpvc.getValue() != '')
			...
			if(refs.refCboBankingType.getValue() != 'BG'){
		...
		if(((refs.ctl2ndJpvc.getValue() != '' && refs.ctl3ndJpvc.getValue() != '')
			...
			MessageUtil.warning('warning_msg', "Duplicated Data between 2nd Vessel and 3th Vessel, please recheck");
			return;
		}
...
//		}
		...
		if(dblBankingStore.data.length > 0){
...
//		validObj.msg += me.getDblBnkFieldsetInvalidItems(refs.ref3rdVslFldset.items.items, '3nd');
	...
	insertNewShipToShipData: function(){
		...
		var refs = me.getReferences();
		var stsOperationList = me.getStore('stsOperationList');
		var cgTpStore = me.getStore('cgTpListCombo');
		var record = Ext.create('MOST.model.operation.ShiftingDoubleBanking');
	...
	onAddVesselShifting: function(){
		...
		vslShftItem.set('scn', refs.ctlScn.getValue())
		vslShftItem.set({ workingStatus : WorkingStatus.INSERT });
		if(!me.validateVslShftgDateFields(vslShftItem).valid){
			MessageUtil.warning('Warning', me.validateVslShftgDateFields(vslShftItem).msg);
			return false;
		} 
	...
	validateVslShftgDateFields: function(record){
		...
		var msg = 'Time should be followed :<br/>' +
				'Prev ATB -> Prev ATU -> New ATB -> 2nd ATA -> ATW -> ATC -> New ATU';
		...
		dateObj = {
			...
			secondAta : (record) ? record.get('ataDate') : refs.refVslShifting2ndATA.getValue(),
		...
		Ext.Object.each(dateObj, function(key, value, myself){
			if(value !== null && key !== 'newAtb' && ['currAtu', 'currAtb', 'secondAta'].includes(key)
	...
	initVslShftgItem: function () {
		...
		record.set('atwDate', vslCurrWharftData.get('prevAtw'));
		record.set('atcDate', vslCurrWharftData.get('prevAtc'));
...
//					sendArray.push(record.data);
	...
	onBindDocAmountSts:function(){
		var me =this;
		var refs = me.getReferences();
		var theShipToShip = me.getViewModel().get('theShipToShip');
		var amountByOPRModeList = me.getStore('amountByOPRModeList');
		var sltOprMode = theShipToShip.get('stsOpTp')
		
		var oprMode = sltOprMode === 'SL' ? 'SL' : sltOprMode == 'SD' ? 'SD' : '';
		var cgTp = theShipToShip.get('cgTpCd');

		if(!cgTp){
			missingRef = 'refStsCargoType'
		}else{
			isComplete = true
		}
		if(!isComplete && missingRef && refs[missingRef] && isStsTab){
			MessageUtil.warning('warning_msg', `Please select ${refs[missingRef].getFieldLabel()}`);
			return
		}

		amountByOPRModeList.clearFilter();
		amountByOPRModeList.filterBy(function(record, id){
			if(record.get('cgOptTpCd') === oprMode && record.get('cgTpCd') === cgTp){
				return record;
			}
		});
		...
		if(amountByOPRModeList.getData().items.length > 0){
			var filterData = amountByOPRModeList.getData().items[0];
			theShipToShip.set('docMt', filterData.get('mt'));
			theShipToShip.set('docM3', filterData.get('m3'));
			theShipToShip.set('docQty', filterData.get('qty'));
			theShipToShip.commit();
		}else{
			theShipToShip.set('docMt', 	'0');
			theShipToShip.set('docM3', 	'0');
			theShipToShip.set('docQty', '0');
			theShipToShip.commit();
		}
	},
