Ext.define('MOST.view.operation.hht.WarehouseCheckForImportHHTController', {
	...
	**/
	...
	prevSaveCheck : function(){
		...
		if (cgTpCd === CodeConstants.MT_CGTP_BBK && whQty >= aprBalQty) {
		...
		} else if (cgTpCd === CodeConstants.MT_CGTP_DBN && whMt >= aprBalMt) {
			MessageUtil.questionModern('info_msg', 'confirmdischarging_isfinalope_msg', null,
				function (button) {
					if (button == 'ok') {
						me.saveHHT('Y')
...
//		}
	...
	checkAmountPreSave: function () {
		...
		if (cgTpCd == CodeConstants.MT_CGTP_BBK && aprBalQty <= 0) {
...
//		}
		...
		if (cgTpCd == CodeConstants.MT_CGTP_BBK && whQty <= 0) {
			MessageUtil.warning('warning_msg', 'hht_whcheckimport_bbk_zero_amount');
			return false;
		} else if (cgTpCd == CodeConstants.MT_CGTP_DBN && whMt <= 0) {
	...
	onWHiTabDamage: function(){
		...
		var refs = me.getReferences();
		var recvData = me.getView().recvData;
		ViewUtil.openCodePopup(this, 'app-damageofcargo', 'refsWHiBtnDamage', recvData);
	...
	onWHiTabDimension: function(){
		...
		var refs = me.getReferences();
		var recvData = me.getView().recvData;
		ViewUtil.openCodePopup(this, 'app-dimensionofcargohht', 'refsWHiBtnDimension', recvData);
	},
