Ext.define('MOST.view.operation.hht.VSRCheckListHHTController', {
...
//			}
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if (Ext.platformTags.modern){
			...
			}else if(targetControl == 'refBtnWorkingAreaStv'){
	...
	onGridPortCraneClick: function(){
		...
		if(!StringUtil.isNullorEmpty(selection.get('empId'))){
			refs.refPortCraneJPBRdo._checked = true;
			refs.refPortCraneJPBRdo.updateChecked(true);
		}
		if(!StringUtil.isNullorEmpty(selection.get('cnrtCd'))){
	...
	onChangeJPBGroup: function(radioField,newValue, oldValue) {
		...
		if(newValue){
			...
			if(radioField.getValue() == me.ADP){
				me.getViewModel().setData({JPBModeHHT:newValue});
				refs.refPCContractorHHTField.setValue('');
			}
			else if(radioField.getValue() == 'contractor'){
				me.getViewModel().setData({JPBModeHHT:!newValue});
				refs.refVSRJPBscbx.clearValue();
			}
			
			refs.refVSRJPBscbx.setRequired(isJPB)
			refs.refVSRJPBscbx.setReadOnly(!isJPB);
			refs.refVSRJPBscbx.setDisabled(!isJPB);

			refs.refPCContractorHHTField.setRequired(isCTR);
			refs.refPCContractorHHTField.setReadOnly(!isCTR);
			refs.refPCContractorHHTField.setDisabled(!isCTR);
			
			refs.refBtnContractorPC.setDisabled(!isCTR);
		}
	},
	...
	onAddPortCraneHHT:function(){
		...
		detailData.workYmd = refs.refWorkingDate.getDate();
		detailData.shftId = refs.refCbxShft.getValue();
...
//		}
	...
	onSearchRequestorStvHHT:function(){
		...
		ViewUtil.openCodePopup(me, 'app-requesterpopuphht', 'refBtnSearchRequesterStv', params);
	},
	...
	onSearchWorkingAreaStvHHT:function(){
		...
		ViewUtil.openCodePopup(me, 'app-workingareapopuphht', 'refBtnWorkingAreaStv', params);
	},
...
//		var endDt = Ext.Date.parse(strEndDt, 'd/m/Y H:i');
	...
	onSelectGridForkLiftHHT: function (){
		...
		refs.refCboJPBFL.setValue(rowDetail.empId.trim());
		refs.refTxtContractorFL.setValue(rowDetail.cnrtCd.trim());
		...
		var sltIndex = grid.store.indexOf(selecedRecord);	
			store.each(function(record, index){
				if(index === sltIndex){					
					...
					record.set("payer", refs.refTRRequestorHHT.getValue());
					record.set("mbsCd", 'CTR');
					record.set("refNo", refs.refTRTxtRefNoHHT.getValue());
...
//	},
    ...
    */
	onSearchPartnerHHT: function(ref){
		...
		var targetCtl = '';
		if(ref.reference == 'refBtnSearchCntrFL'){//Contrartor Forlift Tab
			title = 'Contractor';
			searchType = 'CTT';
			targetCtl = 'refTxtContractorFL';
		} else if(ref.reference == 'refBtnSearchContractorTrHHT'){//Contrartor trailer Tab
			title = 'Contractor';
			searchType = 'CTT';
			targetCtl = 'refContractorTrHHTField';
		}
		else if(ref.reference == 'refMEQF2'){//Contrartor MEQ Tab
			title = 'Contractor';
			searchType = 'CTT';
			targetCtl = 'refMEQContractorCd';
		}else if(ref.reference == 'refBtnContractorPC'){//Contrartor PC Tab
			title = 'Contractor';
			searchType = 'CTT';
	...
	validateTimeShift: function(refStartTime, refEndTime, refSetUpTime){//'d/m/Y H:i'
		...
		var shiftId = refs.refCbxShft.getValue();
		var shiftIdx = refs.refCbxShft.getSelection().get('shftIdx');
		var shift = shiftListStore.findRecord('shftIdx', shiftIdx);
		var strWKDate = Ext.Date.format(MOST.config.Token.getWorkDate(), MOST.config.Locale.getDefaultDateFormatWithNoSeconds());
		var fmShiftTime;
		...
		var strFromDt = strShftDt + ' ' + shift.data.fmHhMm.substr(0, 2) + ':' + shift.data.fmHhMm.substr(2, 4);
		var strToDt = strShftDt + ' ' + shift.data.toHhMm.substr(0, 2) + ':' + shift.data.toHhMm.substr(2, 4);
		stShftDTime =  Ext.Date.parse(strFromDt,  'd/m/Y H:i');
		endShftDTime = Ext.Date.parse(strToDt, 'd/m/Y H:i');
		
		if(sltedShift.get('shftIdx') === '3'){
			endShftDTime = new Date(endShftDTime.getTime() + (24 * 60 * 60 * 1000));
		}
