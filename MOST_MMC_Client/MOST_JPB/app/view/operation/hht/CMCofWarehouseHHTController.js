Ext.define('MOST.view.operation.hht.CMCofWarehouseHHTController', {
	...
	onOpenTruckPopup: function(){
	    ...
	    ViewUtil.openCodePopup(me, popupAlias,  'refTxtLorryNo', params);
	...
	setConstrolSetting: function (tab) {
		...
		if(tab == 'refPnlExport'){
			...
			if(me.LIST_TYPE == 'GR') {
				refs.refFlsGrLoadingGrid.setHidden(false);
				refs.refFlsJobLoadingGrid.setHidden(true);
			}
			else {
				refs.refFlsGrLoadingGrid.setHidden(true);
				refs.refFlsJobLoadingGrid.setHidden(false);
			}
		} else {
			me.caTyCd = 'IM';
			isDs = true;
	...
	showTblWhCheckPopUp(){
		...
		if(me.LIST_TYPE == 'JOBVA') {
			...
			if(tab == 'refPnlImportt'){
				...
				searchParams = selection.clone();
				searchParams.title = title;
				ViewUtil.openHhtPopup(me, 'app-warehousecheckforexporthht', 'refBtnWHcheckExport', searchParams);
			}
		}
	},
