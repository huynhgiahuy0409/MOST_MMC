Ext.define('MOST.view.operation.WHCheckExportController', {
	...
	isShuWhAllocation: false,
	shuWhItems : new Array(),

	packageItems : new Array(),
	whType: '',

	DBK_CD_LIST: ['DBN', 'DBE', 'DBK'], //DRY BULK CARGO
	/**
	 * CONSTANT END
	 * =========================================================================================================================
	 */
...
//		}
		...
		if(detailItem.get('projectCargo') != 'Y' && newValue > 0 && newValue!= null){
			me.autoDeAllocation();
		}
	},

	onChangeShutOutAmt: function(clt, newValue, oldValue, eOpts){
		var me = this;
		var refs = me.getReferences();
		var detailItem = me.getViewModel().get('theDetail');
		var cgTpCd = detailItem.get('cgTpCd');		
		var cgTpNeedToCheck = ['BBK'];
		
		//Auto Calculate with BBK:
		if(clt.reference === 'ctlShutOutQty' 
			&& cgTpNeedToCheck.includes(cgTpCd)){
			...
			detailItem.set('shuMt', mt);
			detailItem.set('shuM3', m3);
		}

		//If change Amount after set DeAllocation => has to Re-DeAllocation with new Amount
		if(detailItem.get('shuLocId') != '' || me.prevWhItems != null){
	...
	onSave:function(){
		...
		var shuM3 = detailItem.get('shuM3');
		var shuQty = detailItem.get('shuQty');
		var isShutOut = (shuMt > 0 || shuM3 > 0 || shuQty > 0);

		if(isShutOut && (detailItem.get('shuLocId') == '' || me.shuWhItems == null)){
			MessageUtil.warning('warning_msg', "warehousecheckexport_alloc_shu_msg");
			return;
		}
...
//	},
	...
	setDetailInitialize: function(masterItem){
		...
		if(recvData.get('cgTpCd') == CodeConstants.MT_CGTP_BBK){
			...
			refs.ctlShutOutQty.setAllowBlank(false);
			refs.ctlShutOutMt.setAllowBlank(true);
			refs.ctlShutOutQty.setReadOnly(false);
			refs.ctlShutOutMt.setReadOnly(true);
			refs.ctlShutOutM3.setReadOnly(true);
		}else{
			...
			refs.ctlShutOutMt.setAllowBlank(false);
			refs.ctlShutOutQty.setReadOnly(false);
			refs.ctlShutOutMt.setReadOnly(false);
			refs.ctlShutOutM3.setReadOnly(false);
		}
...
//		detailItem.set('shftDt', recvData.get('shftDt'));
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		else if(targetControl == 'cltWHCheckExportShuAllocation') {
			var control = me.lookupReference(targetControl);
			control.setValue(returnValue.data.locId);
	...
	saveProcess : function() {
		...
		detailItem.set('shutItems', me.shuWhItems);
		...
		detailItem.set('cgNo', me.prevWhItems[0].cgNo);
	...
	onValidateAmtDeAlloc: function(){
		...
		if(cgTpCd === CodeConstants.MT_CGTP_BBK){
			...
			if(refs.ctlLoadQty.getValue() + shuQty > whBalQty){
			...
			if (shuMt + loadMt > whBalMt || loadM3 + shuM3 > whBalM3) {
				MessageUtil.warning('warning_msg', 'warehousecheckexport_bbk_exceed_msg');
				return false;
			}
