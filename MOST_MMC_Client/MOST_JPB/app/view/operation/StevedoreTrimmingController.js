Ext.define('MOST.view.operation.StevedoreTrimmingController', {
	...
	onInitialize:function(){
		...
		var record =Ext.create('MOST.model.operation.StevedoreTrimming');
	...
	onSelectionChange: function(selectable, selectRecords, eOpts) {
		...
		if(selection){
			...
			if(selection.get('cgTpCd')==='DBK'){
                refs.refBreakBulkContainer.setDisabled(true);
                refs.refLashingStvContainer.setDisabled(false);
				refs.refNosOfGangLashingStv.setDisabled(false);
				refs.refNosOfGangLashingStv.el.animate({
	                opacity: 1
	            });

                refs.refDryBulkContainer.setDisabled(false);
                ...
                refs.refBreakBulkContainer.setDisabled(false);
                refs.refDryBulkContainer.setDisabled(true);
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'ctlVslCallId'){
            if(returnValue){
				...
				me.getViewModel().setData({theSdTrm:null});
			}
...
//			refs.refAddNon.getEditor().setDisabled(true);
	...
	*/	
	onTabletLoad: function(){
		...
		var vslCallid = me.getViewModel().get('globalJpvcNo');
		var pntyDate = Ext.Date.format(MOST.config.Token.getWorkDate(), 'd/m/Y');
		var glbShft = MOST.config.Token.getWorkShift();	
		//Penalty Date
		refs.refWorkDateTextfield.setValue(pntyDate);
	...
	onCheckStvRadioField:function(radioField,newValue, oldValue){
		...
		if((radioField.getValue() == 'stevedore'|| radioField.getValue() == 'shipscreBb') && newValue ){
			...
			refs.refsWithGrearsCbx.setDisabled(!BbEnable); 			
				refs.refFindStvBbTextField.setDisabled(BbEnable);
...
//				refs.refShipsCreBbRdo.enable();		
				refs.refTrimmingTextField.setDisabled(true);	
		}
			...
			refs.refTrimmingTextField.setDisabled(DbEnable);		
...
//			refs.refShipsCreDbRdo.enable();
	...
	onCellClick:function(ctx, td, cellIndex, record, tr, rowIndex, e, eOpts){
		...
		if(selection.get("cgTpCd") == "BBK"){
			me.getViewModel().set('theStevedoreTrimmingBb', selection.getData());			
			...
			me.getViewModel().set('theStevedoreTrimmingDb', selection.getData());			
		...
		me.getViewModel().set('theAdditional', selection.getData());
	},
	...
	onUpdateHHT:function(){
		...
		var validForm = refs.stevedoreTrimmingDetail.validate();
		if(!validForm){
	...
	var item =Ext.create('MOST.model.operation.StevedoreTrimming');
	store.each(function(record, index){
		if(index === sltIndex){
			record.set('crud', 'U');
			record.set('vslCallId', me.getViewModel().get('globalJpvcNo').toUpperCase());
	...
	setTimeWithShiftHHT: function(tab){
		...
		var shift = refs.refShiftCbx.getSelection();
	...
	onAddHHT:function(){
		...
		var vslCallId = me.getViewModel().get('globalJpvcNo').toUpperCase();
		var shiftId = refs.refShiftCbx.getValue();
		...
		masterItem.data.userId = userIdCd;
		
		if(masterItem.data){
			var proxy = masterItem.getProxy();
			proxy.url = MOST.config.Locale.getRestApiDestUrl() + '/v1/vesseldelaypnty/list';		
	...
	onUpdatePenatlyHHT:function(){
		...
		store.each(function(record, index){
			if(index === sltIndex){				
				record.set('vslCallID', me.getViewModel().get('globalJpvcNo').toUpperCase());
				record.set('shftId', refs.refShiftCbx.getValue());
