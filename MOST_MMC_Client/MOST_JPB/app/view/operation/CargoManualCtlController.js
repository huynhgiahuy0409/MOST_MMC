Ext.define('MOST.view.operation.CargoManualCtlController', {
	...
	onLoading : function(){ 
		...
		//OPR-006: Conveyor Mode fro Dry bulk direct loading
		if (!cgTpCdDBList.includes(selection.get('cgTpCd')) && searchParm.get('conveyerCheck')) {
			MessageUtil.warning('warning_msg', 'cargomanualctl_conv_block');
			return;
		}
		...
		if(proceed){
			...
			searchParams.set('conveyerCheck', searchParm.get('conveyerCheck'));
	...
	onHandlingIn : function(){
		...
		if (theSearchParm.get('conveyerCheck') &&  selection.get('delvTpCd') != 'D') {
			MessageUtil.warning('warning_msg', 'cargomanualctl_conv_block', cargoType);
			return;
		}
...
//				}
	...
	onChangeYardTruck: function(ctl, newVal, oldVal, eOpts ){
		...
		if(newVal){
			...
			refs.ctlCargoManualCtlBl.setValue('');
			refs.ctlCargoManualConvType.setValue(false);
	...
	onChangeConveyorChk: function (ctl, newVal, oldVal, eOpts) {
		var me = this;
		var refs = me.getReferences();
		var searchParm = me.getViewModel().get('theSearch');

		if (newVal) {
			refs.ctlCargoManualTruckType.setValue(false);
			searchParm.set('lorryNo', '');
			searchParm.set('truckTypeCheck', false);
		}
	...
	onChangeSearchCondition: function (field){
		var me = this;
		var refs = me.getReferences();
		
		if(field && field.reference == 'refCboHatchNo') {
			return;
		}
	...
	getSearchConditionExport: function(isIndicator){
		...
		if(searchParm.get('truckType') === 'I'){ // YardTruck => ListType is Job (WA)
			...
			me.LIST_TYPE = "JOBWA";//=> LD
				...
				|| searchParm.get('conveyerCheck') //ctlCargoManualConvType
