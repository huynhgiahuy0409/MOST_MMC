Ext.define('MOST.view.operation.VSRCheckListController', {
	...
	onCreate: function(){
		...
		if(refs.ctlVslCallId.getValue() == null || refs.ctlVslCallId.getValue() == ''){
			...
			if (selection) {
				...
				viewModel.set('theVsl.scn', 		selection.data.scn);
	...
	onDetailLoad:function(){
		...
		refs.ctlVerifyText.setText(me.UNVERIFIED_LABEL);
		refs.refBtnVerify.setText(me.VERIFY_LABEL);
	...
	setDetailControl:function(recvData){
			...
			vslStore.load({
				...
				callback: function(records, operation, success) {
					if (success) {
						if(records[0].data){
							var viewModel = me.getViewModel();
							viewModel.set('theVsl.scn', 		records[0].data.scn);
						    viewModel.set('theVsl.vslCallId', 	records[0].data.vslCallId);
						    viewModel.set('theVsl.vslCd', 		records[0].data.vslCd);
							viewModel.set('theVsl.vslNm', 		records[0].data.vslNm);
							viewModel.set('theVsl.voyage', 		records[0].data.voyage);
							viewModel.set('theVsl.arrvSaId', 	records[0].data.arrvSaId);
							viewModel.set('theVsl.eta', 		records[0].data.eta);
							viewModel.set('theVsl.etd', 		records[0].data.etd);
							viewModel.set('theVsl.berthLoc', 	records[0].data.berthLoc);
						}
					}
				}
			});
				...
				callback: function(records, operation, success) {
					if (success) {
						if (records.length > 0) {
							...
							if(records[0].data.searchJPVCItem){
		...
		if(recvData.get('verifyStatus') === 'VERIFIED'){
			refs.ctlVerifyText.setText(me.VERIFIED_LABEL);
			label.getEl().setStyle('background', me.VERIFY_COLOR);
			refs.refBtnVerify.setText(me.UNVERIFY_LABEL);
		}else{
			refs.ctlVerifyText.setText(me.UNVERIFIED_LABEL);
	...
	onRemoveManPower: function() {
		...
		var isVerified = me.checkVerify();

		if(isVerified == true) {
			MessageUtil.alert('Warning', 'vsrchecklist_block_CUD_msg');
			return;
		}
	...
	onVerify : function(){
		var me = this;
		var refs = me.getReferences();
		if(refs.refBtnVerify.getText() == me.VERIFY_LABEL){
			me.onVerifyData();
		}else{
			me.onUnverifyData();
		}
	...
	onVerifyData:function(){
		...
		MessageUtil.question('confirm', 'vsrchecklist_confirm_verify_msg',null, function(button){
			if(button === 'ok'){
				...
				verifyItem.set('verifyDate', me.getCurrentDateFormatted());
				verifyItem.set('verifyBy', MOST.config.Token.getUserId());
	...
	onUnverifyData:function(){
		...
		MessageUtil.question('confirm', 'vsrchecklist_confirm_unverify_msg',null, function(button){
			if(button === 'ok'){
				var verifyVSRCheckList = me.getStore('verifyVSRCheckList');
				var verifyItem = Ext.create('MOST.model.operation.VSRCheckList');
				...
				verifyItem.set('workYmd', refs.refWorkYmd.rawValue);
				verifyItem.set('shftId', refs.refShift.getValue());
				verifyItem.set('verifyStatus', 'UNVERIFIED');
				verifyItem.set('verifyDate', me.getCurrentDateFormatted());
				verifyItem.set('verifyBy', MOST.config.Token.getUserId());
	...
	checkVerify: function () {
		...
		if (recvData) {
			 return recvData.get('verifyStatus') === 'VERIFIED'
		} else {
			return false;
		}
	}
