Ext.define('MOST.view.operation.WHCheckExportForROROController', {
	extend : 'MOST.view.foundation.BaseViewController',

	requires : [],

	alias : 'controller.whcheckexportforroro',

	/**
	 * =========================================================================================================================
	 * CONSTANT START
	 */
	WAREHOUSE_CHECK_EXPORT_PROXY_URL : MOST.config.Locale.getRestApiDestUrl() + '/v1/whcheckexport/whcheckexportforroro',
...
//	},
	...
	setDetailInitialize: function(masterItem){
		...
		if(masterItem.get('unitNo') != null && masterItem.get('unitNo') != ''){
			masterItem.set('loadMt',masterItem.get('whBalMt'));	
			masterItem.set('loadM3',masterItem.get('whBalM3'));	
			masterItem.set('loadQty',masterItem.get('whBalQty'));	
		}
		masterItem.set('pkgNo',recvData.get('pkgNo'))	
		masterItem.set('repkgTypeCd',recvData.get('pkgTpCd'))	
		me.getViewModel().setData({theRRDetail:masterItem});
...
//		refs.ctlLoadM3.setEditable(isDBK);
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'ctlUnitNoSearchField'){
			if(returnValue){
	...
	autoDeAllocation: function () {
		...
		refs = me.getReferences(),
	 	...
	 	balQty = refs.ctlBalQty.getValue();
				...
				if(records && records.length > 0 && records[0].data.locId){
					if (records.length > 1 && loadQty < balQty) {
						...
						for(var i = 0; i < records.length; i++){
							...
							if (
                                invLocItem.get('blSn') ==
                                detailItem.get("shipgNoteNo")
                            ) {
                                if(!locIdArr.includes(invLocItem.get('locId'))) {
                                	res = invLocItem;
									locIdArr.push(invLocItem.get('locId'));
								} 
                            }
						}
						...
						me.prevWhItems = whItems;
						var locId = "";
						if(locIdArr.length == 1) {
							var id = res.data.locId.split('-'); 
							locId = records[0].data.whLocId;
							locId += '(' + id[1] + ',' +  + '1)';
						}  
						...
						for(var i = 0; i < records.length; i++){
							...
							me.whType = invLocItem.data.locTpCd; 
							if (
                                invLocItem.get('blSn') == detailItem.get("shipgNoteNo")
                            ) {
								if(!locIdArr.includes(invLocItem.get('locId'))) {
                                	res = invLocItem;
									locIdArr.push(invLocItem.get('locId'));
...
//		me.onInternalTruckJobValidation();
	...
	onValidateAmtDeAlloc: function(){
		...
		if(refs.ctlLoadQty.getValue() > whBalQty 
				   || refs.ctlLoadMt.getValue() > whBalMt 
				   || refs.ctlLoadM3.getValue() > whBalM3){
	...
	openUnitListPopup:function(field, button, e){
		...
		var detailItem = me.getViewModel().get('theRRDetail');
     	...
     	recvData.set('jobPurpCd', 'WA');
		//NTH240703: Search for RORO items for warehouse check by SN No
		recvData.set('grNo', '');
		recvData.set('searchType', 'WA');
