...
//		lddcTypeCombo.load();
	...
	onSearch: function() {
		var me = this;
     	...
     	var tankerTimeStore = me.getStore('tankerTime')
				...
				if (success) {
					if (records.length > 0) {
						records.forEach(function(record){
							var tankerSource = record.get('tankerSource');
							var value = ['hoseOnDt', 'stDt', 'hoseOffDt', 'endDt', 'cgWgt']
							var bindRefs = ['ctlHoseOn', 'ctlCommenceTime', 'ctlHoseOff', 'ctlCompletionTime', 'ctlCgWgt']
							if(tankerSource === 'SA' || tankerSource === 'OP'){
								bindRefs = bindRefs.map(item => item + tankerSource)
							}
							
							bindRefs.forEach(function(ref, index){
								if(refs[ref]) refs[ref].setValue(record.get(value[index]))
							})
						})
					}
				}
	...
	cmdtChanged: function(combo, value){
		...
		var cmdtCd, cmdt, seq;
		if(sltCmdtCd){
			refs.ctlBlSnNo.setDisabled(false)
			...
			if(blSnCombo.getCount() === 1) refs.ctlBlSnNo.setValue(blSnCombo.data.items[0].get('scd'))

		}else{
			refs.ctlBlSnNo.setDisabled(true) 
	...
	blSnChanged: function(){
		...
		var blSnNo = refs.ctlBlSnNo.getValue()
		var blSnCombo = me.getStore('blSnCombo')
					...
					if (records.length > 0) {
						for(var i = 0; i < records.length; i++){
							if(refs.ctlCommodityCombo.getValue()){
								...
								refs.ctlVORLines.setReadOnly(true);
							}
							...
							refs.ctlVORLines.setValue(records[i].data.eqTpCd);
							// refs.ctlBlSnNo.setValue(records[i].data.blNo);	
	...
	onAddDelaySummary: function() {
		...
		if((refs.refWorkYmd.getValue() != null && refs.refWorkYmd.getValue() != '') && (refs.refShift.getValue() != null && refs.refShift.getValue() != '')){
			...
			record.set("scn", 			refs.refScn.getValue());
			record.set("vslCallId", 	refs.refVslCallId.getValue());
	...
	getSearchCondition : function(){
		var me = this;
     	...
     	searchParm.data.vslCallId = (searchVessel != null && searchVessel.data != null) ? searchVessel.data.vslCallId : '';
		var params = me.createParam(searchParm);
     	
     	if((searchParm.data.vslCallId == null || searchParm.data.vslCallId == '') 
     			&& (searchParm.data.scn == null || searchParm.data.scn == '')){
...
//				me.getViewModel().setData({theSearch:searchParm});
	...
	setDetailInitialize:function(){
		...
		var refs = me.getReferences();
		var detailView = me.getDetailBizView();
		var equipmentComboList = me.getStore('equipmentComboList');
		...
		refs.ctlConsigne.setReadOnly(true);
		refs.ctlLoadDischarge.setReadOnly(true);
		if(!refs.refVorEqTp.getValue()){
			equipmentComboList.clearData()
		}
	},
...
//					refs.refVORTerminalOperator.setVisible(true);
	...
	onAddingCargoProceed: function(){
		...
		record.set('blSnNo', 	refs.ctlBlSnNo.getValue());
		...
		record.set('eqFacNo', 	refs.refVorEqTp.getValue());
