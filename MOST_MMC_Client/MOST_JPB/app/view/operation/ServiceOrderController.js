Ext.define('MOST.view.operation.ServiceOrderController', {
        ...
        if (xtype === 'popup-serviceordersetting'){
			...
			if(returnValue){
				...
				if(!StringUtil.isNullorEmpty(returnValue.item.get('vslCallId'))){
					refs.refServiceOrderVslCallIdField.setValue(returnValue.item.get('vslCallId'));
					me.getViewModel().setData({theVslInfo:returnValue.item});
					me.onSearch();
				}else {
					refs.refServiceOrderVslCallIdField.setValue('');
    ...
    setJpvcFieldValue: function(vslCallId){
        ...
        me.lookupReference('ctlScn').setDisabled(true);
    ...
    setJpvcFieldOnCreate: function(){
        ...
        if(scn !== null && scn !== ""){
        	refs.ctlDetailScn.setValue(scn);
			refs.ctlDetailScn.refs.ctlField.onFocusLeave();
        }
