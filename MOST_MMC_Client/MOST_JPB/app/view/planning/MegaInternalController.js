...
//    	}
	...
	onSave:function(){
		...
		var whConfmBy = MOST.config.Token.getUserId();
		
		// CREATE, UPDATE RECORD
		store.getModifiedRecords().forEach(function(record, index, array){
			record.set('whConfmBy', whConfmBy);
...
//			masterItem.set('insertType', 'forklift');
	...
	afterSetCodePopupData: function (xtype, targetControl, returnValue) {
		...
		if(targetControl === 'ctlVessel'){ // MEGA DETAIL JPVC
			if(returnValue){
				refs.ctlScn.setValue(returnValue.item.get('scn'));
				refs.ctlVessel.setValue(returnValue.code);
			} else {
				refs.ctlScn.setValue(returnValue.item.get('scn'));
