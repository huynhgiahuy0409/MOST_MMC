Ext.define('MOST.view.planning.SpaceMovementRequestController', {
	extend: 'MOST.view.foundation.BaseViewController',

	requires: [
	],

	alias: 'controller.spacemovementrequest',
	...
	DETAIL_GRID_REF_NAME: 'refSpaceMovementRequestDetailGrid',
	DETAIL_STORE_NAME: 'spaceMovementRequestDetail',
	MAX_PERIOD_DAY: 31,	// MAX PERIOD DATE
	alertYN: 'N',
	alertTp: '',
	PARAMETTER_CHECK_SPACE_MOVEMENT_PLAN_FOR_REQ_TYPE_COMBO_STORE: 'spaceMovementRequestForReqTypeCombo',
	PARAMETTER_CHECK_SPACE_MOVEMENT_PLAN_FOR_STATUS_COMBO_STORE: 'spaceMovementRequestForStatusCombo',
	...
	onLoad: function () {
		...
		me.setDateInDays('ctlSpaceMovementRequestFromDt', -me.MAX_PERIOD_DAY);
		me.setDateInDays('ctlSpaceMovementRequestToDt');

		var recvData = me.getView().recvData;

		if (recvData != null) {
			refs.ctlSpaceMovementRequestFromDt.setValue('');
			refs.ctlSpaceMovementRequestToDt.setValue('');
			me.alertYN = 'Y';
	...
	setDetailInitialize: function (masterItem) {
		...
		if (!StringUtil.isNullorEmpty(recvData.get('reqNo'))) {
			refs.ctlReqNo.setValue(recvData.get('reqNo'));
	...
	onAdd: function () {
		...
		if (vesselDetail) {
			...
			item.set('vslCallId', refs.ctlSpaceMovementRequestVesselCallId.getValue());
			item.set('scn', refs.ctlScn.getValue());
		}

		item.set('reqNo', '');

		me.getView().detailViewAlias = 'app-spacemovementrequestdetail';
	...
	onDetailDblClick: function () {
		var me = this;
		var grid = me.lookupReference('refSpaceMovementRequestDetailGrid');
		var selection = grid.getSelection() == null ? null : grid.getSelection()[0];
	...
	onSubmit: function () {
		...
		var removedRecords = detailStore.getRemovedRecords();
		var isCreated = !StringUtil.isNullorEmpty(theDetail.get('reqNo'));
		var detailView = me.getDetailBizView();

		if(detailStore.getCount() <= 0) {
			MessageUtil.warning("warning", "Please add any record.");
			return;
		}

		if (isCreated && modifiedRecords.length <= 0 && removedRecords.length <= 0) {
			MessageUtil.warning("warning", "Please modified any record.");
			return;
		}

		var updateItems = [...modifiedRecords, ...removedRecords].map(item => item.data);
	...
	onRefresh: function () {
		...
		var controlArray = [
			...
			'ctlAreaM2', 'ctlAreaMT', 'ctlEstArrvDt',
			'ctlEstDelvDt', 'ctlPeriod', 'ctlRemark', 'ctlDoNo'
		];
	...
	onSelectedMfDocId: function (combo, record, eOpts) {
		...
		if (combo.reference === 'ctlMasterBL') {
			...
			me.lookupReference('ctlDoNo').reset();
			blCombo.removeAll();
			me.getSnCombo();
		}
	...
	getSearchCondition: function () {
		...
		if (me.alertYN == 'N') {
			var dateCondition = me.checkFromToDate('ctlSpaceMovementRequestFromDt', 'ctlSpaceMovementRequestToDt');
	...
	afterSetCodePopupData: function (xtype, targetControl, returnValue) {
			...
			if (returnValue) {
				if (targetControl === 'ctlScn') {
					...
					if (!StringUtil.isNullorEmpty(returnValue.item.get('vslCallId'))) {
						refs.ctlSpaceMovementRequestVesselCallId.setValue(returnValue.item.get('vslCallId'));
					} else {
						refs.ctlSpaceMovementRequestVesselCallId.setValue('');
					}
				} else {
		...
		} else if (targetControl === 'ctlSpaceMovementRequestVesselCallId') {
			if (returnValue) {
	...
	getLocNmMap: function (whAllocData) {
		var me = this;
		var reqLocId = []; // planList
		var locId = [];		// reqLocId

		Ext.Array.each(whAllocData, function (cell) {
			reqLocId.push(me.generateAccmWhNms(cell));
			locId = locId.concat(cell.get('locIdArr'));
		});

		return { reqLocId: reqLocId, locId: locId };
	},
	...
	setWhDataAtGridSelection: function (locId, area) {
		var me = this;
		var theDetail = me.getViewModel().get('theDetail');
	...
	fncDelete: function () {
		...
		if (selection <= 0) {
			MessageUtil.warning("spacemovementrequestdetail", "spaceMovementPlan_SelectBeforeRemove");
			return;
		}
		...
		if (!validObj.valid) {
			MessageUtil.warning("spacemovementrequestdetail", "There isn't include the planned data.");
			return;
		}
		...
		updateParm.erase({
			callback: function (records, operation, success) {
				if (success) {
					MessageUtil.confirmation("spacemovementrequestdetail", "Success to remove.");
					me.onRetrieve();
				}
			}
		});
	...
	fncClear: function () {
		...
		['ctlReqNo', 'ctlStatus', 'ctlRequestTp', 'ctlVesselName'].forEach(ref => refs[ref].setValue());
		refs['ctlSpaceMovementRequestDetailMvTp'].setVisible(false);
	...
	onAddForDetail: function () {
		var me = this;
		var refs = me.getReferences();
		var theDetail = me.getViewModel().get('theDetail');
		var detailStore = me.getStore('spaceMovementRequestDetail');
		var requestItem = Ext.create('MOST.model.planning.SpaceMovementPlan');
		var vesselDetail = me.getViewModel().get('vesselDetail');
		var id = requestItem.get('id'); 

		requestItem.set(theDetail.data);
		requestItem.set('id', id);
		requestItem.set('callSeq', vesselDetail ? vesselDetail.get('callSeq') : theDetail.get('callSeq'));
		requestItem.set('callYear', vesselDetail ? vesselDetail.get('callYear') : theDetail.get('callYear'));
		requestItem.set('reqTpNm', refs.ctlRequestTp.getRawValue());
		requestItem.set('mfDocId', theDetail.get('masterBL') || theDetail.get('bookingNo'));
		requestItem.set('blSn', theDetail.get('blNo') || theDetail.get('shipgNoteNo'));
		requestItem.set('grNo', theDetail.get('grNo'));
		requestItem.set('workingStatus', WorkingStatus.INSERT);

		var validateFields = ['vslCallId', 'mfDocId', 'blSn', 'reqTpCd', 'eta', 'svcDt'];
		var missingInfo = me.validateMissingRequestInfo(requestItem, validateFields);

		if (!missingInfo.isValid) {
			MessageUtil.warning(
				'Missing Request Information',
				'Please fill in the following request information:<br>'
				+ missingInfo.missingFields.map(field => '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;- ' + field).join('<br>')
			);
			return;
		}

		me.isDuplicatedRequest(requestItem).then(function (response) {
			if (response.isDuplicated) {
				if (response.source == 'store') {
					MessageUtil.warning('Duplicated Request', 'This document is already in the list.');
					return;
				}
				if (response.source == 'database') {
					MessageUtil.warning(
						'Duplicated Request',
						'This document has been submitted on the following request: '
						+ '<b>' + response.duplicatedItem.get('reqNo') + '</b>'
					);
					return;
				}
			} else {
				detailStore.add(requestItem);
			}
		})
	}, 

	validateMissingRequestInfo: function (obj, fields) {
		var missingFields = [];
		var messages = {
			vslCallId: "Vessel Call ID",
			mfDocId: "Master BL/Booking No",
			blSn: "BL No/Shiping Note No",
			reqTpCd: "Request type",
			eta: "Arrival Date",
			svcDt: "Delivery Date",
		}

		fields.forEach(field => {
			if (StringUtil.isNullorEmpty(obj.get(field))) {
				missingFields.push(messages[field]);
			}
		});
	...
	isDuplicatedRequest: function (requestItem) {
		var me = this;
		var detailStore = me.getStore(me.DETAIL_STORE_NAME);
		var duplicatedRequestStore = me.getStore('duplicatedRequest');

		return new Promise(function (resolve, reject) {
			detailStore.getData().items.forEach(item => {
				if (
					item.get('vslCallId') == requestItem.get('vslCallId') 
					&& item.get('mfDocId') == requestItem.get('mfDocId') 
					&& item.get('blSn') == requestItem.get('blSn') 
					&& item.get('reqTpCd') == requestItem.get('reqTpCd')
				) {
					return resolve({
						isDuplicated: true,
						duplicatedItem: item,
			...
			duplicatedRequestStore.load({
				params: {
					vslCd: requestItem.get('vslCd'),
					vslCallId: requestItem.get('vslCallId'),
					mfDocId: requestItem.get('mfDocId'),
					blNo: requestItem.get('blNo'),
					shipgNoteNo: requestItem.get('shipgNoteNo'),
					reqTpCd: requestItem.get('reqTpCd')
				},
				callback: function (records, operation, success) {
					if (success) {
						if(records.length > 0) {
	...
	onRemoveForDetail: function () {
		var me = this;
		var refs = me.getReferences();
		var grid = refs.refSpaceMovementRequestDetailGrid;
		var detailStore = grid.getStore();
		...
		if(selection.length > 0) {
			...
			MessageUtil.warning('warning_msg', 'message_selected_to_remove');
		}
	},
	...
	onUpdateForDetail: function () {
		...
		if(!selection){
			MessageUtil.warning('warning_msg', 'message_selected_to_update');
			return;
		}
		...
		if(modified) {
			...
			selection.set('mfDocId', theDetail.get('masterBL') || theDetail.get('bookingNo'));
			selection.set('blSn', theDetail.get('blNo') || theDetail.get('shipgNoteNo'));
