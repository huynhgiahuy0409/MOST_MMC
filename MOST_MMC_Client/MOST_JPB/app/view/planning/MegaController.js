Ext.define('MOST.view.planning.MegaController', {
	...
	EQUIPMENT_CREATE_UPDATE_FIELD_CONFIG : {
		...
		'Stevedore': {
			'nofGang'		: 'ctlDetailMegaStedoreNosofGang',
			'locId'			: 'ctlWorkingAreaStevedore',
			'workStDt'		: 'refStartTimeStevedore',
			'workEndDt'		: 'refEndTimeStevedore',
			'nofStvdSprr'	: 'ctlDetailMegaStevedoreNofStvdSprr',
			'nofWchmn'		: 'ctlDetailMegaStevedoreNofWchmn',
			'nofStvdGwker'	: 'ctlDetailMegaStevedoreNoStvdGwker'
		},
		'Trimming': {
			'nofGang'		: 'ctlDetailMegaTrimmingNosofGang',
			'locId'			: 'ctlTrimmingWorkingArea',
			'workStDt'		: 'refStartTimeTrimming',
			'workEndDt'		: 'refEndTimeTrimming',
			'nofTrmgSprr'	: 'ctlDetailMegaTrimmingNofTrmgSprr',
			'nofSglmn'		: 'ctlDetailMegaTrimmingNofSglmn',
			'nofDekmn'		: 'ctlDetailMegaTrimmingNofDekmn',
			'nofHopmn'		: 'ctlDetailMegaTrimmingNofHopmn',
		}
	},
...
//				}
	...
	getSearchCondition:function(){
		...
		if(searchParm && searchParm.data) {
			...
			var scn = StringUtil.toUpperCase(searchParm.get('scn') );
     	...
     	if(me.alertYN == 'N'){
     		if(jpvcNo == "" && StringUtil.isNullorEmpty(scn)){
	...
	getDetailSearchCondition:function(){
		var me = this;
     	...
     	var scn = refs.ctlDetailScn.getValue();
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'ctlDetailCommodity'){
	...
	setControlForPurpose:function(purpose){
		...
		if(purpose === 'MP'){
			...
			refs.blankContainer.setHidden(false);
			...
			refs.blankContainer.setHidden(true);
		...
		if(vslItem && !StringUtil.isNullorEmpty(vslItem.get('vslCallId')) 
	...
	setDetailTabControl:function(masterItem, recvData){
		...
		var stevedoreCompany = me.getStore('megaDetailTabStevedoreCompany');
		var trimmingCompany = me.getStore('megaDetailTabTrimmingCompany');
		...
		var megadetail = me.getStore('megaDetailList');
		var megaDetailStevedore = me.getStore('megaDetailStevedore');
		var megaDetailTrimming = me.getStore('megaDetailTrimming');
		var mainItem = null;
		...
		if(recvData != null){
			...
			stevedoreCompany.removeAll();
			trimmingCompany.removeAll();

			//s-PL-016 MEGA detail – Redesign in Stevedore and Trimming tabs
			var stevedoreItem = new Ext.create('MOST.model.planning.Mega');
			var stevedoreCompany = new Ext.create('MOST.model.planning.Mega');
			var trimmingItem = new Ext.create('MOST.model.planning.Mega');
			var trimmingCompany = new Ext.create('MOST.model.planning.Mega');

			me.getViewModel().setData({ theStevedore: stevedoreItem });
			me.getViewModel().setData({ theStevedoreCompany: stevedoreCompany });
			me.getViewModel().setData({ theTrimming: trimmingItem });
			me.getViewModel().setData({ theTrimmingCompany: trimmingCompany });
			//e-PL-016 MEGA detail – Redesign in Stevedore and Trimming tabs
	...
	visibleDetailEquipmentGrid:function(recvData){
		...
		// Stevedore
		refs.ctlCompanyStevedore.setVisible(!visibleYN);

		// Trimming
		refs.ctlCompanyTrimming.setVisible(!visibleYN);
	...
	serverSend: function(statusCode){
		...
		masterItem.set('scn', vslItem.get('scn'));
		...
		var stevedoreItem = me.getModifyDataForStevedore();
		var trimmingItem = me.getModifyDataForTrimming();
		...
		masterItem.set('stevedoreItems', stevedoreItem.updateItems);
		masterItem.set('operInfoItemsForStevedore', stevedoreItem.compayItems);
		masterItem.set('trimmingItems', trimmingItem.updateItems);
		masterItem.set('operInfoItemsForTrimming', trimmingItem.compayItems);
	...
	validateCompany: function () {
		var me = this;
		var refs = me.getReferences();
		var isMissingStevedoreCompany = StringUtil.isNullorEmpty(refs.cboContractorStevedore.getValue());
		// var isMissingWorkingAreaVesselOperation = StringUtil.isNullorEmpty(refs.ctlWorkingAreaVesselOperation.getValue());
		var isMissingStevedoreWorkingArea = StringUtil.isNullorEmpty(refs.ctlWorkingAreaStevedore.getValue());
		var isEmptyDetailMegaStedoreNosofGang = !refs.ctlDetailMegaStedoreNosofGang.getValue() || refs.ctlDetailMegaStedoreNosofGang.getValue() === 0;
		try {
			if ((!isMissingStevedoreCompany && isMissingStevedoreWorkingArea)) {
				throw 'mega_workingarea_null_msg';
			}

			// if (me.CURRENT_VIEW_TYPE === me.VIEW_TYPE_CREATE || me.CURRENT_VIEW_TYPE === me.VIEW_TYPE_COPY) {
			// 	if (refs.ctlDetailPurpose.getValue() == 'NP001' && isMissingWorkingAreaVesselOperation) {
			// 		throw 'mega_workingarea_null_msg';
			// 	}
			// } else {
			// 	if (refs.ctlDetailPurpose.getValue() == 'Vessel Operation' && isMissingWorkingAreaVesselOperation) {
			// 		throw 'mega_workingarea_null_msg';
			// 	}
			// }

			if ((!isMissingStevedoreCompany && isEmptyDetailMegaStedoreNosofGang)) {
	...
	onApprovalMega : function(){
		...
		if(recvData.get('statCd') === CodeConstants.MT_MEGASTAT_AP && StringUtil.isNullorEmpty(refs.ctlDetailRemark.getValue())){
			...
			var isManpowerCompanyTotalValid = me.isManpowerCompanyTotalValid();
			if (isManpowerCompanyTotalValid) {
	...
	validationSubmitAmendCancel : function(execFunc){
		...
		var intGridRowStevedore = refs.refMegaDetailStevedoreGrid.getStore().getCount();
		var intGridRowTrimming = refs.refMegaDetailTrimmingGrid.getStore().getCount();
		...
		if(
			// refs.cboContractorStevedore.getValue() == '' &&
			// refs.ctlDetailMegaStedoreNosofGang.getValue() == '' &&
			// refs.ctlWorkingAreaStevedore.getValue() == '' &&
			...
			intGridRowPortCrn <=0 &&
			intGridRowStevedore <= 0 &&
			intGridRowTrimming <= 0
		) {
	...
	cancelMega : function(me){
		...
		if(recvData.get('statCd') === CodeConstants.MT_MEGASTAT_AP){
			...
			if(me.lookupReference('cboContractorStevedore').getValue() != '' || me.lookupReference('ctlDetailMegaStedoreLashingCompany').getValue() != '') {
	...
	initDefaultReqTime: function(bizName){
		...
		me.setDefaultRequisitionTime('Stevedore');
		me.setDefaultRequisitionTime('Trimming');
	},

	setDefaultRequisitionTime: function(bizName){
		var me = this;
		var refs = me.getReferences();
		var refStartTime = refs[Ext.String.format('refStartTime{0}', bizName)];		
		var refEndTime = refs[Ext.String.format('refEndTime{0}', bizName)];
	...
	updateEquipmentCompanyForm: function(bizName) {		
		...
		var supplyQty = me.lookupReference(Ext.String.format('ctlSypplyQty{0}', bizName));
		var remark = me.lookupReference(Ext.String.format('ctlRmk{0}', bizName));

		numEquip.setValue(selection.get('nofOpe'));
		supplyQty.setValue(selection.get('supplyQty'));
		remark.setValue(selection.get('rmk'));	
	},
	...
	onMasterSelectionChangeForEquipment: function(bizName, record) {
		...
		for (var i = 0; i < shiftComboStore.data.length; i++) {
			var shiftData = shiftComboStore.data.items[i].data;
			if (shiftData.shftIdx == '1') {
				shiftId1 = shiftData.shftId;
			}
			if (shiftData.shftIdx == '2') {
				shiftId2 = shiftData.shftId;
			}
			if (shiftData.shftIdx == '3') {
				shiftId3 = shiftData.shftId;
			}
		}
		...
		if (shiftId1) {
			...
			var startShift1 = me.getShiftTime(shiftData1.get('fmHhMm'));
			var endShift1 = me.getShiftTime(shiftData1.get('toHhMm'));
		}
		if (shiftId2) {
			var shiftData2 = shiftListStore.findRecord('shftId',shiftId2 )
			var startShift2 = me.getShiftTime(shiftData2.get('fmHhMm'));
			var endShift2 = me.getShiftTime(shiftData2.get('toHhMm'));
		}
		if (shiftId3) {
			var shiftData3 = shiftListStore.findRecord('shftId',shiftId3 )
			var startShift3 = me.getShiftTime(shiftData3.get('fmHhMm'));
			var endShift3 = me.getShiftTime(shiftData3.get('toHhMm'));
		}
		...
		if(selection.get('operInfoItems')){
			var operInfoItems = selection.get('operInfoItems');
			operInfoItems.forEach(item => {
				item.workStDt = selection.get('workStDt');
				item.workEndDt = selection.get('workEndDt');
			})
	...
	onEditCompanyForEquipment: function(record, bizName){
		if(record.data.workingStatus == null || record.data.workingStatus === WorkingStatus.UPDATE){
	...
	onAddForEquipmentCompany: function(bizName, selectOpeDivCd) {
		...
		var cboContractor = me.lookupReference(Ext.String.format('cboContractor{0}', bizName));
		var numEquip = me.lookupReference(Ext.String.format('ctlNoOf{0}', bizName));
		var refSupplyQty = me.lookupReference(Ext.String.format('ctlSypplyQty{0}', bizName));
		var refRemark = me.lookupReference(Ext.String.format('ctlRmk{0}', bizName));
		var ctlContractor = me.lookupReference(Ext.String.format('ctlContractor{0}', bizName));
		...
		//s-PL-016 MEGA detail 
		if(bizName === 'Stevedore') {
			var theStevedoreCompany = me.getViewModel().get('theStevedoreCompany');
			record.set('nofGang', theStevedoreCompany.get('nofGang'));
			record.set('nofStvdSprr', theStevedoreCompany.get('nofStvdSprr'));
			record.set('nofWchmn', theStevedoreCompany.get('nofWchmn'));
			record.set('nofStvdGwker', theStevedoreCompany.get('nofStvdGwker'));
		} else if(bizName === 'Trimming') {
			var theTrimmingCompany = me.getViewModel().get('theTrimmingCompany');
			record.set('nofGang', theTrimmingCompany.get('nofGang'));
			record.set('nofTrmgSprr', theTrimmingCompany.get('nofTrmgSprr'));
			record.set('nofSglmn', theTrimmingCompany.get('nofSglmn'));
			record.set('nofDekmn', theTrimmingCompany.get('nofDekmn'));
			record.set('nofHopmn', theTrimmingCompany.get('nofHopmn'));
		} else {
			if(numEquip && !StringUtil.isNullorEmpty('opeCompCd', numEquip.getValue().toString())){
				...
				record.set('supplyQty', StringUtil.isNullorEmpty(refSupplyQty) ? null : refSupplyQty.getValue());
				record.set('rmk', StringUtil.isNullorEmpty(refRemark) ? null : refRemark.getValue());
	...
	onGridUpdateForEquipmentCompany: function(bizName, selectOpeDivCd) {
		...
		if(!StringUtil.isNullorEmpty(supplyQty.getValue())) {
			
		}
		if(StringUtil.isNullorEmpty(numEquip.getValue().toString()) || StringUtil.isNullorEmpty(supplyQty.getValue())){
			MessageUtil.warning('warning_msg', 'mandatoryForm_msg');
			return;
		} else {
			companySelection.set('nofOpe', numEquip.getValue());
			companySelection.set('supplyQty', supplyQty.getValue())
			companySelection.set('rmk', remark.getValue())
		}
	...
	onAddForEquipment: function(bizName, eqDivCd) {	
		...
		var isOverlapped = me.isOverlappedTimeForEquipment(bizName);
	...
	isOverlappedTimeForEquipment: function (bizName) {
		var me = this;
		var refs = me.getReferences();
		var store = me.getStore(Ext.String.format('megaDetail{0}', bizName));
		var eqType;
		...
		var startTime = new Date(refs[Ext.String.format('refStartTime{0}', bizName)].getValue());
		var endTime = new Date(refs[Ext.String.format('refEndTime{0}', bizName)].getValue());

		switch (bizName) {
			case "PortCrane" || "Mechanical":
				eqType = refs[Ext.String.format('cboEqType{0}', bizName)];
				break;
			case "Forklift" || "Trailer":
				eqType = refs[Ext.String.format('cboCapacity{0}', bizName)]
				break;
			case "Gears":
				eqType = CodeConstants.MT_EQCD_GR;
				break;
			case "Stevedore":
				eqType = CodeConstants.MT_EQTP2_SD;
				break;
			case "Trimming":
				eqType = CodeConstants.MT_EQTP2_TM;
				break;
			default:
				eqType = null;
				break;
	...
	onValidateShiftExist: function (store, idx, bizName, startTimeOnTab, endTimeOnTab, getShiftSelect){
		...
		if(store.data.length > 0){			
			for(var i = 0; i < store.data.length; i++){
				...
				var capaCdItem = item.capaCd;
				var startTimeInStore = item.workStDt;
				var endTimeInStore = item.workEndDt;

				// if(StringUtil.isNullorEmpty(startTimeInStore) || StringUtil.isNullorEmpty(endTimeInStore)) {
				// 	return false
				// }
				...
				if(capaCdItem == capaCdToValidate){
					if(getShiftSelect != '3'){		
						if (startDayOnTab == startDayInStore && endDayOnTab == endDayInStore) {
							var isStoreTimeWithinTabTime = startTimeInStore >= startTimeOnTab && endTimeInStore <= endTimeOnTab;
							var isTabEndTimeWithinStoreTime = endTimeOnTab > startTimeInStore && endTimeOnTab <= endTimeInStore;
							var isStoreEndTimeWithinTabTime = endTimeInStore > startTimeOnTab && endTimeInStore <= endTimeOnTab;

							if (isStoreTimeWithinTabTime || isTabEndTimeWithinStoreTime || isStoreEndTimeWithinTabTime) {
	...
	getStartTimeInCurrentTab: function(bizName){
		var me = this;
		var refs = me.getReferences();
		var refStartTime = 'refStartTime' + bizName;
		
		var startTime = refs[refStartTime] ? Ext.Date.format(refs[refStartTime].getValue(), 'd/m/Y H:i') : '';
	...
	getEndTimeInCurrentTab: function(bizName){
		var me = this;
		var refs = me.getReferences();
		var refEndTime = 'refEndTime' + bizName;
		var endtTime = refs[refEndTime] ? Ext.Date.format(refs[refEndTime].getValue(), 'd/m/Y H:i') : '';
	...
	onContractorSelectedChangeForklift : function(radio, newValue, oldValue){
		...
		if	(radio.getValue().radioFlBtn == 1 && newValue){
			refs.cboContractorForklift.setDisabled(true);	
			refs.cboContractorForklift.setValue('');
	...
	//s-PL-016 MEGA detail – Redesign in Stevedore and Trimming tabs 
	//TODO onValidateAddForStevedore
	// onValidateAddForStevedore: function (record) {
	// 	var me = this;
	// 	return true;
	// },
	//e-PL-016 MEGA detail – Redesign in Stevedore and Trimming tabs
	...
	onMasterSelectionChangeForStevedore: function(selectable, selectRecords, eOpts) {
		var me = this;
		me.onMasterSelectionChangeForEquipment('Stevedore');
	},

	onMasterSelectionChangeForTrimming: function(selectable, selectRecords, eOpts) {
		var me = this;
		me.onMasterSelectionChangeForEquipment('Trimming');
	},
	 ...
	 */
		...
		/**
	 * =========================================================================================================================
	 * Stevedore, Trimming TAB START
	 */	
	// Modify Gears Grid Data
	onAddForStevedore: function() {
		var me = this;
		var theStevedore = me.getViewModel().get('theStevedore');
		var megaDetailStevedore = me.getStore('megaDetailStevedore');

		me.onAddForManpower(theStevedore, megaDetailStevedore, CodeConstants.MT_EQTP2_SD);
	},

	onAddForTrimming: function() {
		var me = this;
		var theTrimming = me.getViewModel().get('theTrimming');
		var megaDetailTrimming = me.getStore('megaDetailTrimming');

		me.onAddForManpower(theTrimming, megaDetailTrimming, CodeConstants.MT_EQTP2_TM);
	},

	onAddForManpower: function(manpowerItem, mainpowerStore, manpowerType) {
		var me = this;
		var refs = me.getReferences();

		var workStDt = Ext.Date.format(refs.refStartTimeStevedore.getValue(), MOST.config.Locale.getDefaultDateFormatWithNoSeconds());
		var workEndDt = Ext.Date.format(refs.refEndTimeStevedore.getValue(), MOST.config.Locale.getDefaultDateFormatWithNoSeconds());

		manpowerItem.set('workStDt', workStDt);
		manpowerItem.set('workEndDt', workEndDt);
		manpowerItem.set('workingStatus', WorkingStatus.INSERT);
		manpowerItem.set('divCd', manpowerType);

		mainpowerStore.add(manpowerItem);

		me.refreshManpowerForm(manpowerType);

	},

	refreshManpowerForm: function (manpowerType) {
		var me = this;
		var theEmptyManpowerItem = Ext.create('MOST.model.planning.Mega');

		if (manpowerType === CodeConstants.MT_EQTP2_SD) {
			me.getViewModel().setData({ theStevedore: theEmptyManpowerItem });
		} else if (manpowerType === CodeConstants.MT_EQTP2_TM) {
			me.getViewModel().setData({ theTrimming: theEmptyManpowerItem });
		}
	},

	onGridUpdateForStevedore: function () {
		var me = this;
		me.onGridUpdateForEquipment('Stevedore');
	},

	onGridUpdateForTrimming: function () {
		var me = this;
		me.onGridUpdateForEquipment('Trimming');
	},

	onGridRemoveForStevedore: function () {
		var me = this;
		me.onGridRemoveForManpower('Stevedore');
	},

	onGridRemoveForTrimming: function () {
		var me = this;
		me.onGridRemoveForManpower('Trimming');
	},

	onGridRemoveForManpower: function (bizName) {
		var me = this;
		var refs = me.getReferences();
		var manpowerGrid = refs[Ext.String.format('refMegaDetail{0}Grid', bizName)]

		var store = manpowerGrid.getStore();
		var selections = manpowerGrid.getSelection() == null ? null : manpowerGrid.getSelection();
	...
	onAddForStevedoreCompany: function() {
		var me = this;
		me.onAddForManpowerCompany('Stevedore');
	},

	onAddForTrimmingCompany: function() {
		var me = this;
		me.onAddForManpowerCompany('Trimming');
	},

	onAddForManpowerCompany: function(bizName) {
		var me = this;
		var refs = me.getReferences();
		var manpowerGrid = refs[Ext.String.format('refMegaDetail{0}Grid', bizName)];
		var manpowerSelected = manpowerGrid.getSelection();

		if (manpowerSelected == null || manpowerSelected.length === 0) {
			MessageUtil.warning('warning_msg', 'selectadddata_msg');
			return;
		} else {
			var isExistNofGang = me.isExistManpowerGang(bizName);

			if(isExistNofGang) {
				me.onAddForEquipmentCompany(bizName);
			}
		}
	},

	onGridUpdateForStevedoreCompany: function () {
		var me = this;
		var keys = ['opeCompCd', 'nofGang', 'nofStvdSprr', 'nofWchmn', 'nofStvdGwker'];

		me.onGridUpdateForManpowerCompany('Stevedore', keys);
	},

	onGridUpdateForTrimmingCompany: function () {
		var me = this;
		var keys = ['opeCompCd', 'nofGang', 'nofTrmgSprr', 'nofSglmn', 'nofDekmn', 'nofHopmn'];

		me.onGridUpdateForManpowerCompany('Trimming', keys);
	},

	onGridUpdateForManpowerCompany: function (bizName, keys) {
		var me = this;
		var refs = me.getReferences();
		var grid = refs[Ext.String.format('refMegaDetail{0}CompanyGrid', bizName)];
		var selected = grid.getSelection()[0];
		var theManpowerCompany = me.getViewModel().get(Ext.String.format('the{0}Company', bizName));
		var manpower = refs[Ext.String.format('refMegaDetail{0}Grid', bizName)].getSelection()[0];

		if (!selected) {
			MessageUtil.warning('warning_msg', 'Please select a row of data.');
			return;
		}
		...
		if (isExistNofGang) {
			me.copyCompanyProperties(theManpowerCompany, selected, keys);
		}

		me.onEditCompanyForEquipment(manpower, bizName);
	},

	isExistManpowerGang: function(bizName) {
		var me = this;
		var refs = me.getReferences();
		var theManpowerCompany = me.getViewModel().get(Ext.String.format('the{0}Company', bizName))
		var manpowerGrid = refs[Ext.String.format('refMegaDetail{0}Grid', bizName)];
		var nofGang = theManpowerCompany.get('nofGang');
		var isExistNofGang = false;
		
		isExistNofGang = manpowerGrid.getStore().getData().items.some(item => item.get('nofGang') === nofGang);
		
		if(!isExistNofGang) {
			MessageUtil.warning('warning_msg', 'The gang does not exist. Please check again.');
		}

		return isExistNofGang;
	},

	onGridRemoveForStevedoreCompany: function() {
		var me = this;		
		me.onGridRemoveForEquipmentCompany('Stevedore');
	},

	onGridRemoveForTrimmingCompany: function() {
		var me = this;		
		me.onGridRemoveForEquipmentCompany('Trimming');
	},

	// Modify Stevedore Grid Data
	getModifyDataForStevedore: function () {
		var me = this;
		return me.getModifyDataForEquipment('Stevedore');
	},

	getModifyDataForTrimming: function () {
		var me = this;
		return me.getModifyDataForEquipment('Trimming');
	},

	isManpowerCompanyTotalValid: function() {
		var me = this;
		return me.isStevedoreCompanyTotalValid() && me.isTrimmingCompanyTotalValid();
	},
	
	isStevedoreCompanyTotalValid: function () {
		var me = this;
		var stevedoreStore = me.getStore('megaDetailStevedore');
		var stevedoreData = stevedoreStore.getData().items.filter(stevedore => stevedore.get('workingStatus') !== 'D');
		var isValid = true;

		for (let i = 0; i < stevedoreData.length; i++) {
			var stevedore = stevedoreData[i];
			var stevedoreCompanyData = stevedore.get('operInfoItems');

			var	nofStvdSprr = 0;
			var nofWchmn = 0;
			var nofStvdGwker = 0;

			stevedoreCompanyData.forEach(company => {
				nofStvdSprr += company.nofStvdSprr|| (company.data ? company.get('nofStvdSprr') : 0);
				nofWchmn += company.nofWchmn || (company.data ? company.get('nofWchmn') : 0);
				nofStvdGwker += company.nofStvdGwker || (company.data ? company.get('nofStvdGwker') : 0);
			})

			if (
				stevedore.get('nofStvdSprr') !== nofStvdSprr ||
				stevedore.get('nofWchmn') !== nofWchmn ||
				stevedore.get('nofStvdGwker') !== nofStvdGwker
			) {
				isValid = false;
				break;
			}  
		...
		if (!isValid) {
			MessageUtil.warning('warning_msg', Ext.String.format('The quantity of Stevedore with Gang {0} is invalid. Please check again.', stevedore.get('nofGang')));
		}

		return isValid;
	},

	isTrimmingCompanyTotalValid: function () {
		var me = this;
		var trimmingStore = me.getStore('megaDetailTrimming');
		var trimmingData = trimmingStore.getData().items.filter(trimming => trimming.get('workingStatus') !== 'D');
		var isValid = true;

		for (let i = 0; i < trimmingData.length; i++) {
			var trimming = trimmingData[i];
			var trimmingCompanyData = trimming.get('operInfoItems');
			...
			trimmingCompanyData.forEach(company => {
				nofTrmgSprr += company.nofTrmgSprr|| (company.data ? company.get('nofTrmgSprr') : 0);
				nofSglmn += company.nofSglmn || (company.data ? company.get('nofSglmn') : 0);
				nofDekmn += company.nofDekmn || (company.data ? company.get('nofDekmn') : 0);
				nofHopmn += company.nofHopmn || (company.data ? company.get('nofHopmn') : 0);
			})

			if (
				trimming.get('nofTrmgSprr') !== nofTrmgSprr ||
				trimming.get('nofSglmn') !== nofSglmn ||
				trimming.get('nofDekmn') !== nofDekmn ||
				trimming.get('nofHopmn') !== nofHopmn
			) {
				isValid = false;
				break;
			}  
		...
		if (!isValid) {
			MessageUtil.warning('warning_msg', Ext.String.format('The quantity of Trimming with Gang {0} is invalid. Please check again.', trimming.get('nofGang')));
		}

		return isValid;
	},

	onCompanySelectionChangeForStevedore: function(grid) {
		var me = this;
		var keys = ['opeCompCd', 'nofGang', 'nofStvdSprr', 'nofWchmn', 'nofStvdGwker'];

		me.onCompanySelectionChangeForManpower(grid, 'Stevedore', keys);

	},

	onCompanySelectionChangeForTrimming: function(grid) {
		var me = this;
		var keys = ['opeCompCd', 'nofGang', 'nofTrmgSprr', 'nofSglmn', 'nofDekmn', 'nofHopmn'];

		me.onCompanySelectionChangeForManpower(grid, 'Trimming', keys);

	},

	onCompanySelectionChangeForManpower: function(grid, bizName, keys) {
		var me = this;
		var selected = grid.getSelection()[0];
		var theManpowerCompany = me.getViewModel().get(Ext.String.format('the{0}Company', bizName));

		me.copyCompanyProperties(selected, theManpowerCompany, keys);

	},

	copyCompanyProperties: function (source, target, keys) {
		if (!source) {
			return;
		}
		keys.forEach(key => {
		...
		target.set('workingStatus', WorkingStatus.UPDATE);
	}
