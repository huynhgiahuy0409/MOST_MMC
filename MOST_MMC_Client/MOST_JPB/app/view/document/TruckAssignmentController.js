Ext.define('MOST.view.document.TruckAssignmentController', {
    ...
    onLoad : function() {
		...
		if(MOST.config.Token.getUserType() === CONSTANTS.USER_TYPE_INTERNAL){
			...
			refs.rdTruckMode.setValue({truckMode: 'I'});
		}
		else {
			refs.rdTruckMode.setValue({truckMode: 'I'});
			refs.ctlContractor.setValue(MOST.config.Token.getPtnrCode());
			refs.ctlContractor.setDisabled(true);
		}
	...
	searchGeneral:function(){
		...
		if(me.getViewModel().get('theVslInfo') != null){
			if((me.getViewModel().get('theVslInfo').id).indexOf('DeliveryOrder') == -1 && (me.getViewModel().get('theVslInfo').id).indexOf('GoodsReceipt') == -1){
	...
	onAdd:function(){
		...
		if(record.get('truckMode') == 'E') {
			...
			if(record.get('customsReleasedYn') == 'N'){
				MessageUtil.warning("LorryAssignment", "LAWithoutCRMsg");
			}
		}
		...
		if(!StringUtil.isNullorEmpty(refs.ctlSNNo.getValue())){
			...
			crStore.load({
				params : {
					...
					col1: theVslInfo.get('vslCd'),
					col2: refs.ctlSNNo.getSelection().get('mfDocId'),
					col3: 'E',
					col4: refs.ctlScn.getValue()
				},
				callback: function(records, operation, success) {
					if (success) {
						var theDetail = me.getViewModel().get('theLorryAssignment');
						if(records.length === 0 || (records[0].get('isValidated') !== 'Y')){
							theDetail.set('customsReleasedYn', 'N');
							refs.refBtnCreate.setDisabled(true);
							MessageUtil.warning('warning_msg', 'customsCargoReleaseControl_release_msg');
							return false;
						}else{
							refs.refBtnCreate.setDisabled(false);
							theDetail.set('customsReleasedYn', 'Y');
						}
					}
				}
			});
...
//				mfDocNo: me.getViewModel().data.theSearch.data.mfDocId
	...
	onChangeTruckMode: function (clt, newValue, oldValue, eOpts){
		...
		var	snNoComboStore = me.getStore('shipgNoteCombo');
		var userTp = MOST.config.Token.getUserType();
		var enabledRefs = [];
		var disabledRefs = [];
		var toggleRefs = [
			'refBtnAddFile',
			'refBtnRemoveFile',
			'refSpecialPermissionChk',
			'ctlGR',
			'ctlAdvMt',
			'ctlAdvM3',
			'ctlAdvQty',
		...
		if(refs.rdTruckMode.getValue().truckMode == 'E'){
			enabledRefs = toggleRefs
			disabledRefs.push('ctlContractor')
			blNoComboStore.clearFilter();
			snNoComboStore.clearFilter();
		}
		else {
			refs.ctlSubDoNo.setValue();
			refs.ctlGR.setValue();
			disabledRefs = toggleRefs
			me.filterBlSnComboItems()
			if(userTp === CONSTANTS.USER_TYPE_INTERNAL){
				refs.ctlContractor.setValue('')
				enabledRefs.push('ctlContractor')
			}else{
				refs.ctlContractor.setValue(MOST.config.Token.getPtnrCode());
				disabledRefs.push('ctlContractor')
			}
		...
		enabledRefs.forEach((item) => {
			refs[item].setDisabled(false)
		})
		disabledRefs.forEach((item) => {
	...
	onPreview : function() {
		var me = this;
		var refs = me.getReferences();
		var processQrCodeStore = me.getStore('processQrCode');
		var grid = me.lookupReference(me.MAIN_GRID_REF_NAME);
		var selection = grid.getSelection() == null ? null : grid.getSelection()[0];
		...
		updateParm.set('items', [selection.data]);
		updateParm.getProxy().url = processQrCodeStore.getProxy().url;
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		}else if(targetControl === 'ctlContractor'){
			var theSearch = me.getViewModel().get('theSearch')

			if(theSearch.get('vslCallId')){
				me.filterBlSnComboItems()
			}
		}
	},
	...
	openLorrysPopup:function(){
		...
		if(me.lookupReference('rdTruckMode').getValue().truckMode == 'E'){
			...
			var contractor = refs.ctlContractor.getValue()
			if(StringUtil.isNullorEmpty(contractor)){
				MessageUtil.warning('warning_msg', 'Please select contractor first');
				return
			}
	...
	filterBlSnComboItems: function(){
		var me = this
		var refs = me.getReferences()
		var ptnrCd = refs.ctlContractor.getValue();
		var userTp = MOST.config.Token.getUserType();
		var blNoComboStore = me.getStore('blNoCombo');
		var snNoComboStore = me.getStore('shipgNoteCombo');
		var partnerInfos = MOST.config.Token._patnerInfos.map(item => item.ptnrType)

		blNoComboStore.clearFilter();
		snNoComboStore.clearFilter();
		if(refs.rdTruckMode.getValue().truckMode == 'E') return
		if(userTp === CONSTANTS.USER_TYPE_EXTERNAL){
			if(partnerInfos.includes('CTT')){
				blNoComboStore.filterBy(function(record) {
					var isConveyor = record.data.tsptTpCd === 'CV';
					var blContractor = record.data.lorryComp
					var isMatchContractor = StringUtil.isNullorEmpty(blContractor) && blContractor == ptnrCd;
					return !isConveyor && isMatchContractor;
				});
				snNoComboStore.filterBy(function(record) {
					var isConveyor = record.data.tsptTpCd === 'CV';
					var snContractor = record.data.lorryComp
					var isMatchContractor = Util.isNullorEmpty(snContractor) && snContractor == ptnrCd;
					return !isConveyor && isMatchContractor;
				});
			}else{
				blNoComboStore.filterBy(function(record) {
			...
			blNoComboStore.filterBy(function(record) {
				var isConveyor = record.data.tsptTpCd === 'CV';
				var blContractor = record.data.lorryComp
				var isMatchContractor = StringUtil.isNullorEmpty(blContractor) && blContractor == contractor;
				return !isConveyor && isMatchContractor;
			});
			snNoComboStore.filterBy(function(record) {
				var isConveyor = record.data.tsptTpCd === 'CV';
				var snContractor = record.data.lorryComp
				var isMatchContractor = StringUtil.isNullorEmpty(snContractor) && snContractor == contractor;
				return !isConveyor && isMatchContractor;
			});
		}
	},
	...
	getSearchCondition : function() {
		...
		if(me.getViewModel().get('theVslInfo') != null){
			if((me.getViewModel().get('theVslInfo').id).indexOf('DeliveryOrder') == -1 && (me.getViewModel().get('theVslInfo').id).indexOf('GoodsReceipt') == -1){
				if((vslCallId == null || vslCallId == "") && (scn == null || scn == "")){
					MessageUtil.warning("LorryAssignment", "goodsreceipt_jpvc_input_msg");
					return;
				}
			}
		}
	...
	onBindingDocumentInformation: function(selection){
		...
		if(selection){
			...
			record.set('scn', selection.get('scn'));
		...
		for(var i = 0; i < store.data.length; i++){
			...
			if(rec!==record){
				if(record.getData().truckMode == 'E'){
					if((!StringUtil.isNullorEmpty(rec.get('doNo')) 
							&& !StringUtil.isNullorEmpty(record.getData().doNo)
							&& rec.get('doNo') === record.getData().doNo)
							&&
							(!StringUtil.isNullorEmpty(rec.get('grNo')) 
							&& !StringUtil.isNullorEmpty(record.getData().grNo)
