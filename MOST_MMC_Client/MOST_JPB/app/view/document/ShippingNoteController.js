...
//    			refs.ctlDeliveryMode.setValue(me.DELIVERY_MODE_INDIRECT);
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'ctlVslCallId'){ 
			...
			shipNote.set('contractor', returnValue.code);
			shipNote.set('contractorNm', returnValue.codeName);
		}
	},
...
//		}
		...
		if (StringUtil.isNullorEmpty(refs.ctlportOfDis.getValue()) && !refs.refBunkeringChk.getValue()){
...
//		}
	...
	//-------
		...
		detailItem.set('bkrChk', refs.refBunkeringChk.checked ? 'Y' : 'N');
		...
		shipgNoteDtlItem.set('scn', detailItem.get('scn'));
...
//		}
	...
	onBunkeringCheck: function(){
		var me = this;
     	var refs = me.getReferences();
     	var theSN = me.getViewModel().get('theShippingNote');
     	
     	if(refs.refBunkeringChk.getValue() == true){
     		refs.ctlportOfDis.items.getAt(0).setAllowBlank(true);
     		refs.ctlportOfDis.items.getAt(0).validate();
     	} else {
     		refs.ctlportOfDis.items.getAt(0).setAllowBlank(false);
     		refs.ctlportOfDis.items.getAt(0).validate();
     	}
	}
	/**
