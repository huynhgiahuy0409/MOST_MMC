Ext.define('MOST.view.document.excelupload.PackageDetailListController', {
    extend: 'MOST.view.foundation.BaseViewController',
    alias: 'controller.packagedetaillist',
	 ...
	 * CONSTANT START
	 */
   	MAIN_GRID_REF_NAME: 'refPackageDetailListGrid',
   	MAIN_STORE_NAME: 'packageDetailList',
	...
	onDetailListFileUpload : function() {
    	...
    	var input = document.querySelector("input[id='packageDetailListFileUpload-button-fileInputEl']");
		...
		for( index1 in storeData ) {
			...
			for( index2 in storeData ) {
				...
				if( record1.get('id') == record2.get('id') ){
					...
					if(record1.get('pkgNo') === record2.get('pkgNo')) {
						var targetGridCol = grid.getColumns().find(col => col.dataIndex == 'pkgNo' );
	...
	onSave: function(){
		...
		var roroDetailStore = me.getParentView().lookupReference('refPkgDetailGrid').getStore();
