Ext.define('MOST.view.billing.TariffCodeController', {
	...
	onChangeHandlingTon: function(){
		var me = this;
		var	refs = me.getReferences();
		
		if(refs.cmbHandlingTon.getValue() ==='L1'){
			refs.txtHdlFrom.setValue('');
			refs.txtHdlTo.setValue('350000');
			refs.cmbGlCode.setValue('6822010');
		}
		if(refs.cmbHandlingTon.getValue() ==='L2'){
			refs.txtHdlFrom.setValue('350000');
			refs.txtHdlTo.setValue('');
			refs.cmbGlCode.setValue('6822020');
		}
	},
	...
	onDetailSave: function(){
		...
		if(StringUtil.isNullorEmpty(refs.txtTrfCd.getValue()) 
				|| StringUtil.isNullorEmpty(refs.txtSubTrfCd.getValue()) 
				|| StringUtil.isNullorEmpty(refs.cmbTariffType.getValue()) 
				|| StringUtil.isNullorEmpty(refs.txtTariffDesc.getValue())){
			MessageUtil.warning('warning_msg','tariffInputMadatory');
			return;
		}
	...
	setDetailComboStore: function(recvData) {
		...
		var handlingTonCombo = me.getStore('handlingTonCombo');
		var glCombo = me.getStore('GLCombo');
		var tariffCodeDetail = me.getStore('tariffCodeDetail');
		gstTypeCombo.load();

		tariffConditionList.load();
		searchTariffCodeCombo.load();
		handlingTonCombo.load();
		glCombo.load();
...
//		}
	...
	saveProccess:function(){
		...
		detailItem.set('ivUnit1', 'MT');
		detailItem.set('trfTpCd', refs.cmbTariffType.getValue());
		detailItem.set('ssrTpCd', refs.cmbSSRTariffType.getValue());
		detailItem.set('pyrTpCd', refs.cmbPayer.getValue());
		detailItem.set('costCntCd', refs.refCostCenter.getValue());
		detailItem.set('glCd', refs.cmbGlCode.getValue());
...
//		}
	...
	setTariffConditionList:function(tariffCodeItem){
		...
		var tariffConditionPrptItem1 = tariffConditionPrptItem.copy();
		var tariffConditionPrptItem2 = tariffConditionPrptItem.copy();
		
			
		tariffConditionPrptItem1.set('prptCd', 'S');
		tariffConditionPrptItem1.set('prcTpCd', 'S1');
		tariffConditionPrptItem1.set('prptNm', 'Balance Handling MT');
		tariffConditionPrptItem1.set('dataTpCd', 'TIER');
		tariffConditionPrptItem1.set('dataLen', '2');
		
		tariffConditionPrptItem2.set('prptCd', 'S');
		tariffConditionPrptItem2.set('prcTpCd', 'S2');
		tariffConditionPrptItem2.set('prptNm', 'Order of the contract year');
		tariffConditionPrptItem2.set('dataTpCd', 'TIER');
		tariffConditionPrptItem2.set('dataLen', '2');
		
		tariffConditionPrptList.push(tariffConditionPrptItem1.getData());
		tariffConditionPrptList.push(tariffConditionPrptItem2.getData());
		
		var tariffConditionItem1 = tariffConditionItem.copy();
		var tariffConditionItem2 = tariffConditionItem.copy();
		
		
		tariffConditionItem1.set('seq', '1');
		tariffConditionItem1.set('prptCd', 'S1');
		tariffConditionItem1.set('chrVal', refs.cmbHandlingTon.getValue());
		tariffConditionItem1.set('tierVal1', refs.txtHdlFrom.getValue());
		tariffConditionItem1.set('tierVal2', refs.txtHdlTo.getValue());
		
		tariffConditionItem2.set('seq', '2');
		tariffConditionItem2.set('prptCd', 'S2');
		tariffConditionItem2.set('tierVal1', refs.txtFromYear.getValue());
		tariffConditionItem2.set('tierVal2', refs.txtToYear.getValue());
		
		tariffConditionList.push(tariffConditionItem1.getData());
		tariffConditionList.push(tariffConditionItem2.getData());
		//vessel opr
//		for(var i=1; i < 11; i++){
//			if(i == 1){
