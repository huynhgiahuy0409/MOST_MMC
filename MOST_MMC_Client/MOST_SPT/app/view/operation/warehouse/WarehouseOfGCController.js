Ext.define('MOST.view.operation.warehouse.WarehouseOfGCController', {
	...
	alias: 'controller.warehouseofgc',
	...
	onLoad: function(){
		...
		if(recvData.$className === 'MOST.model.operation.WHCheckExport'){
			whCheckExport = true;
		}
		refs.refWarehouseLayoutView.suspendEvents();
		
		me.loadWarehouseComboList(recvData);
	},
	
	loadWarehouseComboList : function(recvData){
		var me = this;
		...
		warehouseComboListStore.load({
			params : {
				...
				deAllocation: recvData.get('deAllocation'),
	...
	onChangePkgQty: function(clt, newValue, oldValue, eOpts){
		var me = this,
			...
			,store = me.getStore('allocatedCargoAmount')
			,accMt = accM3 = accQty = mt = m3 = 0;

		var recvData = me.getView().recvData;
		var grid = me.lookupReference('refCargoInfoGrid');
		...
		if(cgTpCd === 'BBK' || 
			...
			if(store.data.length > 0){
				...
				if (accQty + inputQty == refs.ctlTotalQty.getValue())
				{
					mt =  Number(recvData.get('grMt')) - Number(accMt);
					m3 = Number(recvData.get('grM3')) - Number(accM3);
				...
				if(inputQty == refs.ctlTotalQty.getValue()) {
					mt = recvData.get('grMt');
					m3 = recvData.get('grM3');
				}
				else {
					mt = eachMt * inputQty;
					m3 = eachM3 * inputQty;
			...
			var inputQty = Number(newValue); 
			if(selection){
				var eachMt = (Number(selection.data.wgt)/Number(selection.data.pkgQty)).toFixed(3);
				var eachM3 = (Number(selection.data.msrmt)/Number(selection.data.pkgQty)).toFixed(3);
				accMt = selection.data.wgt; 
				accM3 = selection.data.msrmt; 
				accQty = selection.data.pkgQty; 
				if (accQty + inputQty == refs.ctlTotalQty.getValue()){
					mt =  Number(recvData.get('grMt')) - Number(accMt);
					m3 = Number(recvData.get('grM3')) - Number(accM3);
				}else{
				...
				if(inputQty == refs.ctlTotalQty.getValue()) {
					...
					var eachMt = (Number(recvData.get('grMt'))/Number(recvData.get('grQty'))).toFixed(3);
					var eachM3 = (Number(recvData.get('grM3'))/Number(recvData.get('grQty'))).toFixed(3);
					mt = eachMt * inputQty;
					m3 = eachM3 * inputQty;
				}
			}
		...
		refs.refAllocatedMT.setValue(mt);
		refs.refAllocatedM3.setValue(m3);
	},
	...
	setHandlingCargoInformation : function(){
		...
		var cgTpCd = recvData.get('cgTpCd');
		var cgTpCdDBKArr = [CodeConstants.MT_CGTP_DBN, CodeConstants.MT_CGTP_DBE, CodeConstants.FZ_CGTP_DBK, CodeConstants.MT_CGTP_LQE, CodeConstants.MT_CGTP_LQNE];
		var isDBK = cgTpCdDBKArr.includes(cgTpCd);
			...
			me.warehouseBuilder.onClick(e, function(builder, item, e){
				...
				if(selectedCell) {
					...
					Ext.Array.each(carfoInfo, function(rec) {
						//Fix: Cargo Info grid show null data
						if(me.getView().recvData.get('vslCallId') == rec.get('vslCallId')){							
							if(me.getView().recvData.get('id').indexOf("HandlingOut") != -1 || me.getView().recvData.get('id').indexOf("WHCheckExport") != -1){
								if(rec.get('shipgNoteNo') == null || rec.get('shipgNoteNo') == ''){		
									if(((me.getView().recvData.get('blSn') == rec.get('shipgNoteNo'))|| 
											(me.getView().recvData.get('blSn') == rec.get('blNo')) ||
											((me.getView().recvData.get('blSn') == null || me.getView().recvData.get('blSn') == "") && (rec.get('blNo') == null || rec.get('blNo') == "")))
											&& (me.getView().recvData.get('cgNo') == rec.get('cgNo')))
										store.add(rec.data);
								}else{
									if(((me.getView().recvData.get('blSn') == rec.get('shipgNoteNo'))|| 
											(me.getView().recvData.get('blSn') == rec.get('blNo')) ||
	...
	onCargoInfoCellClick:function(gridview, tdEl, cellIndex, record, trEl, rowIndex, e){
		...
		var recvData = me.getView().recvData;
		if(recvData.$className === 'MOST.model.operation.CargoHandlingOut'||
		    recvData.$className === 'MOST.model.operation.WHCheckExport'){
