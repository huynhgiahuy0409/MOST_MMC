Ext.define('MOST.view.operation.hht.CargoMovementHHTController', {
	extend: 'MOST.view.foundation.BaseViewController',

	requires: [
	],

	alias: 'controller.cargomovementhhtctl',
	
	/**
	 * =========================================================================================================================
	 * CONSTANT START
	 */
	CARGO_MOVEMENT_PROXY_URL : MOST.config.Locale.getRestApiDestUrl() + '/v1/confirmmovement/cargoMovement',
	prevData:null,
	...
	onSetLocation: function(){
		...
		if(refs.refConfirmMovementPopupMoveDateTime.getValue() == null){
			MessageUtil.warning('warning_msg', 'confirmmovememt_input_date_msg');
			refs.refConfirmMovementPopupMoveDateTime.focus();
			return;
		}
		
		var whConfigurationItem1 = new Ext.create('MOST.model.configuration.WhConfiguration');
		me.updateRecordWithNoCommit(whConfigurationItem1, detailItem); // 'cgNo', 'vslCallId', 'opeClassCd', 'delvTpCd' ...
		
		detailItem.set('fmLocId', 'Y7A(A1,2)');
		detailItem.set('toLocId', 'Y7A(A2,2)');
		detailItem.set('fmLoc', 'Y7A(A1,2)');
		detailItem.set('toLoc', 'Y7A(A2,2)');
		detailItem.set('pkgQty', 1);
		detailItem.set('wgt', 0.068);
		detailItem.set('msrmt', 0.112);

		whConfigurationItem1.set('workingStatus', WorkingStatus.INSERT);
		whConfigurationItem1.set('fmLocId', 'Y7A-A1');
		whConfigurationItem1.set('toLocId', 'Y7A-A2');
		whConfigurationItem1.set('whTpCd', 'G');
		whConfigurationItem1.set('fmPkgQty', 1);
		whConfigurationItem1.set('fmWgt', 0.389);
		whConfigurationItem1.set('fmMsrmt', 0.514);
		whConfigurationItem1.set('pkgQty', 1);
		whConfigurationItem1.set('wgt', 0.068);
		whConfigurationItem1.set('msrmt', 0.112);
		
		store.insert(0, whConfigurationItem1);
	},
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		var store = me.getStore('confirmMovementList'); 
		store.removeAll();
		for(var i = 0; i< returnValue.data.whConfigurationMap.data.length; i++){
			var record = returnValue.data.whConfigurationMap.data.items[i];
			record.dirty = true;
	...
	setDetailInitialize:function(masterItem){
		...
		if(recvData.get('cgType') == 'E'){
			refs.ctlBlGr.setHidden(true);
			refs.ctlSn.setHidden(false);
			refs.ctlSn.setValue(recvData.get('shipgNoteNo'));
		}
...
//	    item.set('shftDt', Ext.Date.format(MOST.config.Token.getWorkDate(), 'd/m/Y'));
//	    item.set('shftId', MOST.config.Token.getWorkShift());

	    if (arrItems.length > 0) {
	    	
	    	item.set('whItems', arrItems);
			var updateParm = Ext.create('MOST.model.foundation.parm.UpdateBizParm');
			updateParm.getProxy().url = MOST.config.Locale.getRestApiDestUrl() + '/v1/cargomovement/cargoMovement';
			updateParm.phantom = false;
