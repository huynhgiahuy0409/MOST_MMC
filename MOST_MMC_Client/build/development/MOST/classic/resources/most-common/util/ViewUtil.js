...
ViewUtil.openPDFPreview1=function(records, operation, success){
	
	var content = records[0].get("content").replace(/&lt;/gi,'<').replace(/&gt;/gi,'>');
	var pdfData = window.btoa(content);
	
	
	if (window.navigator && window.navigator.msSaveOrOpenBlob) {
	    var byteCharacters = window.atob(pdfData);
		...
		var winpdf1 = new Ext.Window({
	        ...
	        width: 1000,
	        height: 700,
	        stateId: 'statePDFViewer',
	        stateful: true,
	        plain:true,
	        constrain:false,
	        constrainHeader:true,
	        html: "<iframe width='100%' height='100%' src='" + URL.createObjectURL(blob) +"'></iframe>"
	    })
		...
		var winpdf1 = new Ext.Window({
	        ...
	        constrain:false,
	        constrainHeader:true,
	        html: "<iframe width='100%' height='100%' src='data:application/pdf;base64, " + pdfData+"'></iframe>"
	    })
