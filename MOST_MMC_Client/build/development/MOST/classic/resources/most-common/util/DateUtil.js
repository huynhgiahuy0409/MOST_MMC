...
DateUtil.checkDateFormat=function(me, dateControlName, isMandatory, isDateTime){
	...
	if(isDateTime){
		dateString = dateValue==null?null:Ext.Date.format(dateValue, MOST.config.Locale.getDefaultDateFormatWithNoSeconds());
	} else {
		dateString = dateValue==null?null:Ext.Date.format(dateValue, MOST.config.Locale.getShortDate());
	}
...
DateUtil.validatePeriodDate=function(fromDate, toDate, maxDatePeriod){
	...
	if(periodDay > maxDatePeriod){
		MessageUtil.warning('Warning','validMaxPeriod_msg', Math.trunc(maxDatePeriod/30));
		return false;
	}
