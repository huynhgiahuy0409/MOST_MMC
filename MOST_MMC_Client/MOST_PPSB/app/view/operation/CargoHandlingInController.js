Ext.define('MOST.view.operation.CargoHandlingInController', {
	...
	onChangeHandlingInAmt: function (clt, newValue, oldValue, eOpts) {
		...
		if (clt.reference === 'ctlLoadQty' && cgTpCd === CodeConstants.MT_CGTP_BBK) {
			...
			if (detailItem.get('eachWgt') == 0) {
				eachWgt = Math.round(1000 * detailItem.get('snMt')/ detailItem.get('snQty')) /1000;
				detailItem.set('eachWgt', eachWgt);
			}

			if (detailItem.get('eachMsrmt') == 0) {
				 eachMsrmt = Math.round(1000 * detailItem.get('snM3')/ detailItem.get('snQty')) /1000;
				detailItem.set('eachMsrmt', eachMsrmt);
			}
			...
			if (inputQty === detailItem.get('balQty')) {
				detailItem.set('wgt', detailItem.get('balMt'));
				detailItem.set('msrmt', detailItem.get('balM3'));
			} else {
				detailItem.set('wgt', Math.round(1000 * detailItem.get('eachWgt') *  inputQty) / 1000);
				detailItem.set('msrmt', Math.round(1000 * detailItem.get('eachMsrmt') *  inputQty) / 1000);
			}
		}
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		}else if (targetControl === 'ctlHIDriverIdField') {
			if (returnValue && returnValue.item) {
				detailItem.set('driverId', returnValue.item.get('driverId'));
				detailItem.set('driverName', returnValue.item.get('driverName'));
			}
		} 
		else {
...
//			detailItem.set('whBalQty', detailItem.get('balQty') - detailItem.get('accuSumQty'));
			...
			//S - Set balance to 0 if negative and default value for handling in:
			if (detailItem.get('balMt') < 0) {
				detailItem.set('balMt', 0);
				detailItem.set('whBalWgt', 0);
				detailItem.set('wgt', 0);
			} else {
				detailItem.set('whBalWgt', detailItem.get('balMt'));
				detailItem.set('wgt', detailItem.get('balMt'));
			}

			if (detailItem.get('balM3') < 0) {
				detailItem.set('balM3', 0);
				detailItem.set('whBalMsrmt', 0);
				detailItem.set('msrmt', 0);
			} else {
				detailItem.set('whBalMsrmt', detailItem.get('balM3'));
				detailItem.set('msrmt', detailItem.get('balM3'));
			}

			if (detailItem.get('balQty') < 0) {	
				detailItem.set('balQty', 0);
				detailItem.set('whBalQty', 0);
				detailItem.set('pkgQty', 0);
			} else {
				detailItem.set('whBalQty', detailItem.get('balQty'));
				detailItem.set('pkgQty', detailItem.get('balQty'));
			} //E - Set balance to 0 if negative and default value for handling in
...
//		}
	...
	openDriverIDPopup:function(field, button, e){
		...
		me.openCodePopup('popup-driverlistpopup',  'ctlHIDriverIdField', params);
	},
	/**
	 * GENERAL METHOD END
	 * =========================================================================================================================
