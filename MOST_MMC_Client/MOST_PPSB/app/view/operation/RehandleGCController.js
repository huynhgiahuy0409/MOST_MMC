Ext.define('MOST.view.operation.RehandleGCController', {
	...
	onChangeNxRefNo: function(){
		var me = this,
		refs = me.getReferences()
		,sltNxRefNo = refs.ctlTxtRehandleOpModeSNNo.getSelection()
		,detailItem = me.getViewModel().get('theDetail');
		if(sltNxRefNo){
			var nxRefNo = sltNxRefNo.get('shipgNoteNo')
			var mfDocId = sltNxRefNo.get('mfDocId')
			detailItem.set('nxGrNo', sltNxRefNo.get('grNo'))
			refs.ctlTxtRehandleOpModeBkNo.setValue(mfDocId)
		}
...
//		params['sizePerPage'] = sizePerPage;
	...
	onChangeRehandleMode: function(combo, value, obj){
		...
		if(value === 'C'){
			...
			me.defaultSN = detailItem.clone().get('nxRefNo')
			...
			refs.refCbxAutoGeneration.setValue(true)
		}else if(value === 'R'){
			isRTS = true;
	...
	cudData : function(){
		...
		var isChecked = refs.refCbxAutoGeneration.checked
		var bkNo = detailItem.get('bookingNo')
		recvData.commit();
		...
		if(detailItem.get('rhdlMode') === 'C' && StringUtil.isNullorEmpty(bkNo)){
			MessageUtil.warning('warning_msg', 'Please enter Booking No');
			return;
		}else if(detailItem.get('rhdlMode') === 'C' && !StringUtil.isNullorEmpty(bkNo)){
			addItem.set('bkNo', bkNo);
		}
		...
		addItem.set('scn', detailItem.get('scn'));
		addItem.set('nxScn', detailItem.get('nxScn'));
		addItem.set('nxGrNo', detailItem.get('nxGrNo'));
...
//		}
	...
	onCbNxtSNChange: function() {
		var me = this
		, refs = me.getReferences()
		, isChecked = refs.refCbxAutoGeneration.getValue()
		if(refs.ctlRehandleOpModeRehandleMode.getValue() === 'C'){
			if(isChecked){
				refs.ctlTxtRehandleOpModeSNNo.setDisabled(true)
				refs.ctlTxtRehandleOpModeSNNo.setValue(me.defaultSN)
				refs.ctlTxtRehandleOpModeBkNo.setDisabled(false)
			}else{
				refs.ctlTxtRehandleOpModeSNNo.setDisabled(false)
				refs.ctlTxtRehandleOpModeSNNo.setValue('')
				refs.ctlTxtRehandleOpModeBkNo.setDisabled(true)
			}
		}
	}
