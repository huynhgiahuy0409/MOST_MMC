Ext.define('MOST.view.operation.CargoManualCtlController', {
	...
	onTabChange : function(tabPanel, newCard, oldCard, eOpts) {
		...
		switch(tabPanel.getActiveTab().name){
			...
			case 'export':
				me.caTyCd = 'EX';
				refs.refBL.setHidden(true);
				refs.refShippingNote.setHidden(false);
			...
			case 'import':
				me.caTyCd = 'IM';
				refs.refShippingNote.setHidden(true);
				refs.refBL.setHidden(false);
				refs.ctlLoadingButton.setDisabled(true);
	...
	onLoading : function(){
		...
		// if(selection.get('statCd') != 'CF'){
		// 	MessageUtil.warning('warning_msg', 'sn_not_confirmed');
		// 	return;
		// }
		...
		if(cgTpCdList.includes(selection.get('cgTpCd'))){
			if(refs.ctlCargoManualCtlShiftDt.getValue() == '' || refs.ctlCargoManualCtlShiftDt.getValue() == null ||
					...
					refs.refCboHatchNo.getValue() == '' || refs.refCboHatchNo.getValue() == null){
...
//				}
	...
	onChangeSearchCondition: function(ctl){
		var me = this;
		var refs = me.getReferences();

		if(ctl != undefined && ctl.reference === 'refCboHatchNo') {
			return;
		}
	...
	setHiddenSnBlButton: function(visible){
		...
		refs.ctlCargoManualCtlGr.setHidden(!visible);
		refs.ctlCargoManualCtlGrButton.setHidden(!visible);
		refs.ctlCargoManualCtlBookingNo.setHidden(!visible);
		refs.ctlBlContainer.setHidden(visible);
		refs.ctlCargoManualCtlBl.setHidden(visible);
	},
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'txtTruckNo'){
			if(returnValue) {
				...
				}else if(returnValue.item && returnValue.item.data.eqFacNo){
					theSearch.set('lorryNo',returnValue.item.data.eqFacNo)
				}else{
		...
		if(targetControl === 'ctlCargoManualCtlJpvcfield'){ // JPVC Popup
			...
			if(returnValue){
				...
				if(!StringUtil.isNullorEmpty(returnValue.item.get('vslCallId'))){
					...
					me.getViewModel().setData({theVsl:returnValue.item});
					var theVsl =  me.getViewModel().get('theVsl');
					theSearch.set('scn', theVsl.get('scn'));
	...
	setShiftDtCombo: function(){
		...
		var refs = me.getReferences();
		var theVsl =  me.getViewModel().get('theVsl');
		var vslCallId = StringUtil.isNullorEmpty(refs.ctlCargoManualCtlJpvcfield.getValue()) ? theVsl.get('vslCallId') : refs.ctlCargoManualCtlJpvcfield.getValue();
		var shiftDtStore = me.getStore('cargoManualCtlForShiftDtCombo');
		var shiftStore = me.getStore('cargoManualCtlForShiftNoCombo');
	...
	setGeneralTabInitialize: function (record) {
		...
		if (record) {
			...
			theGeneral.eqStDt = theGeneral.eqStDt ? new Date(theGeneral.eqStDt) : '';
...
//		}
	...
	getSearchConditionExport: function(isIndicator){
		...
		else if (!searchParm.get('lorryNo') && (searchParm.get('vslCallId') || searchParm.get('scn'))){// Warehouse check => ListType is SN level
...
//		}
	...
	getSearchConditionImport: function(isIndicator){
		...
		else if (searchParm.get('vslCallId') || searchParm.get('scn')){
...
// 	},
	...
	onSelectConveyorNo: function (combobox, record, eOpts) {
		var me = this;
		var refs = me.getReferences();
		var conveyor = combobox.getSelection();

		if (conveyor) {
			refs.refLoadingRate.setValue(conveyor.get('capa'));
			me.calculateETCAndSave();
		} else {
			refs.refLoadingRate.setValue(null);
		}
	},

	calculateETCAndSave: function () {
		var me = this;
		var refs = me.getReferences();
		var conveyorNo = refs.refConveyorNo.getValue();
		var startTime = refs.refStartTime.getValue();
	
		if (StringUtil.isNullorEmpty(conveyorNo) || StringUtil.isNullorEmpty(startTime)) {
			return;
		}
	
		var theGeneral = me.getViewModel().get('theGeneral');
		var documentInfo = me.getStore('cargoManualCtlGeneralTabTotal').getAt(0);
	
		var etc = me.calculateETC(startTime, theGeneral, documentInfo);
		
		if (etc) {
			refs.refETC.setValue(etc);
			me.saveEquipmentForDocument(conveyorNo, startTime, etc);
		}
	},
	
	calculateETC: function(startTime, theGeneral, documentInfo) {
		var me = this;
		var refs = me.getReferences();
		var loadingRate = theGeneral.loadingRate;
	...
	saveEquipmentForDocument: function(conveyorNo, startTime, etc) {
		var me = this;
		var updateParm = Ext.create('MOST.model.foundation.parm.UpdateBizParm');
		var insertItem = Ext.create('MOST.model.operation.CargoManualCtl');
		var {blNo, catgCd, mfDocId, shipgNoteNo} = me.getViewModel().get('theGeneral');
		var {vslCd, vslCallId, callYear, callSeq, scn} = me.getViewModel().get('theVsl').getData();
		...
		var data = {
			...
			catgCd, 
			mfDocId, 
			cgNo: blNo || shipgNoteNo,
			eqFacNo: conveyorNo,
			eqStDt: Ext.Date.format(startTime, MOST.config.Locale.getDefaultDateFormatWithNoSeconds()),
		}

		updateParm.getProxy().url =  MOST.config.Locale.getRestApiDestUrl() + '/v1/cargomanualctl/cargoManualEquipment';
		updateParm.phantom = false;
		updateParm.set('item', data);
		updateParm.save({
	...
	enableETCCalculationArea: function(chk) {
		...
		var documentInfo = me.getStore('cargoManualCtlGeneralTabTotal').getAt(0);
		var theGeneral = me.getViewModel().get('theGeneral'); 
		var etc = me.calculateETC(theGeneral.eqStDt, theGeneral, documentInfo);
		if (etc) {
			refs.refETC.setValue(etc);
		}
	}
