...
//	     	refs.ctlVslCallId.setValue(recvData.vslCallId);
			...
			refs.ctlHatchCombo.setValue(recvData.hatchNo);
	     	...
	     	me.onGetVesselInformationItem();
		}
	},
	
	onGetVesselInformationItem: function() {
		var me = this;
		var refs = me.getReferences();
		
		var store = me.getStore('vesselInformation');
...
//					stDt: Ext.Date.format(refs.refInputDate.getValue(), MOST.config.Locale.getShortDate()),
	...
	updateDelayStartDate:function(value){
		...
		if(!selection){
			var start = Ext.Date.format(value, MOST.config.Locale.getDefaultDateFormatWithNoSeconds());
			if(StringUtil.isNullorEmpty(refs.refEndDate.getValue())){
				return;
			}
			var end = Ext.Date.format(refs.refEndDate.getValue(), MOST.config.Locale.getDefaultDateFormatWithNoSeconds());
			var startTime = Ext.Date.parse(start,  'd/m/Y H:i');
			var endTime = Ext.Date.parse(end,  'd/m/Y H:i');
			
			if (startTime != null && endTime != null) {
				totalHours = Math.round((endTime.getTime()-startTime.getTime())*0.001/3600*100)/100;
				refs.refTotalHRS.setValue(totalHours);
			}
	...
	updateDelayEndDate:function(value){
		...
		if(!selection){
			var end = Ext.Date.format(value, MOST.config.Locale.getDefaultDateFormatWithNoSeconds());
			if(StringUtil.isNullorEmpty(refs.refStartDate.getValue())){
				return;
			}
			var start = Ext.Date.format(refs.refStartDate.getValue(), MOST.config.Locale.getDefaultDateFormatWithNoSeconds());
			var startTime = Ext.Date.parse(start,  'd/m/Y H:i');
			var endTime = Ext.Date.parse(end,  'd/m/Y H:i');
