Ext.define('MOST.view.operation.CargoManualCtlForROROController', {
	...
	alias: 'controller.cargomanualctlforroro',
	
	/**
	 * =========================================================================================================================
	 * CONSTANT START
	 */
	EXPORT_GRID_REF_NAME: 'refCargoManualCtlROROTabExportGrid',
	IMPORT_GRID_REF_NAME: 'refCargoManualCtlROROTabImportGrid',
	GATEPASS_GRID_REF_NAME: 'refCargoManualCtlTabGatePassGrid',

	GENERAL_STORE_NAME: 'cargoManualCtlCargoGeneral',
	EXPORT_STORE_NAME: 'cargoManualCtlROROTabExport',
	IMPORT_STORE_NAME: 'cargoManualCtlROROTabImport',
...
//		me.onHatchDataSet();
		me.onTabChange(refs.ctlCargoManualCtlROROTabPanel);
	},
	...
	onUpperCaseGP: function(control){
		...
		refs.ctlCargoManualCtlROROTabPanel.setActiveTab(3);
	},
	...
	onTabChange : function(tabPanel, newCard, oldCard, eOpts) {
		...
		switch(tabPanel.getActiveTab().name){
			case 'export':
				me.caTyCd = 'EX';
				refs.refCargoManualROROSearchMode.setValue('SN');
				me.onSearchModeClick(refs.refCargoManualROROSearchMode);
			...
			case 'import':
				...
				refs.refCargoManualROROSearchMode.setValue('BL');
				me.onSearchModeClick(refs.refCargoManualROROSearchMode);
				break;
		}
	},
	...
	onSearch: function(){
		...
		var tabPanel = refs.ctlCargoManualCtlROROTabPanel.getActiveTab().name;
	...
	onSearchModeClick: function(btn, e) {
    	...
    	if(btn.getValue() === 'SN'){
    		me.setHiddenSnBlButton(true);
    		refs.ctlCargoManualCtlROROTabPanel.setActiveTab(0);	
			//refs.txtTruckNo.params.flag = true;		
    	} else {
    		me.setHiddenSnBlButton(false);
    		refs.ctlCargoManualCtlROROTabPanel.setActiveTab(1);
...
//						me.setShiftDtCombo();
	...
	onSnBlComboChange: function(combo, value, obj){
		...
		if(combo.reference === 'ctlCargoManualCtlSn'){
			...
			var gridExport = refs.refCargoManualCtlROROTabExportGrid;
...
//			me.getViewModel().setData({theGeneral: null});
	...
	onLoading : function(){
		...
		var grid = me.lookupReference('refCargoManualCtlROROTabExportGrid');
		var selection = grid.getSelection() == null ? null : grid.getSelection()[0];
		var validationCodeStore = me.getStore('cargoManualCtlValidationCode');
		var searchParm = me.getViewModel().get('theSearch');

		var isYardTruck = refs.refSelectedTruck.value;
		
		//if(!me.checkWorkButtonMandatory()) return;

//		if(Number(selection.get('storedQty')) == 0 && selection.get('delvTpCd') == 'I') return;
				...
				if (success) {
					if(records != null && records.length > 0){
						...
						if(records[0].get('isValidated') === 'Y'){
							if((refs.txtTruckNo.getValue() == null || refs.txtTruckNo.getValue() == '') 
									&& (refs.txtDriverId.getValue() == null || refs.txtDriverId.getValue() == '')
									&& selection.get('delvTpCd') == 'D'){
								MessageUtil.warning('warning_msg', 'Please input lorry number or driver number.');
							}else{
								me.getView().detailViewAlias = 'app-confirmloadingofroro';
...
//			searchParams.set('shftNm', refs.ctlCargoManualCtlShiftNo.getRawValue());
		...
		validationCodeStore.load({
			...
			callback: function(records, operation, success) {
				if (success) {
					if(records != null && records.length > 0){
						if(records[0].get('isValidated') === 'Y'){
							me.getView().detailViewAlias = 'app-whcheckexportforroro';
							me.openDetailPopup(searchParams, title);
						} else {
	...
	onDischarging : function(){
		var me = this;
		var refs = me.getReferences();
		var title = {type: 'bundle', key: 'hht_discharingLDRORO'};
		var grid = me.lookupReference('refCargoManualCtlROROTabImportGrid');
		var selection = grid.getSelection() == null ? null : grid.getSelection()[0];
		var searchParm = me.getViewModel().get('theSearch');
		...
		if (me.LIST_TYPE == 'BL') {
			...
			me.getView().detailViewAlias = 'app-confirmdischargingofroro';
			me.openDetailPopup(searchParams, title);
		}
		else {
	...
	onHandlingIn : function(){
		...
		if(selection == null){
			...
			if (me.gateObject.driverId != '') {
				driverId = me.gateObject.driverId;
			}
		...
		validationCodeStore.load({
			...
			callback: function(records, operation, success) {
				if (success) {
					if(records != null && records.length > 0){
						if(records[0].get('isValidated') === 'Y'){
							if(me.lookupReference('refCargoManualCtlROROTabExportGrid').getSelection()[0].get('delvTpCd') == 'D'){
								MessageUtil.warning('warning_msg', 'This is can not confirm handling in because delivery type is direct.');
							}else{
								me.getView().detailViewAlias = 'app-confirmhandlinginofroro';
								me.openDetailPopup(searchParams, title);
							}
						} else {
	...
	onHandlingOut:function(){
		...
		switch(refs.ctlCargoManualCtlROROTabPanel.getActiveTab().name){
			case 'export':
				grid = me.lookupReference('refCargoManualCtlROROTabExportGrid');
				break;
			
			case 'import':
				grid = me.lookupReference('refCargoManualCtlROROTabImportGrid')
				break;
		}
		...
		if (me.LIST_TYPE == 'BL') {
			...
			if(StringUtil.isNullorEmpty(refs.txtDriverId.getValue()) && StringUtil.isNullorEmpty(refs.txtTruckNo.getValue())){
				MessageUtil.warning('warning_msg', 'cargomanualctl_no_truck_handling_out_msg');
				return;
			}
			...
			searchParams.set('gateTxnNo', me.gateObject.gateTxnNo);
//			searchParams.set('wbTransactionNo', me.gateObject.wbTransactionNo);
			searchParams.set('sdoNo', selection.data.sdoNo);
			searchParams.set('grNo', me.gateObject.grNo);
			searchParams.commit();
			
			me.getView().detailViewAlias = 'app-confirmhandlingoutofroro';
			me.openDetailPopup(searchParams, title);
		}
		else {
...
//		searchParams.set('shftNm', refs.ctlCargoManualCtlShiftNo.getRawValue());
		...
		me.getView().detailViewAlias = 'app-whcheckimportforroro';
		me.openDetailPopup(searchParams, title);
	...
	onChangeYardTruck: function(ctl, newVal, oldVal, eOpts ){
		...
		var refs = me.getReferences();
		var searchParm = me.getViewModel().get('theSearch');
		var mode = refs.refCargoManualROROSearchMode.getValue();
	...
	onChangeSearchCondition: function(){
		...
		var grid2 = me.lookupReference('refCargoManualCtlROROTabImportGrid');
	...
	onOpenTruckPopup: function(){
		...
		var params = {
				...
				shipgNoteNo: refs.ctlCargoManualCtlSn.getValue(),
				blNo: refs.ctlCargoManualCtlBl.getValue(),
				cgTpCd: 'RORO'
			};
...
//				if(!refs.ctlCargoManualTruckType.checked){
					me.gateObject = returnValue.item.data;
					refs.txtTruckNo.setValue(returnValue.code);
//				}
			}
			return
			
		}
		// sMantis: 0166710 - After selecting the driver from the combo box, set the value of the sDoNo parameter.
		if(xtype ==='popup-driverlistpopup'){
			me.onChangeSearchCondition();
			if(returnValue) {
				me.gateObject = returnValue.item.data;
				refs.txtDriverId.setValue(returnValue.code);
			}
			return;
	...
	jpvcChange : function(){
		var me = this;
		var refs = me.getReferences();
		var exportStore = me.getStore('cargoManualCtlROROTabExport');
		var importStore = me.getStore('cargoManualCtlROROTabImport');
		var tabPanel = refs.ctlCargoManualCtlROROTabPanel.getActiveTab().name;
...
//		me.setShiftDtCombo();
	...
	onMasterBLBookingNoChange: function(ctl){
		...
		if(ctl.reference === 'ctlCargoManualCtlBookingNo'){
			snComboStore.clearFilter();
			var mfDocId = ctl.getValue();
			snComboStore.filterBy(function(rec) {
				return (rec.get('mfDocId') === mfDocId)
			});
		}
		else if(ctl.reference === 'ctlCargoManualCtlMasterBL'){
			blComboStore.clearFilter();
			var mfDocId = ctl.getValue();
			blComboStore.filterBy(function(rec) {
				return (rec.get('mfDocId') === mfDocId)
			});
		}
		me.onChangeSearchCondition();
...
//		searchParams.set('shiftId',	refs.ctlCargoManualCtlShiftNo.getValue());
	...
	getSearchConditionExport: function(isIndicator){
		...
		}else if(searchParm.get('lorryNo') || searchParm.get('driverId')){// Handling-In or Loading Direct => ListType is GR List
			me.LIST_TYPE = "GR"; // => HI - LD
		}
		else if ((!searchParm.get('lorryNo') || !searchParm.get('driverId')) && searchParm.get('vslCallId')){// Warehouse check => ListType is SN level
			me.LIST_TYPE = "SN"; // => WH
		}else {
			return null;
		}
		...
		var driverId = searchParm.get('driverId');
...
//		}
//		else 
		if (refs.ctlBargeOperationChk.getValue() == true) {
			...
			grid.setColumns(GridUtil.getGridColumns('CargoManualCtlROROTabImport'));
			me.LIST_TYPE = 'BL';
		}else {
			return null;
		}
	...
	searchExportTbl:function(isScan){
		var me = this;
		var refs = me.getReferences();
		var store = me.getStore('cargoManualCtlROROTabExport');
...
// 	},
	...
	onRadioEditableChange: function(field, newValue, oldValue) {
		var me = this;
		var refs = me.getReferences();
		var exStore = me.getStore(me.EXPORT_STORE_NAME);
		var imStore = me.getStore(me.EXPORT_STORE_NAME);

		if(!oldValue){
			if(field.getReference() != null && field.getReference() == 'refSelectedTruck'){
				refs.txtDriverId.setDisabled(true);
				refs.txtTruckNo.setDisabled(false);
				refs.txtDriverId.setValue('');
			}else if( field.getReference() == 'refSelectedDriver'){
				refs.txtDriverId.setDisabled(false);
				refs.txtTruckNo.setDisabled(true);
				refs.txtTruckNo.setValue('');
			}
		}
	...
	onOpenDriverPopup:function(field, button, e){
		var me = this;
		var refs = me.getReferences();
		var vslCallId = refs.ctlCargoManualCtlJpvcfield.getValue();
		var params = {
			vslCallId: vslCallId,
			driverId: refs.txtDriverId.getValue(),
			cgTpCd: 'RCV',
			shipgNoteNo: refs.ctlCargoManualCtlSn.getValue(),
			blNo: refs.ctlCargoManualCtlBl.getValue()
//			codeType: 'GI'
		};
	    me.openCodePopup('popup-driverlistpopup',  'txtDriverId', params);
	},
	
	onChangeDriverId: function(ctl, newVal, oldVal, eOpts ){
		var me = this;
		var refs = me.getReferences();
		var exStore = me.getStore(me.EXPORT_STORE_NAME);
		var imStore = me.getStore(me.EXPORT_STORE_NAME);
