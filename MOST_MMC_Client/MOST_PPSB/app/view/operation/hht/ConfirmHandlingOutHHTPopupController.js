Ext.define('MOST.view.operation.hht.ConfirmHandlingOutHHTPopupController', {
		...
		viewPlanInfoStore.load({
			...
			callback: function(records, operation, success) {
				if(records && records.length > 0 && records[0].data.locId){
					if (records.length > 1 && loadQty < balQty && (recvData.get('cgTpCd') == 'BBK' || recvData.get('cgTpCd') == 'RBK')|| 
						records.length > 1 && loadMt < balMt && recvData.get('cgTpCd') == 'DBN' ) 
					{
						me.prevWhItems = null;
						refs.ctlConfirmHandlingOutLocId.setValue();
						return;
					}
					else if(records.length > 1 && loadQty == balQty && (recvData.get('cgTpCd') == 'BBK' || recvData.get('cgTpCd') == 'RBK')){
					...
					else if(records.length > 1 && loadMt == balMt && recvData.get('cgTpCd') == 'DBN'){
