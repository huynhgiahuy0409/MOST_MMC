Ext.define('MOST.view.billing.ImportReconcileController', {
	extend: 'MOST.view.foundation.BaseViewController',

	requires: [
	],

	alias: 'controller.importreconcile',
	...
	onLoad: function(){
		...
		var searchParm = Ext.create('MOST.model.billing.SearchImportReconcileParm');
		
		var cargoTypeCombo = me.getStore('cargoTypeCombo');
		var transportTypeCombo = me.getStore('importReconcileTransportTypeList');
		var packageTypeCombo = me.getStore('importReconcilePackageTypeList');
	...
	onSearch: function() {
    	...
    	var statusList = me.getStore('importReconcileStatusList');
		var transportTypeCombo = me.getStore('importReconcileTransportTypeList');
		var packageTypeCombo = me.getStore('importReconcilePackageTypeList');
	...
	onGridComboRenderer: function(val, cell){
		...
		}else if(cell.column.dataIndex === 'tsptTpCd'){
			codeComboStore = me.getViewModel().getStore('importReconcileTransportTypeList');
		}
		
		if(codeComboStore != null){
			var indx = -1;
			cell.style =  'background: #ffffb3;; color: red; align: right';
	...
	onCellClick: function(ctx, td, cellIndex, record, tr, rowIndex, e, eOpts) {
		...
		var statusStore = me.getStore('importReconcileStatusList');
		var transportTypeCombo = me.getStore('importReconcileTransportTypeList');
		var packageTypeCombo = me.getStore('importReconcilePackageTypeList');
		...
		var grid = me.lookupReference('refimportReconcileGrid');
		...
		refs.refimportReconcileGrid.getPlugin('importReconcileEditor').cancelEdit();
		
		refs.ctlBLMT.getEditor().setEditable(true);
		refs.ctlBLM3.getEditor().setEditable(true);
		refs.ctlBLQTY.getEditor().setEditable(true);
	},
	...
	onValidateEdit:	function(editor, context) {
		...
		if(context.record.phantom == true) {
			var systemCodeDuplicateCheckStore = me.getStore('importReconcileRecclList');
			
			me.gridDupliationCheck(editor, context, systemCodeDuplicateCheckStore, params);
		}
	},
	
	onSelectTransportType: function( combo, record, eOpts ) {
		var lcdVal = combo.getValue();
		this.lookupReference('importReconcileEditor').setValue();
		var mStore = this.getStore('importReconcileTransportTypeList');
	...
	onSave: function(){
		...
		var statusStore = me.getStore('importReconcileStatusList');
		var masterItem = Ext.create('MOST.model.billing.ImportReconcile');
		var statusItem = Ext.create('MOST.model.billing.ImportReconcile');
...
//		To perform the save logic only when modified
		if(masterItem.dirty||arrItems.length>0){
			...
			updateParm.getProxy().url = MOST.config.Locale.getRestApiDestUrl() + '/v1/importreconcile/list';
...
//		To perform the save logic only when modified
		if(masterItem.dirty||arrItems.length>0 || statusItems.length>0){
			...
			masterItem.set('scn', theVsl.get('scn'));
			masterItem.set("vslCallId", theSearch.data.vslCallId);
...
//		To perform the save logic only when modified
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		var me = this;
		var refs = me.getReferences();
		var grid = refs.refimportReconcileGrid;
		var selection = grid.getSelection() == null ? null : grid.getSelection()[0];
	...
	onBillingImportReconcileExport:function(){
		var me = this;
		var refs = me.getReferences();
		
		me.getView().detailViewAlias = 'popup-reportbillingimportreconcile';
		var record = '';
		var title = 'Billing Import Reconcile Report';
	...
	onExportExcelPdfWithServer : function(gridNameString,isExcel) {
		...
		searchBizParm.classID = 'com.tsb.most.biz.parm.billing.SearchImportReconcileParm';
		searchBizParm.serviceID = 'MOST.importReconcile.selectImportReconcile';
