		...
		for(var i=0; i<tariffConditionList.length; i++){
			...
			if(record.prptCd.indexOf('O') > -1){
				if(record.prptCd == 'O1'){
					...
					theDetail.set('chargePerWb', record.chrVal);
			...
			if(record.prptCd.indexOf('C') > -1){
				...
				} else if (record.prptCd == 'C18') {
					//s-BILL-011 Tariff Code – Add Transshipment Approve checkbox
					theDetail.set('transhipmentApprove', record.chrVal == 'Y' ? true : false);
					refs.chkTranshipmentApprove.checked = record.chrVal == 'Y' ? true : false;
					//e-BILL-011 Tariff Code – Add Transshipment Approve checkbox
				}
			}
	...
	setTariffConditionList:function(tariffCodeItem){
		...
		for (var i = 1; i < 19; i++) {
			if(i == 1){
				...
				var tariffVslOprConditionItem = tariffConditionItem.copy();
				var tariffCargoConditionPrptItem = tariffConditionPrptItem.copy();
				var transApproveChkVal = theDetail.get('transhipmentApprove') == 'true' ? 'Y' : 'N';

				tariffVslOprConditionItem.set('seq', seq);
				tariffVslOprConditionItem.set('prptCd', 'C18');
				tariffVslOprConditionItem.set('oprIdtCd', 'EQ');
				tariffVslOprConditionItem.set('chrVal', transApproveChkVal);

				tariffConditionList.push(tariffVslOprConditionItem.getData());

				tariffCargoConditionPrptItem.set('prcTpCd', 'S');
				tariffCargoConditionPrptItem.set('prptCd', 'C18');
				tariffCargoConditionPrptItem.set('prptNm', 'Transhipment Approve');
				tariffCargoConditionPrptItem.set('dataTpCd', 'YN');
				tariffCargoConditionPrptItem.set('dataLen', '1');
		...
		for (var i = 1; i < 9; i++) {
			if(i == 1){
				...
				tariffVslOprConditionItem.set('oprIdtCd', 'EQ');
				tariffVslOprConditionItem.set('chrVal', refs.cboChargePerWB.getValue());

				tariffConditionList.push(tariffVslOprConditionItem.getData());

				tariffOtherConditionPrptItem.set('prcTpCd', 'S');
				tariffOtherConditionPrptItem.set('prptCd', 'O5');
				tariffOtherConditionPrptItem.set('prptNm', 'Charge per WB Transaction');
				tariffOtherConditionPrptItem.set('dataTpCd', 'CHAR');
				tariffOtherConditionPrptItem.set('dataLen', '1');
