Ext.define('MOST.view.popup.ShippingNoteListPopupController', {
	extend: 'MOST.view.foundation.BaseViewController',

	requires: [],

	alias: 'controller.shippingnotelistpopup',

	/**
	 * =========================================================================================================================
	 * CONSTANT START
	 */
	MAIN_GRID_REF_NAME: 'refShippingNotePopupGrid', // Main Grid Name
	...
	getSearchCondition: function () {
		...
		if (!scn && !ptnrCd) {
			MessageUtil.info('info', 'shipNotePopup_missing_mandatory_msg');
			return;
		}
	...
	onDblClick: function () {
		...
		var returnItem = {
			code: selection.get('shipgNoteNo'),
			codeName: selection.get('shipgNoteNo'),
			item: selection,
		};
	...
	afterSetCodePopupData: function (xtype, targetControl, returnValue, me, parent) {
		...
		me.updateBookingNoCombo(returnValue);
	},

	updateBookingNoCombo: function (returnValue) {
		var me = this;
		var bookingNoStore = me.getStore(me.COMBO_BOX_STORE);
		bookingNoStore.load({
			params: {
