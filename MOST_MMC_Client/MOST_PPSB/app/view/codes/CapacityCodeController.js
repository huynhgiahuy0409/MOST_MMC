Ext.define('MOST.view.codes.CapacityCodeController', {
	...
	onDetailSave:function(){
		...
		var capacityValidationCodeStore = me.getStore('capacityValidationCode');
		...
		if(detailView){
			...
			if(!detailItem.phantom) {
				...
				capacityValidationCodeStore.load({
					params : {
						tyCd : 'capacityCode',
						col1 : detailItem.get('capaCd')
					},
					callback: function(records, operation, success) {
						if (success) {
							if(records != null && records.length > 0){
								if(records[0].get('isValidated') === 'Y'){
									...
									MessageUtil.warning('warning_msg', 'capacityDuplicatedCodeMsg');
									return;
								}
							}
						}
