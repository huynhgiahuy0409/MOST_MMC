Ext.define('MOST.view.usercontrol.TruckFieldController', {
	...
	openCodePopup:function(){
		...
		}else if(me.getView().params['searchDivCd'] == 'EQ-YT'){
			me.getView().params.title = 'Equipment Code Popup';
			me.getView().params['isYardTruckChecked'] = true;
			me.getView().params['isYardTruckDisabled'] = true;
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue, me, parent){
		...
		if(returnValue){
			...
			if(returnValue.item && returnValue.item.data.eqFacNo){
				fieldControl.setValue(returnValue.item.data.eqFacNo)
			}
		} else {
