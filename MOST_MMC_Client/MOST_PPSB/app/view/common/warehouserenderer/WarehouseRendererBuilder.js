Ext.define('MOST.view.common.warehouserenderer.WarehouseRendererBuilder', {
	...
	loadBayRowInformation : function(){
		...
		if(me.searchCondition && me.searchCondition.$className === 'MOST.model.operation.CargoHandlingOut'){
			opMode = true;
				...
				&& me.searchCondition.data.cgTpCd === 'RCV'){
			opMode = true;
		}
	...
	onClick : function(event, callback){
		...
		if(me.selectable && selectedItem && me.selectMode === me.SM_RENT)
			if(selectedItem.get('refNo') != null && selectedItem.get('refNo') != ''
				&& selectedItem.get('rentalExpiredCheck') != 'Expired'
				&& Ext.WindowManager.getActive().config.reference == 'popupAliaspopup-warehouserentalpopup'){
				MessageUtil.error('warning_msg','The selected location already rented, please choose others');
				return;
			}
