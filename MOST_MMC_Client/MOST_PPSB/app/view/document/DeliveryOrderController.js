...
//		params.searchType = 'detail';
		if(recvData.get('cgtpcd') == CodeConstants.MT_CGTP_RCV){
			...
			var dOperationMode = refs.ctlDirectModeOper.getValue()
			var iOperationMode = refs.ctlIndirectModeOper.getValue()
			if(dOperationMode == 'LR'){
				refs.refsDirectAmount.disabled(true)
			}else{
				refs.refsDirectAmount.disabled(false)
			}
			if(iOperationMode = 'LR'){
				refs.refsIndirectAmount.disabled(true)
			}else{
				refs.refsIndirectAmount.disabled(false)
			}
		}else{
...
//				}
	...
	updateRefsState: function(mode, tstp, cgTpCd){
		...
		standardAmtRefs = mode === 'D' ? dMtRefs : iMtRefs,
		isBKK = cgTpCd === CodeConstants.MT_CGTP_BBK
		;
		enableMap = tstpCombos.map(item => {
			...
			var disabledAmtRefs = []
			if(tstpCd == 'CV'){
				enabledAmtRefs = mode ==='D' ?  ['ctlDoConveyorDmt'] : ['ctlDoConveyorDmt']
			}else if(tstpCd == 'LC'){
				enabledAmtRefs = mode ==='D' ? ['ctlDoConveyorDmt', 'ctlDoLorryDmt'] : ['ctlDoConveyorImt', 'ctlDoLorryImt']
			}else if(tstpCd == 'LR'){
				enabledAmtRefs = mode ==='D' ? ['ctlDoLorryDmt'] : ['ctlDoLorryImt']
			}else if(tstpCd == 'PL'){
				enabledAmtRefs = mode ==='D' ? [] : []
			}else if(tstpCd == 'SE'){
				enabledAmtRefs = mode ==='D' ? ['ctlDoVesselDmt'] : []
			}else if(tstpCd == 'WG'){
				enabledAmtRefs = mode ==='D' ? ['ctlDoWagonDmt'] : ['ctlDoWagonImt']
			}else if(tstpCd == 'WC'){
				enabledAmtRefs = mode ==='D' ? ['ctlDoWagonDmt', 'ctlDoConveyorDmt'] : ['ctlDoWagonImt', 'ctlDoConveyorImt']
			}else if(tstpCd == 'WP'){
				enabledAmtRefs = mode ==='D' ? ['ctlDoWagonDmt'] : ['ctlDoWagonImt']
			}
			disabledAmtRefs = standardAmtRefs.filter(item => !enabledAmtRefs.includes(item))
			return {
				tstpCd,
				enabledAmtRefs,
				disabledAmtRefs
		...
		enabledAmtRefs = enableMap.find(item => item.tstpCd == tstp).enabledAmtRefs
		disabledAmtRefs = enableMap.find(item => item.tstpCd == tstp).disabledAmtRefs
		if(enabledAmtRefs.length <= 1){
			if(mode === 'D'){
				disabledAmtRefs = ['refsDirectAmount']
				enabledAmtRefs = ['ctlDDmt', 'ctlDM3', 'ctlDQty']
				dMtRefs.forEach(item => {
					refs[item].setValue(0)
				})
			}else{
				disabledAmtRefs = ['refsIndirectAmount']
				enabledAmtRefs = ['ctlImt', 'ctlIm3', 'ctlIQty']
			...
			if(isBKK && tstp == 'LR'){
				if(mode === 'D'){
					enabledAmtRefs = ['ctlDQty'] 
					disabledAmtRefs.push(...['ctlDDmt', 'ctlDM3'])
				}else{
					enabledAmtRefs = ['ctlIQty'] 
					disabledAmtRefs.push(...['ctlImt', 'ctlIm3'])
				}
			}
			...
			if(mode === 'D'){
				...
				enabledAmtRefs.push(...['ctlDM3', 'ctlDQty', 'refsDirectAmount'])
				disabledAmtRefs.push(...['ctlDDmt'])
			}else{
				disabledAmtRefs.forEach(item => {
					refs[item].setValue(0)
				})
				enabledAmtRefs.push(...['ctlIm3', 'ctlIQty', 'refsIndirectAmount'])
				disabledAmtRefs.push(...['ctlImt'])
			}
		}
	...
	autoCalcAmount: function(mode, tsptTpCd){
		...
		theMain = me.getViewModel().get('theMain'),
		isBBK = theMain.get('cgtpcd') === CodeConstants.MT_CGTP_BBK,
		{wgt, vol, pkgqty, eachWgt, eachVol} = theMain.data,
		wgtRef,
		volRef,
		qtyRef
		;
