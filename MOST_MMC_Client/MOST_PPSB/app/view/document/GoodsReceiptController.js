Ext.define('MOST.view.document.GoodsReceiptController', {
	...
	onSearch:function() {
    	...
    	var recvData = me.getView().recvData;
    	var ctlJpvc = refs.ctlJpvc.getValue();
    	var ctlScn = refs.ctlScn.getValue();
    	
    	if((ctlScn == null || ctlScn == '') && (ctlJpvc == null || ctlJpvc == '')){
    		MessageUtil.warning("goodsReceipt", "goodsreceipt_search_null_msg");
    		return;
    	}
	...
	saveProcess:function(){
		...
		if(detailItem.get('cgTpCd') != 'DBN'){
			...
			if(detailItem.get('delvTpCd') == 'I' && refs.ctlReturnToShipperFromDetail.checked == false){
				//Check sum MT, M3, QTY existed in GR store
				if(store.data.length > 0){
					store.each(function (rec) { if(detailItem.get('gdsRecvNo') != rec.get('gdsRecvNo')){   	
						sumMt += Number(rec.get('grWgt')); 
			...
			if(detailItem.get('delvTpCd') == 'I' && refs.ctlReturnToShipperFromDetail.checked == true){
...
//		}
	...
	onAdd: function(){
		...
		if(sn == null || sn == ""){
			...
			MessageUtil.warning("goodsReceipt", "selectvesselid_msg");
		}else{
...
//		me.openDetailPopup(params, 'Download Report');
	...
	onSubmit:function(){
		...
		if(refs.ctlReturnToShipperFromDetail.checked == true){
	...
	setDetailControl:function(recvData){
		...
		if (recvData.data.rhdlMode === 'R') {
			refs.ctlReturnToShipperFromDetail.setRawValue(true);
			refs.refPkgQty.setReadOnly(true);
			refs.refM3.setReadOnly(true);
			refs.refWeight.setReadOnly(true);
		}
		
		if (refs.ctlReturnToShipper.getRawValue(true)) {
			refs.ctlReturnToShipperFromDetail.setValue(true);
		}
...
//		me.setComboBoxWithLocalCache(CacheServiceConstants.WAGON_MODE_COMBOBOX, me.WAGON_MODE_STORE);
		...
		if(recvData.get(WorkingStatus.FIELD_NAME) == WorkingStatus.INSERT){
				...
				callback: function(records, operation, success) {
					if (success) {
						if (records.length > 0) {
							...
							theMainModel.data.scn =  theVessel.data.scn;
			...
			var goodsReceiptDetail = me.getStore(me.GR_DETAIL_STORE);
			refs.ctlReturnToShipperFromDetail.setDisabled(true);
			goodsReceiptDetail.load({
				params: {
...
//								refs.refCboModeOfTransport.setDisabled(false);
	...
	onBookingNoChange: function(ctl){
		var me = this;
		var refs = me.getReferences();
		var snComboStore = me.getStore(me.SN_COMBO_STORE);
		
		if(ctl.reference === 'ctlBookingNoCombo'){
			...
			} else if(!StringUtil.isNullorEmpty(refs.ctlScn.getValue())){
				params = {
					scn: refs.ctlScn.getValue(),
					mfDocNo: mfDocId
				};
			...
			snComboStore.load({
				...
				callback: function(records, operation, success) {
					if(success) {
						if (records.length == 1) {
							if(!StringUtil.isNullorEmpty(records[0].get('shipgNoteNo'))){
								refs.ctlSNN.setValue(records[0].get('shipgNoteNo'));
							}
	                    }
					}
				}
