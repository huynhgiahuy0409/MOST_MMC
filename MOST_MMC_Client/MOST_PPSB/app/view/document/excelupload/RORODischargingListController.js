Ext.define('MOST.view.document.excelupload.RORODischargingListController', {
    extend: 'MOST.view.foundation.BaseViewController',
    alias: 'controller.rorodischaginglist',
	...
	onValidateEdit:	function(editor, context) {
		if(context.record.phantom == true) {
			var rORODischargingListDuplicateCheck = Ext.getStore('rORODischargingListDuplicateCheck');
			    
			rORODischargingListDuplicateCheck.load({
				params: {
	...
	onDischargingListFileUpload : function() {
    	...
    	var input = document.querySelector("input[id='rORODischargingListFileUpload-button-fileInputEl']");
	...
	nullCheckColumnList : ['opeClassCd', 'mfDocId', 'blNo',
		'chassisNo', 'consignee', 'shipper',
		'cargoAgent', 'cargoSubType', 'commodity',
		'packageType', 'vehicleBrand', 'vehicleModel',
		'loadPort', 'dischargePort', 'cargoDesc',
		'deliveryMode', 'hatchNo', 'modeofOp', 'newUsed'],
	...
	onCheckExcelItself: function(grid, storeData, viewModel) {
		...
		for( index1 in storeData ) {
			...
			for( index2 in storeData ) {
				...
				if ((record2.get('opeClassCd') != '' && record2.get('opeClassCd') != null) && record2.get('opeClassCd') != 'I') {
			...
			callback: function(record, operation, success) {
				...
				if(responseObj.errorNumber == 412) {
					if(responseObj.errorDescription === "notExistVslCallId") {
						...
						for( index in responseObj.data ) {
							...
							store.each(function(storeRecord) {
								if(storeRecord.get('vslCallId') === row.vslCallId) {
									var targetGridCol = grid.getColumns().find(col => col.dataIndex == 'vslCallId' );
									var targetHtmlCell = grid.view.getCell(storeRecord, targetGridCol); // var targets = Ext.fly(targetHtmlCell); 
						...
						MessageUtil.warning('warning_msg', 'cargoDischargingListBrandModelWrnPopup');
						...
						for( index in responseObj.data ) {
							...
							for( colName of ['vehicleBrand', 'vehicleModel'] ) {
								var targetGridCol = grid.getColumns().find(col => col.dataIndex == colName );
								var targetHtmlCell = grid.view.getCell(targetRecord, targetGridCol); // var targets = Ext.fly(targetHtmlCell); 
								var qtipMsg = ViewUtil.getLabel('cargoDischargingListBrandModelWrnMsg');
