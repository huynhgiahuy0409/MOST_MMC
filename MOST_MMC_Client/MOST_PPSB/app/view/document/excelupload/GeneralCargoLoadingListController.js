Ext.define('MOST.view.document.excelupload.GeneralCargoLoadingListController', {
    extend: 'MOST.view.foundation.BaseViewController',
    alias: 'controller.generalcargoloadinglist',
	 ...
	 * CONSTANT START
	 */
   	MAIN_GRID_REF_NAME: 'refGeneralCargoLoadingListGrid',
   	MAIN_STORE_NAME: 'generalcargoloadinglist',
	...
	onValidateEdit:	function(editor, context) {
		if(context.record.phantom == true) {
			var generalCargoLoadingListDuplicateCheck = Ext.getStore('generalCargoLoadingListDuplicateCheck');
			    
			generalCargoLoadingListDuplicateCheck.load({
				params: {
	...
	onLoadingListFileUpload : function() {
    	...
    	var input = document.querySelector("input[id='loadingListFileUpload-button-fileInputEl']");
	...
	nullCheckColumnList : ['opeClassCd', 'mfDocId', 'shipgNoteNo', 'consignee', 
		'shipper', 'transporter', 'cargoType', 
		'cargoSubType', 'commodity', 'packageType',
		'loadPort', 'dischargePort', 'cargoDesc',
		'deliveryMode', 'modeofOp', 'estimateArrivalDate'],

	numberCheckColumnList : ['quantity', 'totalWeight', 
		'totalVolumn',],
	...
	onCheckExcelItself: function(grid, storeData, viewModel) {
		...
		for( index1 in storeData ) {
			...
			if ((record1.get('estimateArrivalDate') != '' && record1.get('estimateArrivalDate') != null) 
				...
				targetHtmlCell.setAttribute('data-qtip', 'adsadasdsad');
				targetHtmlCell.bgColor = 'yellow'; 
			...
			for( index2 in storeData ) {
				...
				if( record1.get('id') == record2.get('id') ){
					...
					if(record1.get('vslCallId') === record2.get('vslCallId') && record1.get('shipgNoteNo') === record2.get('shipgNoteNo')) {
						...
						targetHtmlCell.setAttribute('data-qtip', 'Duplicated for SN No in Excel.');
						targetHtmlCell.bgColor = 'yellow'; 
