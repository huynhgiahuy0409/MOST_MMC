Ext.define('MOST.view.document.BLController', {
	...
	onDetailLoad: function(){
			...
			pkgDetailList.load({
				...
				callback: function(records, operation, success) {
					if(success){
						...
						// }
						if(theBL.get('projectCargo') === 'Y'){
							refs.refEachWeight.setValue(Number(recvBl.get('eachWgt')));
							refs.refEachVolume.setValue(Number(recvBl.get('eachVol')));
						}
					}
				}
			});
		...
		if(theBL.data.crud == 'C'){
			...
			if(MOST.config.Token.getUserType() == CodeConstants.USER_TYPE_INTERNAL){
				...
				if(theBL.get('docStatCd') == CodeConstants.MT_IVSTAT_ST){
					...
					refs.ctlCancelApprove.setHidden(false);
				}
			}
			...
			me.setPackageDetailSettingControl(theBL.get('cmdtGrpCd'));
		}
		var isBBK = theBL.get('cgTpCd') == CodeConstants.MT_CGTP_BBK;
		var isPkgCargo = theBL.get('cmdtGrpCd') === me.PACKING_LIST || theBL.get('cmdtGrpCd') === me.MEASUREMENT_CARGO;

		//refs.refProjectCargo.setDisabled(!(isBBK && isPkgCargo));
		...
		refs.refTsAprovalYNChk.checked = theBL.get('tsAprovalYN') == 'Y' ? true : false;
		...
		//Checkbox Transshipment Approve is enabled 
		// When Category is Transshipment only
		if(theBL.get('opClassCd') == 'T'){
			refs.refTsAprovalYNChk.setDisabled(false);

			// Users cannot make any changes to the checkbox when 
			// Bill of Lading has Approved status or Operation is started
			if(theBL.get('docStatCd') == 'AP' || theBL.get('isOperated') == 'Y'){
		...
		if(theBL.get('docStatCd') == 'AP' && theBL.data.crud == 'U'){
			refs.refCategory.on('beforequery', function() {
				return false;
			});
		}
	},
	...
	*/
	checkPackageCargo: function () {
		...
		try {
			isBBK = theBL.get('cgTpCd') == CodeConstants.MT_CGTP_BBK;
			isPkgCargo = theBL.get('cmdtGrpCd') === me.PACKING_LIST || theBL.get('cmdtGrpCd') === me.MEASUREMENT_CARGO;
			
			return (isBBK && isPkgCargo);

		  } catch (err) {
		  		console.log ('BLController.Funciton error: checkPackageCargo')
				console.log (err);			
		  }
	},

	/**
	 * Set Package Detail Tab Setting Control
	*/
	setPackageDetailSettingControl: function (cmdtGroupCd) {
		var me = this;
		var refs = me.getReferences();
		...
		var isPkgCargo = me.checkPackageCargo()
		refs.refProjectCargo.setValue(isPkgCargo);

		refs.refTotalQty.setEditable(true);
		refs.refTotalMt.setEditable(true);
	   					...
	   					callback: function(records, operation, success){
	   						if(success){
	   							if(refs.ctlPartner.getValue() != records.at(0).data.arrvSaId){
	   								...
	   								blItem.set("scn", records.at(0).data.scn);
...
//				}
		...
		if(theBL.data.crud == 'C'){
			...
			if((theBL.get('docStatCd') == 'CA') && (theBL.get('docStatNm') == 'Canceled')){
                theBL.set('docStatCd', 'ST');
			}
...
//			refs.ctlModeofOperation.setValue('PL');
	...
	onLWHChange: function (ctl) {
		...
		thePgkDetail.set('eachPkgVol', m3);
	...
	/**
	 * PPSB_20241108. Re-calculate the Unit CgWgt of package detail
	*/
	onChangePkgDetailQty: function (ctlField, newQty, oldQty, eOpts ) {
		var me = this;
		var refs = me.getReferences();
		var thePgkDetail = me.getViewModel().get('thePgkDetail');
		...
		var checkValue = refs.refMsrmt.getValue();

		//If eachPkgVol is not null, calculate total volume
		if(eachPkgVol != '' && eachPkgVol > 0){
			totalVol = Number(eachPkgVol * newQty).toFixed(3)
			//refs.refTotalM3.setValue(totalVol);
			thePgkDetail.set('pkgTotalVol', totalVol);
		}
	...
	*/
	...
	/**
	 * PPSB_20241108.Re-calculate the Unit TotalVol of package detail
	*/
	...
	onStrgCheck: function(){
		...
		var refs = me.getReferences();
		var theBL = me.getViewModel().get('theBL');
		var dispatchType = refs.refCategory.getValue();
		var detailView = me.getDetailBizView();
		...
		*/
		if(dispatchType == 'T'){
			refs.refTsAprovalYNChk.setDisabled(false);
			if(recvData.get('tsAprovalYN') === 'Y'){
				refs.refTsAprovalYNChk.setValue(true);
				recvData.set('tsAprovalYN', 'Y');
			}else{
...
//     	refs.refWeightChk.setDisabled(true);
        ...
        if(theBL.data.crud == 'C'){				
     	...
     	// 	refs.refEachVolume.setValue(0);
     	// 	refs.refTxtNoOfPackage.setValue(0);
     	// } Removed. From OPR-013. B2 – Ver0.2
...
//		}
		...
		var isBBK = refs.ctlTypeofCargo.getValue() == CodeConstants.MT_CGTP_BBK;
		var isPkgCargo = refs.ctlCommodityGroup.getValue() === me.PACKING_LIST || refs.ctlCommodityGroup.getValue() === me.MEASUREMENT_CARGO;
	...
	onAddPackage: function(btn){
		...
		me.updateRecord(record, thePgkDetail, listFields2, false);
		
		if(me.validateAddUpdatePackage(btn) == false){
			return;
		}
     	record.set('userId', Token.getUserId());
     	record.set('workingStatus', WorkingStatus.INSERT);
		...
		if(me.onPkgDuplicatedValidation(record.get('pkgNo'))){
			store.insert(0, record);
	...
	/**
	 * Validate package field add or update function
	*/
	validateAddUpdatePackage: function (btn) {
		var me = this;
		var thePgkDetail = me.getViewModel().get('thePgkDetail');
		var theBL = me.getViewModel().get('theBL');
		var cmdtGroupCd = theBL.get('cmdtGrpCd');
		var eachPkgVol = thePgkDetail.get('eachPkgVol');
		...
		if (StringUtil.isNullorEmpty(thePgkDetail.get('blNo'))) {
			if (StringUtil.isNullorEmpty(strValidation)) {
				strValidation += ViewUtil.getLabel('blNo');
			} else {
				strValidation = strValidation + ", " + ViewUtil.getLabel('blNo');
	...
	onUpdatePackage: function(btn){
		...
		var currPkgNo;
		var strValidation = "";
		var checkDoStore = me.getStore(me.CHECK_DO_STORE)
		var checkBlOperation = me.getStore('getDataBLOperation')
		var detailItem = me.getViewModel().get('theBL');
     	if(selection == null)
			return;
		
		currPkgNo = selection.get('pkgNo');
		// if(!selection.phantom){
		// 	//MessageUtil.warning("warning_msg", "Cannot update Package No. of an record which is not a new one.");
		// 	//return;
		// } 
		
		if(!me.validateAddUpdatePackage(btn)){
			return;
		} else if(currPkgNo != thePgkDetail.get('pkgNo')) {
			if(!me.onPkgDuplicatedValidation(thePgkDetail.get('pkgNo'))){
				return;
			}
		}
		...
		checkDoStore.load({
			...
			callback: function(DOrecords, operation, success) {
				if(success){
					if(DOrecords && DOrecords.length > 0){	
						MessageUtil.warning("warning_msg", "bl_validate_update_do_exist");
						return;
					}else{
						checkBlOperation.load({
							params:{
								blNo: detailItem.get('blNo'),
								vslCallId: detailItem.get('vslCallId')
							},
							callback: function(records, operation, success) {
								if(success){
									if(records && records.length > 0){	
										MessageUtil.warning("warning_msg", "operationExist_update_msg");
										return;
									}else{
	...
	*/
	onPkgGridItemClick:function(){
		...
		// Suspend events on the BLPackageDetail component
		...
		// Resume events on the BLPackageDetail component
		setTimeout(function() {
	...
	onOpenPackageDetail: function () {
		...
		selection.set("docType", "BL");

		var title = {type: 'bundle', key: 'title_bl_package_detail'};
...
//		}
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue, me, parent){
					...
					if (success) {
						if (records.length > 0 
							...
							if(targetControl == 'ctlCommodityGroup'){
								...
								//refs.refPackageDetailTab.setDisabled(!(isBBK && isPkgCargo));
								me.setPackageDetailSettingControl(theBL.get('cmdtGrpCd'));
		...
		}else if(targetControl == 'ctlCommodityGroup') {
			me.setPackageDetailSettingControl(theBL.get('cmdtGrpCd'));
		}
	},
	...
	*/
	onOpenDG: function () {
		...
		var theBLDtl = me.getViewModel().get('theBL');

		if(theBLDtl == null) 
			return;
		var cnsne = theBLDtl.get('cnsne');
		var shpr = theBLDtl.get('shpr');
		
		if(refs.ctlUnno.getValue() == '' || refs.ctlUnno.getValue() == null || refs.txtImdgNo.getValue() == '' || refs.txtImdgNo.getValue() == null){
			MessageUtil.error('warning_msg','shippingNoteInquiry_unno_empty');
			return;
		}

		if(StringUtil.isNullorEmpty(cnsne) || StringUtil.isNullorEmpty(shpr)){
		...
		var detailVslItem = me.getViewModel().get('theJpvc');
		
		theDGModel.set("vslCallId",theBLDtl.get("vslCallId"));
		theDGModel.set("scn",theBLDtl.get("scn"));
		theDGModel.set("vslCd",theBLDtl.get("vslCd"));
		theDGModel.set("callYear",theBLDtl.get("callYear"));
		theDGModel.set("callSeq",theBLDtl.get("callSeq"));
		theDGModel.set("vslNm",theBLDtl.get("vslNm"));
		theDGModel.set("arrvSaId",theBLDtl.get("arrvSaId"));
		theDGModel.set("berthLoc",theBLDtl.get("berthLoc"));
		theDGModel.set("voyage",theBLDtl.get("voyage"));
		theDGModel.set("eta",theBLDtl.get("eta"));
		theDGModel.set("portCd",theBLDtl.get("portCd"));
		theDGModel.set("portCdNext",theBLDtl.get("portCdNext"));
		theDGModel.set("freeZoneDiv",'FZN');
		theDGModel.set("arrvSaId",theBLDtl.data.arrvSaId);
		theDGModel.set("impNm",theBLDtl.data.cnsneNm);
		theDGModel.set("impAddr",theBLDtl.data.cnsneAddr);
		theDGModel.set("expNm",theBLDtl.data.shprNm);
		theDGModel.set("expAddr",theBLDtl.data.shprAddr);
		theDGModel.set("unno",theBLDtl.data.unno);
		theDGModel.set("imdg",theBLDtl.data.imdgClass);
		theDGModel.set("cgNo",theBLDtl.data.blNo);
		theDGModel.set("pkgQty",theBLDtl.data.pkgQty);
		theDGModel.set("wgt",theBLDtl.data.wgt);
		theDGModel.set("catgCd","Import");
		theDGModel.set("ixCd","I");
		theDGModel.set("dgSeq", theBLDtl.data.dgSeq);
		theDGModel.set("seq", theBLDtl.data.dgSeq);
	...
	getSearchCondition:function(){
		...
		if(MOST.config.Token.getUserType() == 'E'){
			...
			var isSHA = me.existsPatnerType(CodeConstants.CM_PTNRTP_SHA);
			var isFWD = me.existsPatnerType(CodeConstants.CM_PTNRTP_FWD);
	...
	/**
	 * 20241104. Add upload DG file process
	 */
    ...
    saveProcess: function(){
		...
		if(detailItem.get("crud") == 'C'){
            ...
            if(detailItem.get('blNo') != detailItem.get('oldBlNo')){
				...
				updateParm.save({
					success : function(records,success){
						if((updateParm.get('docStatNm') == 'Approved') && (updateParm.get('docStatCd') == CodeConstants.MT_DOCSTAT_AP)){
							deliverOrderStore.load({
								params:{
									blno: detailItem.get('blNo'),
									vslCallId: detailItem.get('vslCallId')
								...
								callback: function(DOrecords, operation, success) {
									if(success){
										if((DOrecords[0].get('dono') != null && DOrecords[0].get('dono') != '') && DOrecords[0].get('delvTpCd') == 'I'){	
										...
										} else if((DOrecords[0].get('dono') != null && DOrecords[0].get('dono') != '') && DOrecords[0].get('delvTpCd') == 'B'){
											me.processFileInfoUpload(isCreated);
											store.commitChanges();
											me.openCodePopup('popup-doamountidentificationpopup', 'doamountidentificationpopup');
										} else {
											MessageUtil.show(Ext.Msg.INFO, 'success_msg', 'savesuccess_msg', '', function(button){
												if (button === 'ok') {
													me.processFileInfoUpload(isCreated);
...
//    },
	...
	onUpperCaseModified: function(control){
		...
		if(!me.isClickFromRow){
			refs.refSubBLNo.setValue(newString.toUpperCase());
		}
		
		me.isClickFromRow = false;
		/*if(me.mblEditable){
	...
	*/
	...
	saveDGProcess:function(retunDgItem){
		...
		me.getView().detailViewAlias = 'app-bldetail';
		if (dgView) {			
			dgView.close();
		}
	},
	...
	*/
	processFileInfoUpload: function (isCreated) {
		var me = this;
		var refs = me.getReferences();
		var store = me.getStore('blFileUploadInfo');
		var fileUploadStore = me.getStore('blFileUploadInfo');
		var detailItem = me.getViewModel().get('theBL');
		var uploadItems = new Array();

		var updateParm = Ext.create('MOST.model.foundation.parm.UpdateBizParm');
		var proxy = detailItem.getProxy();
		proxy.url = fileUploadStore.getProxy().url;
		
		/**
		 * BL Upload file information
		 */
			// File Upload CREATE, UPDATE RECORD
		fileUploadStore.getModifiedRecords().forEach(function (modifiedRecord, index, array) {
			...
			modifiedRecord.set('workingStatus', WorkingStatus.INSERT);
			if (isCreated) {
				modifiedRecord.set('catgCd', modifiedRecord.get('catgCd') + detailItem.get('blNo'));
			}
		...
		detailItem.set('uploadItems', uploadItems);
		updateParm.getProxy().url = fileUploadStore.getProxy().url;
		updateParm.phantom = isCreated;
		...
		deliverOrderStore.load({
			...
			callback: function(DOrecords, operation, success) {
				if(success){
					if(DOrecords.length > 0 && DOrecords[0].data.tsptr != null && DOrecords[0].data.tsptr != ''){
						refs.ctlSplit.setHidden(true);
						MessageUtil.warning('warning_msg','cannotCancelTheBL');
					} else {
						theBL.set('docStatCd', 'CA');
	...
	onCheckBlNo:function(){
		var me = this;
		var refs = me.getReferences();
		var theBL = me.getViewModel().get('theBL');
		var checkBLNo = me.getStore('checkBLNo');
		
		if(!StringUtil.isNullorEmpty(refs.refSubBLNo.getValue()) 
				&& (refs.refSubBLNo.getValue() != theBL.get('oldBlNo'))
				&& (refs.refSubBLNo.getValue() != theBL.get('oldMfDocId'))){
			checkBLNo.load({
				params:{
					vslCallId: theBL.get('vslCallId'),
					mfDocId: theBL.get('mfDocId'),
				...
				callback:function(records, operation, success){
					if(success){
						var theBL = me.getViewModel().get('theBL');
						if(Ext.WindowManager.getActive() != null && Ext.WindowManager.getActive().title != 'Success.'){
							if(records.length == 0){
								if(theBL.data.crud == 'C'){
									me.mblEditable = true;
								}
