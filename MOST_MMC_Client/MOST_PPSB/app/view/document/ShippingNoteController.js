Ext.define('MOST.view.document.ShippingNoteController', {
	...
	onDetailLoad:function(){
		...
		me.getViewModel().set({thePgkDetail: Ext.create('MOST.model.document.ShippingNote')});
		...
		//Checkbox is enabled when Category is Transshipment only
		if(recvData.get('catgCd') == 'T'){
			refs.refTsAprovalYNChk.setDisabled(false);

			// Users cannot make any changes to the checkbox when 
			// Shipping Note already had Confirm Loading status or Operation is started
			if(recvData.get('statCd') == 'CF' || recvData.get('isOperated') == 'Y'){
				refs.refTsAprovalYNChk.getEl().on('click', function(e) {
					e.stopEvent(); 
				});			
			}
		}

		// Category Combobox is not editable 
		// when Status is Confirmed
		if(recvData.get('statCd') == 'CF' && searchType == 'update'){
			refs.ctlCategory.on('beforequery', function() {
				return false;
			});
		}
	},
	...
	setDetailInitialize:function(){
		...
		if(me.SCREEN_MODE === CONSTANTS.SCREEN_MODE_CREATE ){ // CREATE
			...
			theShippingNoteItem.set("wgtChk", "N");
						...
						searchDtlShipNote.load({
							...
							callback:function(records, operation, success){
								if(success){
									...
									refs.refAnchorage.checked = theSN.get('anchorageChk') == 'Y' ? true : false;
									refs.refPrivateJetty.checked = theSN.get('privateJettyChk') == 'Y' ? true : false;
									refs.refTsAprovalYNChk.checked = theSN.get('tsAprovalYN') == 'Y' ? true : false;
									//refs.refAdditionalChk.checked = theSN.get('additionalChk') == 'Y' ? true : false;
									refs.ctlNewMfDocId.setReadOnly((theSN.get('statCd') == 'AP' && theSN.get('statCdNm') == 'Approved') || (theSN.get('statCd') == 'CF' && theSN.get('statCdNm') == 'Confirmed'));
	...
	setDetailControl:function(masterItem){
		var me = this;
		var refs = me.getReferences();
		var cmdtGroupCd = masterItem.cmdtGroupCd;
		var cgTpCd = masterItem.cgTpCd;
		var isPkgCargo = false;
	...
	/**
	 * PPSB. Open Package Detail Popup from SN List
	*/
	onOpenPackageDetail: function() {
		var me = this;
		var grid = me.lookupReference(me.MAIN_GRID_REF_NAME);
		var initData = me.getView().shipNoteInitData;
		var selection = grid.getSelection() === null ? null : grid.getSelection()[0];
		var detailView = 'app-docpackagedetail';
		
		if(!selection) {
			return;
		}

     	selection.set("docType", "SN");
		selection.commit();

		me.getView().detailViewAlias = detailView;
		me.openDetailPopup(selection);
	},
		...
		if(refs.ctlDeliveryMode.getValue() == 'D'){
			if(mtQtyMandatory.includes(refs.ctlTypeofCargo.getValue())){
				if(refs.refProjectCargo.getValue() == true){
					// var pkgQty = 0, pkgMt = 0, pkgM3 = 0;					
					
					// pkgDetailStore.each(function(record) {
					// 	if(record.get('workingStatus') != WorkingStatus.DELETE){
					// 		pkgQty += parseInt(record.get('pkgUnitQty'));
					// 		pkgMt +=  parseFloat(record.get('pkgTotalWgt'));
					// 		pkgM3 +=  parseFloat(record.get('pkgTotalVol'));
					// 	}
					// });
			        
					// refs.ctlLorryDmt.setValue((refs.ctlImt.getValue() == null || refs.ctlImt.getValue() == '') ? pkgMt.toFixed(3) : refs.ctlImt.getValue());
					// refs.ctlLorryDm3.setValue((refs.ctlIm3.getValue() == null || refs.ctlIm3.getValue() == '') ? pkgM3.toFixed(3) : refs.ctlIm3.getValue());
					// refs.ctlLorryDQty.setValue((refs.ctlIQty.getValue() == null || refs.ctlIQty.getValue() == '') ? pkgQty : refs.ctlIQty.getValue());
					
					// detailItem.set("imt", '');
					// detailItem.set("im3", '');
					// detailItem.set("iqty", '');
					
					// detailItem.set("dLrMt", (refs.ctlImt.getValue() == null || refs.ctlImt.getValue() == '') ? pkgMt.toFixed(3) : refs.ctlImt.getValue());
					// detailItem.set("dLrM3", (refs.ctlIm3.getValue() == null || refs.ctlIm3.getValue() == '') ? pkgM3.toFixed(3) : refs.ctlIm3.getValue());
					// detailItem.set("dLrQty", (refs.ctlIQty.getValue() == null || refs.ctlIQty.getValue() == '') ? pkgQty : refs.ctlIQty.getValue());
				} else {
					refs.ctlLorryDmt.setValue('');
...
//			refs.ctlModeofOperation.setDisabled(true);
			...
			if(mtQtyMandatory.includes(refs.ctlTypeofCargo.getValue())){
				if(refs.refProjectCargo.getValue() == true){
					...
					// refs.ctlImt.setValue((refs.ctlLorryDmt.getValue() == null || refs.ctlLorryDmt.getValue() == '') ? pkgMt.toFixed(3) : refs.ctlLorryDmt.getValue());
					// refs.ctlIm3.setValue((refs.ctlLorryDm3.getValue() == null || refs.ctlLorryDm3.getValue() == '') ? pkgM3.toFixed(3) : refs.ctlLorryDm3.getValue());
					// refs.ctlIQty.setValue((refs.ctlLorryDQty.getValue() == null || refs.ctlLorryDQty.getValue() == '') ? pkgQty : refs.ctlLorryDQty.getValue());
					
					// detailItem.set("dLrMt", 0);
					// detailItem.set("dLrM3", 0);
					// detailItem.set("dLrQty", 0);
					
					// detailItem.set("imt", (refs.ctlLorryDmt.getValue() == null || refs.ctlLorryDmt.getValue() == '') ? pkgMt.toFixed(3) : refs.ctlLorryDmt.getValue());
					// detailItem.set("im3", (refs.ctlLorryDm3.getValue() == null || refs.ctlLorryDm3.getValue() == '') ? pkgM3.toFixed(3): refs.ctlLorryDm3.getValue());
					// detailItem.set("iqty", (refs.ctlLorryDQty.getValue() == null || refs.ctlLorryDQty.getValue() == '') ? pkgQty : refs.ctlLorryDQty.getValue());
...
//		}			
		...
		if (field.reference === 'ctlTypeofCargo') {
			me.setPackageDetailSettingControl();
		}
...
//    			refs.ctlDeliveryMode.setValue(me.DELIVERY_MODE_INDIRECT);
	...
	onChangeLWH: function (ctl) {
		...
		if(cgLen * cgWth * cgHgt > 0){
			...
			refs.refMsrmt.setValue(Number(m3.toFixed(3)));
	...
	onOpenDG: function () {
		...
		if(refs.ctlUnno.getValue() == '' || refs.ctlUnno.getValue() == null || refs.ctlImdg.getValue() == '' || refs.ctlImdg.getValue() == null){
			MessageUtil.error('warning_msg','shippingNoteInquiry_unno_empty');
			return;
		}

		if(refs.ctlShipperCd.getValue() == '' || refs.ctlShipperCd.getValue() == null || refs.ctlConsigneeCd.getValue() == '' || refs.ctlConsigneeCd.getValue() == null){
			MessageUtil.error('warning_msg','shippingNoteInquiry_shp_cons_empty');
			return;
		}

		var title = {type: 'bundle', key: 'dgDeclTitle'};
		var detailItem = me.getViewModel().get('theShippingNote');
		var detailVslItem = me.getViewModel().get('theSearchDetail');
		var theDGModel = Ext.create('MOST.model.document.DGDeclaration');

		me.getView().detailViewAlias = 'app-dgdeclarationdoc';

		if(detailItem.get('dgItems') != null){
			theDGModel.data = detailItem.get('dgItems')[0];
		}
		
		if(detailItem.get('statCd') == me.STAT_APPROVED){
			MessageUtil.warning('warning_msg','shippingNoteApproved_block_dg');
			return;
		}
		...
		theDGModel.set("scn", detailVslItem.get("scn"));
		...
		if(detailItem.get("catgCd") != ''){
			var catgItem = categoryCombo.findRecord('scd', detailItem.get("catgCd"));
			theDGModel.set("catgCd",catgItem.get("scdNm"));
		}

		theDGModel.set("impNm",detailItem.get("cnsneNm"));

		var cnsneAddr2 = (detailItem.get("cnsneAddr2") == ''? '' : ', ' + detailItem.get("cnsneAddr2"));
		var cnsneAddr3 = (detailItem.get("cnsneAddr3") == ''? '' : ', ' + detailItem.get("cnsneAddr3"));
		var cnsneAddr4 = (detailItem.get("cnsneAddr4") == ''? '' : ', ' + detailItem.get("cnsneAddr4"));
		var shprAddr2 = (detailItem.get("shprAddr2") == ''? '' : ', ' + detailItem.get("shprAddr2"));
		var shprAddr3 = (detailItem.get("shprAddr3") == ''? '' : ', ' + detailItem.get("shprAddr3"));
		var shprAddr4 = (detailItem.get("shprAddr4") == ''? '' : ', ' + detailItem.get("shprAddr4"));

		theDGModel.set("impAddr",detailItem.get("cnsneAddr") + cnsneAddr2 + cnsneAddr3 + cnsneAddr4) ;
		theDGModel.set("expNm",detailItem.get("shprNm"));
		theDGModel.set("expAddr",detailItem.get("shprAddr") + shprAddr2 + shprAddr3 + shprAddr4) ;
		theDGModel.set("unno",detailItem.get("unno"));
		theDGModel.set("imdg",detailItem.get("imdg"));
		theDGModel.set("cgNo",detailItem.get("shipgNoteNo"));
		theDGModel.set("pkgQty",detailItem.get("cgWgt"));
		theDGModel.set("wgt",detailItem.get("wgt"));
		theDGModel.set("pkg",detailItem.get("pkgTpCd"));
		theDGModel.set("pkgtpcdnm",detailItem.get("pkgTpCdNm"));
		theDGModel.set("seq",detailItem.get("dgSeq"));
		theDGModel.set("dgSeq",detailItem.get("dgSeq"));
	...
	*/
	onTabChange : function(tabPanel, newCard, oldCard, eOpts) {
		...
		switch(tabPanel.getActiveTab().name){
			case 'packagedetail':
				...
				break;
			default:
				console.log(`Tab ${tabPanel.getActiveTab().name} is not handled`);
		}
	},
	  /**
	 * =========================================================================================================================
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'ctlVslCallId'){ 
			...
			if(returnValue){
				...
				me.setPackageDetailSettingControl(); //Set Control Setting for PackageTab again
			} else {
	...
	submitDocument:function(){
		...
		if(me.STAT_APPROVED == detailItem.get('statCd') || "CF" == detailItem.get('statCd')){
			MessageUtil.warning('warning_msg','sn_hasBeenApproved1');
			return;
		}
	...
	checkCatgType:function() {
		...
		/*
		Enable/Disable Transshipment Approval Check Box
		WHEN Category type is 'T' (Transshipment)
		*/
		if(dispatchType === "T"){
			refs.refTsAprovalYNChk.setDisabled(false);
			refs.refTsAprovalYNChk.setValue(recvData.get('tsAprovalYN') === 'Y' ? true : false);
		}else{
			refs.refTsAprovalYNChk.setDisabled(true);			
			refs.refTsAprovalYNChk.setValue(false);
		}
	},
	...
	getSearchCondition : function(){
		...
		var shippingNoteItem = me.getView().getShipNoteInitData();
     	...
     	if((vslCallId.length > 0 && vslCallId !== CodeConstants.VESSEL_SCHEDULE_STRG) 
     			||  shipgNoteNo.length > 0 || scn.length > 0) {
...
//		}
	...
	//-------
		...
		detailItem.set('anchorageChk', refs.refAnchorage.checked ? 'Y' : 'N');
		detailItem.set('privateJettyChk', refs.refPrivateJetty.checked ? 'Y' : 'N');
		detailItem.set('tsAprovalYN', refs.refTsAprovalYNChk.checked ? 'Y' : 'N');
		//detailItem.set('additionalChk', refs.refAdditionalChk.checked ? 'Y' : 'N');
		detailItem.set("scn", theSearchDetail.data.scn);
		...
		if(detailItem.dirty || sendArray.length > 0 || isDG){
			...
			updateParm.set('item', detailItem.data);
				...
				success: function(record) {
					...
					me.processFileInfoUpload(isCreated);
	...
	/**
	 * 20241101
	 * Process File Upload Information (Save file information):
	*/
	processFileInfoUpload: function (isCreated) {
		var me = this;
		var refs = me.getReferences();
		var store = me.getStore('shippingNoteUploadInfo');
		var fileUploadStore = me.getStore(me.FILE_UPLOAD_STORE_NAME);
		var detailItem = me.getViewModel().get('theShippingNote');
		var uploadItems = new Array();
		...
		fileUploadStore.getRemovedRecords().forEach(function (removedRecord, index, array) {
			removedRecord.set('fileStream', null);
			removedRecord.set('workingStatus', WorkingStatus.DELETE);
			uploadItems.push(removedRecord.data);
		});

		/**
		 * DG Upload file information
		 */
		
		me.dgFileInfoUpload.forEach(function (modifiedRecord, index, array) {
			modifiedRecord.fileStream = null;
			if (isCreated) {
				modifiedRecord.set('catgCd', modifiedRecord.get('catgCd') + detailItem.get('shipgNoteNo'));
			}
		});
		...
		if (me.dgFileInfoUpload.length > 0) {
			uploadItems = uploadItems.concat(me.dgFileInfoUpload);
		}
	...
	*/
	...
	/**
	 * PPSB_20241112.Re-calculate the Unit TotalVol of package detail
	*/
	...
	/**
	 * PPSB_20241112. Re-calculate the Unit CgWgt of package detail
	*/
	...
	onClearPackage: function(){
		var me = this;
     	...
     	refs.refPkgDetailGrid.setSelection(null);
		...
		refs.refPackageNo.focus(true);
		refs.refPackageNo.setEditable(true);;
	},
	...
	*/
	...
	onAddPackage: function(btn){
		...
		//1.Validation
		var isValidPkgDtl = me.validatePackageDetail(btn);
		if(!isValidPkgDtl){
			return;
		}

		//2. Build the newRecord Model
	...
	/**
	 * PPSB_20241112 Update Package row
	*/
	onUpdatePackage: function(btn){
		var me = this;
     	var refs = me.getReferences();
     	var theSN = me.getViewModel().get('theShippingNote');
		 var thePgkDetail = me.getViewModel().get('thePgkDetail');
		...
		if (selection.get('pkgNo') != thePgkDetail.get('pkgNo') 
			&& me.onPkgDuplicatedValidation(refs.refPackageNo.getValue())) {
			return;
		}
	...
	validatePackageDetail: function(btn){
		var me = this;
     	var refs = me.getReferences();
     	var theSN = me.getViewModel().get('theShippingNote');
		var thePgkDetail = me.getViewModel().get('thePgkDetail');
		...
		var cmdtGroupCd = theSN.get('cmdtGroupCd');
		var eachPkgVol = thePgkDetail.get('eachPkgVol');
		var pkgTotalWgt = thePgkDetail.get('pkgTotalWgt');
		var pkgUnitQty = thePgkDetail.get('pkgUnitQty');
		var strValidation = "";
		var pkgForm = btn.up('form');
		...
		if(StringUtil.isNullorEmpty(thePgkDetail.get('pkgUnitQty'))){
			if(StringUtil.isNullorEmpty(strValidation)) {
		...
		if (cmdtGroupCd == me.MEASUREMENT_CARGO && StringUtil.isNullorEmpty(thePgkDetail.get('pkgAgentId'))) {
			if (StringUtil.isNullorEmpty(strValidation)) {
				strValidation += ViewUtil.getLabel('package_total_agent_required');
			} else {
				strValidation = strValidation + ", " + ViewUtil.getLabel('package_total_agent_required');
			}
		}

		//Validate Amout:
		if (pkgUnitQty == 0 || pkgUnitQty == null) {
			strValidation = ViewUtil.getLabel('qty');
		}

		if (cmdtGroupCd == me.PACKING_LIST) {
			if (pkgTotalWgt == 0 || pkgTotalWgt == null) {
				if (StringUtil.isNullorEmpty(strValidation)) {
					strValidation += ViewUtil.getLabel('package_total_weight_required');
				} else {
					strValidation = strValidation + ", " + ViewUtil.getLabel('package_total_weight_required');
				}

			}
		} else if (cmdtGroupCd == me.MEASUREMENT_CARGO) {
			if (eachPkgVol == 0 || eachPkgVol == null) {
				if (StringUtil.isNullorEmpty(strValidation)) {
					strValidation += ViewUtil.getLabel('package_l_w_h_required');
				} else {
					strValidation = strValidation + ", " + ViewUtil.getLabel('package_l_w_h_required');
				}
			}
		}
	...
	*/
	onPkgGridItemClick:function(){
		...
		refs.refPackageNo.setEditable(false);
	...
	onPackageDetailTabSetting1: function(){
		var me = this;
     	...
     	var theShippingNote = me.getViewModel().get('theShippingNote');
     	var pkgStore = me.getStore(me.PACKAGE_DETAIL_STORE);
     	var cgTpCd = theShippingNote.get('cgTpCd');	
     	//refs.refPackageDetailTab.setDisabled(true);   
 		...
 		if(refs.refProjectCargo.getValue() == true && cgTpCd == CodeConstants.MT_CGTP_BBK){
			refs.refPackageDetailTab.setDisabled(false);
		} else {
			refs.refPackageDetailTab.setDisabled(true);
		}
...
//		}
	...
	*/
	processUploadDG: function () {
		...
		if (detailItem.get('dgItems') != null) {
			if (me.dgFileInfoUpload != null) {
				me.dgFileInfoUpload.forEach(function (record, index, array) {
					if (record.workingStatus != 'D') {
						formData.append(record.fileName, record.fileStream);
						isFileUpload = true;
					}

					if(record.fileStream == null) {
						isFileUpload = false;
						return false;
					}
				});
	...
	/**
	 * Process DG after Declaration Popup close.
	*/
	dgFileInfoUpload: [],
	saveDGProcess: function (retunDgItem) {
		var me = this;
		var dgUploadArr = new Array();
		...
		var detailItem = me.getViewModel().get('theShippingNote');
		var isCreate = detailItem.phantom;
		me.dgFileInfoUpload = [];

		var dgItem = retunDgItem;

		//If DG exits and status != Save => Block Update:
		if (!StringUtil.isNullorEmpty(dgItem.get('dgSeq')) && dgItem.get('dgChk') !== 'S') {
			detailItem.data.dgItems = null;
			...
			fileUploadStore.getModifiedRecords().forEach(function (modifiedRecord, index, array) {
				...
				dgUploadArr.push(modifiedRecord.data);
			});

			// File Upload DELETE RECORD
			fileUploadStore.getRemovedRecords().forEach(function (deleteRecord, index, array) {
				deleteRecord.set('userId', MOST.config.Token.getUserId());
				deleteRecord.set('workingStatus', WorkingStatus.DELETE);

				dgUploadArr.push(deleteRecord.data);
			});
			me.dgFileInfoUpload = dgUploadArr;
			dgItem.set('ixCd', 'X');
			dgItem.commit();

			dgItems.push(dgItem.data);
			detailItem.data.dgItems = dgItems;
		}

		me.getView().detailViewAlias = 'app-shippingnotedetail';
		if (dgView) {
			dgView.close();
		}
	},
	...
	onCellClick: function(ctx, td, cellIndex, record, tr, rowIndex, e, eOpts){
		var me = this;
		var refs = me.getReferences();
		if(record.get('statCd') == 'AP' && record.get('statCdNm') == 'Approved'){
			refs.refBtnConfirmLoading.setDisabled(false);
		} else {
			refs.refBtnConfirmLoading.setDisabled(true);
		}
	},
	...
	onConfirmLoading:function(){
		...
		var grid = me.lookupReference('refShippingNoteGrid');
		var saveStore = me.getStore('updateConfirmLoadingStatus');
		var mainStore = me.getStore(me.MAIN_STORE_NAME);
		
		if(grid.getSelection() && grid.getSelection().length > 0) {
			detailItem = grid.getSelection()[0];
		}
	...
	/**
	 * Check Packing/ Measerment Cargo
	*/
	checkPackageCargo: function (screeMode) {
		var me = this;
		var refs = me.getReferences();
		var theSN = me.getViewModel().get('theShippingNote');
		var isBBK = false, isPkgCargo = false;
		var detailView = me.getDetailBizView();

		try {
			if(screeMode === CONSTANTS.SCREEN_MODE_CREATE) {
				isBBK = refs.ctlTypeofCargo.getValue() == CodeConstants.MT_CGTP_BBK;
				isPkgCargo = refs.ctlCommodityGroupCode.getValue() === me.PACKING_LIST || refs.ctlCommodityGroupCode.getValue() === me.MEASUREMENT_CARGO;
	
			} else {
				theSN = me.getViewModel().get('theShippingNote'); //detailView.items.get(0).recvData;
				isBBK = theSN.get('cgTpCd') == CodeConstants.MT_CGTP_BBK;
				isPkgCargo = theSN.get('cmdtGroupCd') === me.PACKING_LIST || theSN.get('cmdtGroupCd') === me.MEASUREMENT_CARGO;
			}
			return (isBBK && isPkgCargo);

		  } catch (err) {
		  		console.log ('ShippingNoteController.Funciton error: checkPackageCargo')
				console.log (err);			
		  }
	},

	setPackageDetailSettingControl: function () {
		var me = this;
		var refs = me.getReferences();
		var cgTypeCd = '', cmdtGroupCd = '';

		if(me.SCREEN_MODE === CONSTANTS.SCREEN_MODE_CREATE) {
			//cgTypeCd = refs.ctlTypeofCargo.getValue();
			cmdtGroupCd = refs.ctlCommodityGroupCode.getValue();

		} else {
			var theSN = me.getViewModel().get('theShippingNote');
			cmdtGroupCd = theSN.get('cmdtGroupCd');
		}

		me.CMDT_GROUP_CODE = cmdtGroupCd;

		var isPkgCargo = me.checkPackageCargo(me.SCREEN_MODE);		
		refs.refProjectCargo.setValue(isPkgCargo);

		if(!isPkgCargo) {
			refs.refTotalQty.setEditable(false);
			refs.refTotalMt.setEditable(false);
			refs.refLength.setEditable(false);
			refs.refWidth.setEditable(false);
			refs.refHeight.setEditable(false);
			refs.refMsrmt.setEditable(false);

		} else {
			//Set Detail Control for Packagetab
			if(me.CMDT_GROUP_CODE === me.PACKING_LIST) {
				...
				refs.refLength.setEditable(true);
				refs.refWidth.setEditable(true);
				refs.refHeight.setEditable(true);
				refs.refMsrmt.setEditable(false); //auto calculate
				
				refs.refTotalMt.allowBlank = true;
				...
				refs.refPkgAgentId.allowBlank = false;		
			} 
		}
	},
	/**
