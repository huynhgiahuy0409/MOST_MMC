		...
		dgDetail.load({
			...
			callback: function(records, operation, success) {
				if (success) {
					...
					me.setControlByStatus();
				}
			}
		});
	},
	
	setControlByStatus: function () {
		var me = this;
		var refs = me.getReferences();
		var theDG = me.getViewModel().get('theDG');
		var dgStatus = theDG.get('dgChk');

		if(theDG.get('dgChk') == 'Save' || StringUtil.isNullorEmpty(dgStatus))
			refs.refBtnSubmit.show();

		else{
			refs.refBtnSubmit.hide();
