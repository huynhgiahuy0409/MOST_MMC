Ext.define('MOST.view.planning.VesselScheduleInternalController', {
				...
				if (success) {
					if(records.length > 0){
						Ext.each(records, function(record) {
							if (record.get("isOverVslWkComplTime") === 'Y' && record.get('reconcileStatus') === '') {
								record.set('vslColor', 'R');
							}
						});
					}
	...
	setDetailInitialize:function(){
		...
		var berthMaintenanceList = me.getStore('berthMaintenanceList');
		...
		berthMaintenanceList.load({
			callback:function(records, operation, success){
				if(success){
					me.LIST_BERTH_MAINTENANCE = records;
				}
			}
		})
	...
	setDetailTabControl:function(masterItem){
		...
		confirmationSlipItem.set('scn', masterItem.vesselScheduleListDetail.scn);
	...
	onBerthLocationComboChange: function(combo, value, obj){
		...
		var berthMaintenanceStore = me.getStore('berthMaintenanceList');
		var berthLoc = refs.ctlBerthLocation.getValue();
	...
	onWharfMarkStartsFocusleave : function(){
		...
		me.checkBerthMaintenance(false);
	},
	...
	checkBerthMaintenance: function(isSelectATB) {
		var me = this,
			refs = me.getReferences(),
			atb = refs.txtATB.getValue(),
			wharfMarkStart = Number(refs.ctlVesselScheduleDetailWharfMarkStarts.getValue()),
			wharfMarkEnd = Number(refs.ctlVesselScheduleDetailWharfMarkEnds.getValue());

		if (atb && wharfMarkStart && wharfMarkEnd) {
			for (let i = 0; i < me.LIST_BERTH_MAINTENANCE.length; i++) {
				const berthMaintenance = me.LIST_BERTH_MAINTENANCE[i];
				const maintenanFrom = Number(berthMaintenance.get('repFromMeter'));
				const maintenanTo = Number(berthMaintenance.get('repToMeter'));
				const maintenanStopSTime = berthMaintenance.get('repStopStime');
				const maintenanStopETime = berthMaintenance.get('repStopEtime');
				
				if (me.validateTimeBetween(atb, maintenanStopSTime, maintenanStopETime)) {
					var isWharfMarkStartBetween = wharfMarkStart >= maintenanFrom && wharfMarkStart <= maintenanTo; 
					var isWharfMarkEndBetween = wharfMarkEnd >= maintenanFrom && wharfMarkEnd <= maintenanTo; 
					if (isWharfMarkStartBetween || isWharfMarkEndBetween) {
						if (isSelectATB) {
							refs.txtATB.setValue();
						} else {
							refs.ctlVesselScheduleDetailWharfMarkStarts.setValue();
							refs.ctlVesselScheduleDetailWharfMarkEnds.setValue();
						}
						MessageUtil.warning('warning_msg', 'berthMaintenance_block_msg', [maintenanFrom, maintenanTo, Ext.Date.format(maintenanStopSTime, 'd/m/Y H:i'), Ext.Date.format(maintenanStopETime, 'd/m/Y H:i')] );
						return;
					}
				}
	...
	validateTimeBetween : function(date, start, end) {
        var t = date.getTime();
	...
	onRefConfirmationSlipDryBreakBulkGrid_CellClick:function(ctx, td, cellIndex, record, tr, rowIndex, e, eOpts){
		...
		// refs.refCgOptTpCd.setValue(selection.get("cgOptTpCd"));
	...
	onUpdateCs1: function() {
		...
		// selection.set('cgOptTpCd', refs.refCgOptTpCd.getValue());
	...
	// Submit Confirmation Slip1 Process
	...
	onAddCs2:function(){
		...
		record.set('stDt', theConfirmationSlip.get('stDt'))
		record.set('etc', refs.refLiquidETC.getValue())
	...
	onBlNoComboChange: function(combo, value, obj){
		...
		if(selection != null){
			...
			theConfirmationSlip.set('pol', (pol != '' ? pol : CONSTANTS.PORT_CODE)) 	// Fix issue Mantis 0201577
	...
	onRefConfirmationSlipLiquidBulkGrid_CellClick:function(ctx, td, cellIndex, record, tr, rowIndex, e, eOpts){
		...
		refs.refLiquidStartTime.setValue(selection.get("stDt"));
		refs.refLiquidETC.setValue(selection.get("etc"));
	...
	onClearConfirmationSlip2:function(){
		...
		refs.refLiquidStartTime.setValue();
		refs.refLiquidETC.setValue();
	},
	...
	calcLiquidHour: function(tonnage, pumpingRate){
		var hours =  (tonnage / pumpingRate).toFixed(2);
		return hours
	},
	...
	onStartTimeChange: function () {
		var me = this,
			refs = me.getReferences(),
			startTime = refs.refLiquidStartTime.getValue(),
			sltBlSn = refs.refBlSnNoComboBox.getSelection()
			;

		if (StringUtil.isNullorEmpty(sltBlSn) || StringUtil.isNullorEmpty(startTime)) {
			return;
		}

		var startTimeDate = new Date(startTime),
			tonnage = refs.refTotalTonnage.getValue(),
			pumpRate = sltBlSn.get('pumpRate'),
			calcHours = pumpRate ? me.calcLiquidHour(tonnage, pumpRate) : 0,
			hours = Math.floor(calcHours);
			mins = (calcHours - hours) * 60
			;

		startTimeDate.setHours(startTimeDate.getHours() + hours);
		startTimeDate.setMinutes(startTimeDate.getMinutes() + mins);

		refs.refLiquidETC.setValue(startTimeDate)
	}
	/**
	 * ISPS END
	 * =========================================================================================================================
