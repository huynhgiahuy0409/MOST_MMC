Ext.define('MOST.view.monitoring.HandlingInOutListController', {
			...
			if(returnValue){
				...
				if(!StringUtil.isNullorEmpty(returnValue.item.get('vslCallId'))){	
					...
					masterBlCombo.load({
						params:{
							/*ptnrCd: ptnrCd,*/
							scn: returnValue.item.get('scn').toUpperCase()
						}
					});
	...
	rgChange:function(){
		...
		if(!StringUtil.isNullorEmpty(refs.txtVslCallId.getValue()) || !StringUtil.isNullorEmpty(refs.ctlScn.getValue())){
			if(StringUtil.isNullorEmpty(MOST.config.Token.getPtnrCode())){
				ptnrCd = '';
			}else{
				ptnrCd = MOST.config.Token.getPtnrCode();
			...
			listCombo.load({
				params:{
					/*ptnrCd: ptnrCd,*/
					scn: refs.ctlScn.getValue().toUpperCase(),
					vslCallId: refs.txtVslCallId.getValue().toUpperCase()
				},
				callback:function(records,success){
					if(success){
