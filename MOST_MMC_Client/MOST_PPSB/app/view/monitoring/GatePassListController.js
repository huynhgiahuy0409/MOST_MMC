Ext.define('MOST.view.monitoring.GatePassListController', {
	...
	DELIVERY_MODE_COMBO: 'arrSchDModeCombo',
	SHIFT_COMBO: 'arrShftIdCombo',
	/**
	 * CONSTANT END
	 * =========================================================================================================================
	 */	
	...
	onLoad: function(){
		...
		me.setComboBoxWithLocalCache(CacheServiceConstants.DELIVERY_MODE, me.DELIVERY_MODE_COMBO);
		me.setComboBoxWithLocalCache(CacheServiceConstants.SHIFT_COMBO, me.SHIFT_COMBO);
		
		refs.ctlVslCallId.updateEditableControl(false);
		refs.ctlVslCallId.setDisabled(true)
		refs.ctlScn.updateEditableControl(false);
		...
		if (me.userType === CodeConstants.USER_TYPE_EXTERNAL) {
			if (me.existPatnerType(CONSTANTS.PTNR_TYPE_SHIPPING_AGENCY) || 
					(me.existPatnerType(CONSTANTS.PTNR_TYPE_SHIPPING_AGENCY) 
				 	...
				 	if (me.existPatnerType(CONSTANTS.PTNR_TYPE_SHIPPING_AGENCY) 
			 			&& me.existPatnerType(CONSTANTS.PTNR_TYPE_FORWARDER)) {
							me.authority = CodeConstants.MT_EQPURP_BH;
					} else if(me.existPatnerType(CONSTANTS.PTNR_TYPE_SHIPPING_AGENCY)) {
						me.authority = CodeConstants.MT_PTNRTP_SHA;
					}
			} else if (me.existPatnerType(CONSTANTS.PTNR_TYPE_FORWARDER)) {
				me.authority = CodeConstants.MT_PTNRTP_FWD;
			} else if (me.existPatnerType(CONSTANTS.PTNR_TYPE_SHIPPER_CONSIGNEE)) {
				me.authority = CodeConstants.MT_PTNRTP_CNS;
			}
		} else if (me.userType === CodeConstants.USER_TYPE_INTERNAL) {
			if (me.authority != 'CSC') {
		...
		var searchParm = Ext.create('MOST.model.monitoring.SearchGatePassListParm');
	...
	onFind: function() {
		var me = this;
     	var refs = me.getReferences();
    	var store = me.getStore('sn2Combo');
	...
	onExportExcelPdfWithServer : function(gridNameString,isExcel) {
		...
		searchBizParm.classID = 'com.tsb.most.biz.parm.monitoring.SearchGatePassListParm';
		searchBizParm.serviceID = 'MOST.gatePassList.selectCargoGatePassList'
	...
	getSearchCondition : function(){
		...
		if(me.selectValue === 'rdGp') {
			if (!StringUtil.isNullorEmpty(searchParm.data.gatePassNo) || !StringUtil.isNullorEmpty(searchParm.data.cgNo)) {
				cgNo =  searchParm.data.cgNo;
				gatePassNo = searchParm.data.gatePassNo;
				searchType = 'gpCgMst';
			} else {
				MessageUtil.warning('warning_msg', "gatepasslist_gp_nodata_warning_msg");
				return;
			...
			if(refs.refGpFromDt.getValue() != null && refs.refGpToDt.getValue() != null) {
				gatePassNo = searchParm.data.gatePassNo;
				lorryNo = searchParm.data.lorryNo;
				searchType = "gpCgMst";
			} else {
				MessageUtil.warning('warning_msg', "gatepasslist_gp_nodata_warning_msg");
				return;
			...
			if((scn == null || scn == '') && (vslCallId == null || vslCallId == '')) {
				MessageUtil.warning('warning_msg', "gatepasslist_vscallid_scn_nodata_warning_msg");
				return;
			}
			shipgNoteNo = searchParm.data.shipgNoteNo;
			cgNo = searchParm.data.cgNo;
			blNo = searchParm.data.blNo;
			gatePassNo = searchParm.data.gatePassNo;
			delvTpNm = searchParm.data.delvTpNm;
			searchType = "gpCgMst";
			shftId = searchParm.data.shftId;
		...
		if(dateConditionGp != null) {
			if(me.selectValue === 'rdGpDt') {
				params["startDt"] = Ext.Date.format(dateConditionGp.fromDt, 'm/d/Y H:i');
				params["endDt"] = Ext.Date.format(dateConditionGp.toDt, 'm/d/Y H:i');
			}
		}
		
		if(dateConditionEta != null){
			if(me.selectValue === 'rdEta') {
				params["arrvDtFm"] = Ext.Date.format(dateConditionEta.fromDt, 'm/d/Y H:i');
				params["arrvDtTo"] = Ext.Date.format(dateConditionEta.toDt, 'm/d/Y H:i');
			}
		}
	...
	setEditableDisabledRdGp: function(mode) {
		...
		if(mode === false) {
			refs.txtGp1.setValue('');
			refs.txtGr1.setValue('');
		}
	},
	...
	setEditableDisabledRdGpDt: function(mode) {
		...
		if(mode === false) {
			refs.txtGp2.setValue('');
			refs.txtLorry.setValue('');
		}
	},
	...
	setEditableDisabledRdJpvc: function(mode) {
		...
		refs.ctlVslCallId.updateEditableControl(mode);
		refs.ctlVslCallId.setDisabled(!mode)
		refs.ctlScn.updateEditableControl(mode);
		refs.ctlScn.setDisabled(!mode)
		...
		if(mode === false) {
			refs.txtGr3.setValue('');
			refs.txtGp3.setValue('');
		}
	},
	...
	setEditableDisabledRdEta: function(mode) {
		...
		if(mode === false) {
			refs.txtGr4.setValue('');
			refs.txtGp4.setValue('');
		}
	},
	...
	setComboBox: function(vslValue) {
		var me = this;
		var refs = me.getReferences();
		var sn1Store = me.getStore('sn1Combo');
	...
	onCheckJpvcRadioField: function(radioField, newValue, oldValue, eOpts) {
 	...
 	onSearchGatePassList : function(){
		...
		var gatePassListData = me.getStore(me.MAIN_STORE_NAME);
		var txtGRBL = refs.refFindGRBLTextField.getValue();
		var txtGP = refs.refFindGPTextField.getValue();
		var tGPStart = refs.refStartDatefield.getDate();
		var tGPEnd = refs.refEndDatefield.getDate();
	...
	print: function(htmlElement, flag, owner) {
 	...
 	setHHTPrint: function(){
 		...
 		gatePassDetail.load({
			...
			callback: function(records, operation, success) {
				if (success) {
					var printDate = Ext.Date.format(new Date(), 'm/d/Y H:i');
					...
					refs.refShipper.setHtml("SHIPPER : <br><b>"+records[0].data.shprNm+ "</b>");
					refs.refConsignee.setHtml("CONSIGNEE : <br><b>"+records[0].data.cnsneNm+ "</b>");
					...
					refs.refPrintDate.setHtml("DATE/TIME : <b>"+Ext.Date.format(new Date(), 'm/d/Y H:i')+ "</b>");
