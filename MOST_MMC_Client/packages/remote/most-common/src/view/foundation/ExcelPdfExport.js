Ext.define('MOST.view.foundation.ExcelPdfExport', {
	...
	exportExcelPdfWithServer : function(me, gridNameString, searchBizParm, isExcel, addString){
		...
		var exportItem = Ext.create('MOST.model.common.export.FileExportBizParm');
		var proxy = exportItem.getProxy();
		proxy.url = MOST.config.Locale.getRestApiDestUrl() + '/v1/fileexport/createFile';
		...
		var gridItem = Ext.create('MOST.model.common.export.GridItem');
			   ...
			   column.hidden !== true) {
				if(column.ownerCt.id.indexOf('gridcolumn')>-1){
					...
					if(headerItem==null||headerItem.get('headerId')!=ownerCt.headerId){
						...
						//headerItem.set('type', column.get("style")); //modified by Brian (2021/01/19)
 					}else{
		...
		gridItem.set('thousandSeparator', MOST.config.Locale.getThousandSeparator());
		gridItem.set('decimalPoint', MOST.config.Locale.getDecimalSeparator());
	...
	getExcelPdfExportFileName : function(addString){
		...
		var dateString = Ext.Date.format(currentDate, MOST.config.Locale.getExcelExportDateTimeFormat());
		return Ext.String.format("{0}_{1}_{2}", addString, MOST.config.Token.getUserId(), dateString);
	},
	...
	setExcelPdfExportSearchParam : function(exportItem, searchBizParm){
		...
		var searchDataBizParm = Ext.create('MOST.model.common.export.FileExportServiceBizParm');
		...
		for (var key in searchBizParm){
			searchCondition = Ext.create('MOST.model.common.export.StringValueItem');
		...
		var bizParmMetaInfo = Ext.create('MOST.model.foundation.parm.BizParmMetaInfo');
	...
	setCellItem : function(item, text, mergeX, mergeY, mergeX1, mergeY1){
		var gridCellItem = Ext.create('MOST.model.common.export.GridCellItem');
		...
		//added by Brian (2021/01/19)
		if(item.precision != undefined){
			gridCellItem.set('precision', item.precision);
		}
		return gridCellItem;
	},
