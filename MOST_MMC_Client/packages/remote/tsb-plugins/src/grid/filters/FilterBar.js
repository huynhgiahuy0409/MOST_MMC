 ...
 */
Ext.define('TSB.plugin.grid.filters.FilterBar', {
	/**
     * @memberOf TSB.plugins
     */
    extend: 'Ext.plugin.Abstract',
    alias: 'plugin.tsb-gridfilterbar',
    ...
    init: function (grid) {
        ...
        me.headerCtListeners = headerCt.on({
            ...
            scope: me,
            add: me.onAdd,
            beforerender: me.onCreateFilterItems
//            afterrender: me.onCreateFilterItems
...
//        debugger;
    ...
    onCreateFilterItems: function (headerCt) {
    	var grid = this.grid,
	        store = grid.getStore(),
	        columns = grid.columnManager.getColumns(),
	        len = columns.length,
    	...
    	if(headerCt.itemId === 'normalHeaderCt') {
    		// We start with filters defined on any columns.
    		for (i = 0; i < len; i++) {
    			column = columns[i];
    			filter = column.filter;
...
//    					flex: 1
//    				});
//    				column.add(Ext.Factory.gridFilter(filter));
//    				filter.active = true;
    				
//    				debugger;
    				
    				var item = column.add(filter);
//    				debugger;
    			}
    		}
    	}
