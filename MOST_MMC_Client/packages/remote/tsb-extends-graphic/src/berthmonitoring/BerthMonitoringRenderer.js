Ext.define('TSB.gux.berthmonitoring.BerthMonitoringRenderer', {
    /**
    ...
    alias: 'widget.app-berthmonitoringrenderer',
    ...
    initializeMetaValue: function() {
		...
		me.meta.berthHeight = parseFloat(me.storeMeta.getAt(me.storeMeta.findExact('key','berthHeight')).data.value);
		...
		me.meta.bothOpeColor = me.storeMeta.getAt(me.storeMeta.findExact('key','bothOpeColor')).data.value;
		me.meta.imOpeColor = me.storeMeta.getAt(me.storeMeta.findExact('key','imOpeColor')).data.value;
		me.meta.exOpeColor = me.storeMeta.getAt(me.storeMeta.findExact('key','exOpeColor')).data.value;
		me.meta.nonOpeColor = me.storeMeta.getAt(me.storeMeta.findExact('key','nonOpeColor')).data.value;
		me.meta.onWrkStat = me.storeMeta.getAt(me.storeMeta.findExact('key','onWrkStat')).data.value;
		me.meta.nonWrkStat = me.storeMeta.getAt(me.storeMeta.findExact('key','nonWrkStat')).data.value;
		...
		me.meta.viewWidth = me.bodyWidth + 800  //me.meta.timelineWidth + me.meta.berthWidth;
		me.meta.viewHeight = 500//me.meta.berthHeight + me.meta.windowHeight;
		me.meta.totalMeterLength = totalMeterLength// me.meta.berthHeight + me.meta.windowHeight;

		me.meta.stBerthX = parseFloat(me.storeMeta.getAt(me.storeMeta.findExact('key','stBerthX')).data.value);
		me.meta.stBerthY = parseFloat(me.storeMeta.getAt(me.storeMeta.findExact('key','stBerthY')).data.value);
    },
    ...
    initializeDrawComponent: function() {
		...
		me.getSurface('blink').setRect([0,me.scrollY,me.meta.viewWidth,me.meta.viewHeight]);	//Berth
    ...
    redraw: function (record, change) {
		...
		berthSurface.add(Ext.create('widget.app-berthmonitoringstructuredraw', config));
...
//			x: 0,
//			y: me.meta.berthHeight + 10
//		};
//		windowSurface.add(Ext.create('widget.app-berthmonitoringwindowdraw', config));
...
//		});
		...
		me.plans.each(function(record,idx){
			if(x > record.data.startPos && x < record.data.endPos
			&& y > record.data.startH && y < record.data.endH) {
				indexes.push(idx);
			}
		});
	...
	renderBerthPlanTooltipSprites : function(pos, rec, opts, div){
		...
		if(rec.berthSide == 'L'){
			tooltipXPos = pos[0] - tooltipWidth- 5;
			tooltipYPos = pos[1] + (rec.loa - tooltipHeight)/2;
		}else if(rec.berthSide == 'R'){
			tooltipXPos = pos[0] +30;
			tooltipYPos =  pos[1]+ (rec.loa - tooltipHeight)/2;	
		}else if(rec.berthSide == 'T'){
			tooltipXPos = pos[0];
			tooltipYPos = pos[1]- tooltipHeight- 10;
		}else{
			if(rec.rotate < 180){
				tooltipXPos = pos[0] ;
			}else{
				tooltipXPos = pos[0] - tooltipWidth + 20 ;
			}	
			tooltipYPos = pos[1] + 25;
		}
	...
	getBerthPlanPos: function(rec) {
		...
		return [rec.startPos, rec.startH, 0, 0];
	...
	//	var legendSurface = me.getSurface('legend');
		...
		berthSurface.setRect([0,y,me.meta.viewWidth,me.meta.viewHeight]);
		berthSurface.renderFrame();
