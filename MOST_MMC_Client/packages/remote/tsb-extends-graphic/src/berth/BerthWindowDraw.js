...
Ext.define('TSB.gux.berth.BerthWindowDraw', {
    ...
    render: function(surface, ctx) {
		...
		//Draw Maintenance Berth
		me.maintenanceBerth.each(function(record,idx){
			var berthCdIdx = me.berths.findExact('berthCd',record.data.berthCd);
			if(berthCdIdx > -1) {
				berthRecord = me.berths.getAt(berthCdIdx);
				if(berthRecord.data.drawable === 1) {
					var pos = me.getBerthPlanPos(null,record.data);
					//Draw Maintenance
					me.drawMaintenanceBerth(ctx, pos, record.data, berthRecord.data);
				}
			}
		});
    },
	...
	drawVessel: function(ctx, pos, rec, berth) {
		...
		ctx.fillText(rec.vslCallId, me.x + pos[0] + pos[2] / 2, me.y + pos[1]+ hgtOffset / 2 - (hgtOffset / 4));
	...
	drawMaintenanceBerth: function(ctx, pos, rec, berth) {
		var me = this;

		//Draw Vessel Rect
		ctx.font = me.meta.vesselFontSize * me.meta.baseBerthUnit + 'px ' + me.meta.fontType;
		ctx.strokeStyle = me.meta.vesselGuideColor;
		ctx.fillStyle = me.meta.maintenanceBerthColor;
		ctx.globalAlpha = "0.7";
		...
		ctx.fillText(rec.berthCd, me.x + pos[0] + pos[2] / 2, me.y + pos[1]+ hgtOffset / 2 - (hgtOffset / 4));
		...
		ctx.textBaseline = 'top';
		ctx.fillStyle = me.meta.vesselGuideColor;
		ctx.fillText(rec.repFromMeter, me.x + posX +1, me.y + pos[1]+ hgtOffset / 2 - (hgtOffset / 4));
		...
		ctx.textBaseline = 'top';
		ctx.fillStyle = me.meta.vesselGuideColor;
		ctx.fillText(rec.repToMeter , me.x + posX + 1, me.y + pos[1]+ hgtOffset / 2 - (hgtOffset / 4));
