Ext.define('TSB.gux.berth.BerthRenderer', {
    ...
    initializeMetaValue: function() {
		...
		me.meta.maintenanceBerthColor = me.storeMeta.getAt(me.storeMeta.findExact('key','maintenanceBerthColor')).data.value;
		...
		if (me.site == 'JPB') {
			me.meta.viewDisplacement = me.storeMeta.getAt(me.storeMeta.findExact('key','viewDisplacement')).data.value;
			me.meta.viewBerthDepth = me.storeMeta.getAt(me.storeMeta.findExact('key','viewBerthDepth')).data.value;
			me.meta.viewEquipment = me.storeMeta.getAt(me.storeMeta.findExact('key','viewEquipment')).data.value;
			me.meta.viewLengthBerth = me.storeMeta.getAt(me.storeMeta.findExact('key','viewLengthBerth')).data.value;

			me.meta.displacementArea = parseFloat(me.storeMeta.getAt(me.storeMeta.findExact('key','displacementArea')).data.value);
			me.meta.berthDepthArea = me.storeMeta.getAt(me.storeMeta.findExact('key','berthDepthArea')).data.value;
			me.meta.equipmentArea = me.storeMeta.getAt(me.storeMeta.findExact('key','equipmentArea')).data.value;
			me.meta.lengthBerthArea = parseFloat(me.storeMeta.getAt(me.storeMeta.findExact('key','lengthBerthArea')).data.value);

			var viewLengthBerthOffset = me.meta.viewLengthBerth === 'Y' ? 1 : 0;
			var viewDisplacementOffset = me.meta.viewDisplacement === 'Y' ? 1 : 0;
			var viewBerthDepthOffset = me.meta.viewBerthDepth === 'Y' ? 1 : 0;
			var viewEquipmentOffset = me.meta.viewEquipment === 'Y' ? 1 : 0;
			
			me.meta.berthHeight = me.meta.baseFixedUnit * 
			(me.meta.stsCoverageArea * viewSTSCoverageOffset 
				+ me.meta.terminalNameArea * viewTerminalNameOffset  
				+ me.meta.displacementArea * viewDisplacementOffset 
				+ me.meta.equipmentArea * viewEquipmentOffset 
				+ me.meta.berthDepthArea * viewBerthDepthOffset 
				+ me.meta.berthNameArea * viewBerthNameOffset 
				+ me.meta.lengthBerthArea * viewLengthBerthOffset 
				+ me.meta.meterArea * viewMeterOffset 
				+ me.meta.bittArea * viewBittOffset);
		} else {
	...
	addPlanTooltipSprites : function(planSurface){
		...
		me.bodyTooltipTotalDSLDCargoMTSprite = planSurface.add({
			type: 'text',
			font: me.meta.vesselFontSize + 'px ' + me.meta.fontType,
			fillStyle: me.meta.tooltipBodyForeColor,
			textAlign: 'left',
		...
		me.bodyTooltipTotalDSLDCargoQTYSprite = planSurface.add({
			type: 'text',
			font: me.meta.vesselFontSize + 'px ' + me.meta.fontType,
			fillStyle: me.meta.tooltipBodyForeColor,
			textAlign: 'left',
		...
		me.bodyTooltipBalanceDSLDCargoMTSprite = planSurface.add({
			type: 'text',
			font: me.meta.vesselFontSize + 'px ' + me.meta.fontType,
			fillStyle: me.meta.tooltipBodyForeColor,
			textAlign: 'left',
		...
		me.bodyTooltipBalanceDSLDCargoQTYSprite = planSurface.add({
			type: 'text',
			font: me.meta.vesselFontSize + 'px ' + me.meta.fontType,
			fillStyle: me.meta.tooltipBodyForeColor,
			textAlign: 'left',
		...
		me.bodyTooltipCommodityDescSprite = planSurface.add({
			type: 'text',
			font: me.meta.vesselFontSize + 'px ' + me.meta.fontType,
			fillStyle: me.meta.tooltipBodyForeColor,
			textAlign: 'left',
		...
		me.bodyTooltipReasonSprite = planSurface.add({
			type: 'text',
	        font: me.meta.vesselFontSize + 'px ' + me.meta.fontType,
			fillStyle: me.meta.tooltipETDForeColor,
			textAlign: 'left',
    ...
    redraw: function (record, change) {
		...
		me.maintenanceBerth = me.storeMaintenanceBerth;
		me.site = me.site
		...
		config =  {
		    ...
		    bitts: me.bitts,
		    plans: me.plans,
			maintenanceBerth: me.maintenanceBerth,
	...
	getSelectedBerthPlanIndexesById: function(vslCallId) {
		var me = this;
		var indexes = new Array();
		var x, y;
		//Global Flag for Planning
		...
		me.plans.each(function(record,idx){
			...
			if(berthCdIdx > -1) {
				berthRecord = me.berths.getAt(berthCdIdx);
				if(berthRecord.data.drawable === 1) {
					if(record.data.vslCallId===vslCallId){
						indexes.push(idx);
					}
				}
			}
	...
	getSelectedBerthMaintenanceIndexes: function(x, y) {
		var me = this;
		var indexes = new Array();
		...
		//Draw BerthMaintenance Plan
		me.maintenanceBerth.each(function(record,idx){
			var maintenanceBerthCdIdx = me.berths.findExact('berthCd', record.data.berthCd);
			
			if(maintenanceBerthCdIdx > -1) {
				maintenanceBerthRecord = me.berths.getAt(maintenanceBerthCdIdx);
				
				if(maintenanceBerthRecord.data.drawable === 1) {
					var pos = me.getBerthPlanPos(null,record.data);// Berth Maintenance location 
	...
	getMinDistViolateVessels : function(rec){
		...
		me.plans.each(function(record,idx){
			if(record.get('vslCallId') !== rec.vslCallId){
	...
	selectMaintenanceBerthPlan: function(rec) {
		var me = this;
		var pos = me.getBerthPlanPos(null,rec); 
	...
	renderBerthPlanSprites: function(pos, rec, opts, div) {
		...
		if(rec.drawType === "R"){
			me.renderBerthMaintenanceTooltipSprites(pos, rec, opts, div);
		}else{
	...
	renderBerthPlanTooltipSprites : function(pos, rec, opts, div){
		...
		me.bodyTooltipVslNameSprite.setAttributes({
			...
			y: me.bodyTooltipVslCallIdSprite.attr.y + tooltipHeight/10,
			hidden: false,
			text : rec.vesselName
		});
		...
		me.bodyTooltipETASprite.setAttributes({
			x: tooltipXPos + tooltipTextOffsetLeft,
			y: me.bodyTooltipVslNameSprite.attr.y + tooltipHeight/10,
			hidden: false,
		...
		me.bodyTooltipETDSprite.setAttributes({
			x: tooltipXPos + tooltipTextOffsetLeft,
			y: me.bodyTooltipETASprite.attr.y + tooltipHeight/10,
			hidden: false,
			text: "ETD : " + Ext.Date.format(rec.etd, 'd-m H:i'),
		});

		me.bodyTooltipTotalDSLDCargoMTSprite.setAttributes({
			x: tooltipXPos + tooltipTextOffsetLeft,
			y: me.bodyTooltipETDSprite.attr.y + tooltipHeight/10,
			hidden: false,
			text: "Total Discharge/ Loading Cargo (Mt): " + rec.totalDSLDMt,
		});

		me.bodyTooltipTotalDSLDCargoQTYSprite.setAttributes({
			x: tooltipXPos + tooltipTextOffsetLeft,
			y: me.bodyTooltipTotalDSLDCargoMTSprite.attr.y + tooltipHeight/10,
			hidden: false,
			text: "Total Discharge/ Loading Cargo (Qty): " + rec.totalDSLDQty,
		});

		me.bodyTooltipBalanceDSLDCargoMTSprite.setAttributes({
			x: tooltipXPos + tooltipTextOffsetLeft,
			y: me.bodyTooltipTotalDSLDCargoQTYSprite.attr.y + tooltipHeight/10,
			hidden: false,
			text: "Balance Discharge/ Loading Cargo (Mt): " + rec.balanceDSLDMt,
		});

		me.bodyTooltipBalanceDSLDCargoQTYSprite.setAttributes({
			x: tooltipXPos + tooltipTextOffsetLeft,
			y: me.bodyTooltipBalanceDSLDCargoMTSprite.attr.y + tooltipHeight/10,
			hidden: false,
			text: "Balance Discharge/ Loading Cargo (Qty): " + rec.balanceDSLDQty,
		});

		me.bodyTooltipCommodityDescSprite.setAttributes({
			x: tooltipXPos + tooltipTextOffsetLeft,
			y: me.bodyTooltipBalanceDSLDCargoQTYSprite.attr.y + tooltipHeight/10,
			hidden: false,
			text: "Commodity Description: " + rec.commodityList,
		});
	},
	...
	//ToolTip display for Maintenance Berth (2024/11/11)
	renderBerthMaintenanceTooltipSprites : function(pos, rec, opts, div){
		var me = this;
		var windowWidth = me.getSize().width;
		var windowHeight = me.getSize().height;
		...
		me.bodyTooltipETASprite.setAttributes({
			...
			hidden: false,
			text: "Start : " + Ext.Date.format(rec.repStopStime, 'd-m H:i'),
		});
		...
		me.bodyTooltipETDSprite.setAttributes({
			...
			y: me.bodyTooltipETASprite.attr.y + tooltipHeight/10,
			hidden: false,
			text: "End : " + Ext.Date.format(rec.repStopEtime, 'd-m H:i'),
		});
		
		me.bodyTooltipReasonSprite.setAttributes({
			x: tooltipXPos + tooltipTextOffsetLeft,
			y: me.bodyTooltipETDSprite.attr.y + tooltipHeight/10,
			hidden: false,
			text: "Reason : " + rec.repStopRsnNm,
		});
	...
	getBerthPlanPos: function(rec, maintenanceRec, sftRec) {
		var me = this;
		...
		if(sftRec != null){
			arrTime = sftRec.actShiftStime === null ? sftRec.estShiftStime : sftRec.actShiftStime;
			depTime = sftRec.actShiftEtime === null ? sftRec.estShiftEtime : sftRec.actShiftEtime;
			
			berthingWidth = me.getBerthingWidth(sftRec);
			
			// Validation
			if(sftRec.startPos && sftRec.startPos > 0){
				startPos = sftRec.startPos;
				if(sftRec.berthAlongside === 'S' || sftRec.berthAlongside === 'P') {	
					endPos = sftRec.startPos + sftRec.loa;
				}else{
					if (rec.width) {
						endPos = rec.startPos + rec.width;
					} else {
		...
		} else if (maintenanceRec != null) { 
			arrTime = maintenanceRec.repStopStime;
			depTime = maintenanceRec.repStopEtime;
			berthingWidth = maintenanceRec.repToMeter - maintenanceRec.repFromMeter;
			
			startPos = maintenanceRec.repFromMeter;
			endPos = maintenanceRec.repToMeter;
			
			rec = maintenanceRec;
			
		} else {
			if(rec.planType ==="S"){
				arrTime = rec.actShiftEtime ? rec.actShiftEtime : rec.estShiftEtime ? rec.estShiftEtime : rec.eta;
			}else{
				arrTime = me.getBerthPlanTimeRule(rec, 'arrival');
			}
			
			depTime = me.getBerthPlanTimeRule(rec, 'departure');
			berthingWidth = me.getBerthingWidth(rec);
			// Validation
	...
	doPlanning: function (newpos, rec, prevrec, div) {
		...
		var checkValue = me.checkMaintenanceBerth(planInfo);
		
		if(checkValue.isValid === false){
	...
	checkMaintenanceBerth: function(planInfo){
		...
		if(planInfo[2] != null && planInfo[3] != null){
			for (var i=0;i<me.maintenanceBerth.getData().length;i++){
				var record = me.maintenanceBerth.getData().getAt(i);
