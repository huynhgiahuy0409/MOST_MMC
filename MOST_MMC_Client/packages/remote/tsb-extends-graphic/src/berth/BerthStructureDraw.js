...
Ext.define('TSB.gux.berth.BerthStructureDraw', {
    ...
    render: function(surface, ctx) {
		...
		if (me.site == 'JPB') {
			...
			var lengthBerthY = terminalAreaY + 22;
			var lengthBerthHeight = me.meta.baseFixedUnit * me.meta.lengthBerthArea * viewLengthBerthOffset;
	
			var displacementY = lengthBerthY + 7 + lengthBerthHeight;
			var displacementHeight = me.meta.baseFixedUnit * me.meta.displacementArea * viewDisplacementOffset;
	
			var berthDepthY = displacementY + 7 + displacementHeight;
			var berthDepthHeight = me.meta.baseFixedUnit * me.meta.berthDepthArea * viewBerthDepthOffset;
	
			var equipmentY = berthDepthY + 7 + berthDepthHeight;
			var equipmentHeight = me.meta.baseFixedUnit * me.meta.equipmentArea * viewEquipmentOffset;
	
			var stsAreaY = equipmentY + equipmentHeight;
		...
		me.berths.each(function(record,idx){
			if(record.data.drawable === 1) {
				...
				if (me.site == 'JPB') {
					...
					ctx.fillText(record.data.length, berthX + length / 2 * dirOffset, lengthBerthY);
					...
					ctx.textBaseline = 'top';
					ctx.fillStyle = me.meta.berthNameColor;	
					ctx.fillText(record.data.displacement, berthX + length / 2 * dirOffset, displacementY);
					...
					ctx.textBaseline = 'top';
					ctx.fillStyle = me.meta.berthNameColor;	
					ctx.fillText(record.data.berthDept, berthX + length / 2 * dirOffset, berthDepthY);
					...
					ctx.textBaseline = 'top';
					ctx.fillStyle = me.meta.berthNameColor;	
					ctx.fillText(record.data.equipments, berthX + length / 2 * dirOffset, equipmentY);
