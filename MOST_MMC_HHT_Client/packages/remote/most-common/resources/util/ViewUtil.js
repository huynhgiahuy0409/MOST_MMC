...
ViewUtil.openDetailPopup = function(me, record, userTitle, isModal, isOnlyRetrieve, userDetailViewAlias){
	...
	//2024.01.17 Added by Leslie (To apply Component authority)
	var initThis = this;
	var mainController = me.getView().findParentByType('app-main').getController();
	var menuStore = mainController.getViewModel().getStore("menuList");
	var authStore = mainController.getViewModel().getStore("retrieveAuthList");
	
	var idx = menuStore.findExact('screenPathAddress', me.getView().alias.at(0).substr(7));
	var viewObj = menuStore.getAt(idx);
	
	authStore.load({
		params: {
			pgmId: viewObj.get("menuId"),
			cargoType: MOST.config.Token.getCgTpAuth()
		},
		callback: function(authItems, operation, success){
			if(success){
				if (!win) {
					var listeners = {
...
//								
				...
				if(!isOnlyRetrieve){ // Only Retrieve
					...
					if(detailController.onDetailCreate && CONSTANTS.SYSTEM_CODE == 'KMDC'){ 
						...
						if(initThis.checkAccess(me, "insert", record)){
							win.tools.push({
								xtype: 'button',
					...
					if(detailController.onDetailSave){
						...
						if(initThis.checkAccess(me, "save", record)){
							win.tools.push({
								xtype: 'button',
					...
					if(detailController.onDetailRemove){ // Activated only when the function of the DELETE button is implemented
						...
						if(initThis.checkAccess(me, "delete")){
							win.tools.push({
								xtype: 'button',
				...
				//				cls: 'delete-button',
					...
					if(detailController.onPrintPDF){
						...
						if(initThis.checkAccess(me, "inquiry")){
							win.tools.push({
								xtype: 'button',
				...
				//				cls: 'preview-button',
					...
					if(detailController.onDetailDownload){
						...
						if(initThis.checkAccess(me,"download", record)){
							win.tools.push({
								xtype: 'button',
				...
				//				cls: 'delete-button',
					...
					//2024.01.17 Added by Leslie (To apply Component authority to detail screen)
					var componentList = new Array();
					var authItems = authItems.filter((value) => value.get("screenType") == "D")
							.filter((value) => value.get("pgmId") == viewObj.get("menuId"));
					
					authItems.forEach(function(item){
						if(!componentList.includes(item.get('xtype'))){
							componentList.push(item.get('xtype'));
						}
					});
					
					for(var i = 0; i < componentList.length; i++){
						var fieldList = Ext.ComponentQuery.query('[xtype="'+componentList.at(i)+'"]',bizView);
						...
						for(var j = 0; j < fieldList.length; j++){
							...
							if(!StringUtil.isNullorEmpty(fieldList[j].reference)){
								for(var k = 0; k < authItems.length; k++){
									if(authItems[k].get("refId") === fieldList[j].reference){
										isHidden = authItems[k].get("hidden") === "Y" ? true : false;
										isReadOnly = authItems[k].get("readOnly") === "Y" ? true : false;
										isDisabled = authItems[k].get("disabled") === "Y" ? true : false;
										
										if(fieldList[j].hidden != undefined){
											fieldList[j].setHidden(isHidden);
										}
										
										if(fieldList[j].readOnly != undefined){
											fieldList[j].setReadOnly(isReadOnly);
										}
										
										if(fieldList[j].disabled != undefined){
											fieldList[j].setDisabled(isDisabled);
										}
									}
								}
							}
				...
				var tabPanelList = Ext.ComponentQuery.query('tabpanel').filter((item) => item.reference != 'ref-maintab');
				
				for(var i = 0; i < tabPanelList.length; i++){
					if(tabPanelList[i].getRefOwner().up() != null && tabPanelList[i].getRefOwner().up().reference != 'ref-maintab'){
						tabPanelList[i].setActiveTab(0);
					}
				}
