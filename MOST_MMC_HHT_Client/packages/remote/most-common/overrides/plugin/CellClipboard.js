 ...
 * clipboard format.
 */
Ext.define('MOST.plugin.CellClipboard', {
    ...
    getCellData: function (format, erase) {
        ...
        if(selection) {
            selection.eachCell(function (cellContext) {
                ...
                if(mode && mode === 'Cell' && (!column.dataIndex || column.dataIndex != cmp.selectedDataIndex))
                {
                	return;
                }
    ...
    putCellData: function (data, format) {
        ...
        for (sourceRowIdx = 0; sourceRowIdx < recCount; sourceRowIdx++) {
        	...
        	if (destination.column.editor !== undefined && destination.column.editor !== null) {
