Ext.define('MOST.view.popup.CMMCdPopupHHTController', {
	extend: 'MOST.view.foundation.BaseViewController',

	requires: [
	],

	alias: 'controller.cmmcdpopuphht',	
	...
	onSearchHHT: function() {
		var me = this;
     	...
     	var grid = me.lookupReference('refCommonCodePopupGrid');
     	var grideStore = grid.getStore();
     	var searchType ='';
		...
		if(searchType == 'CMDT'){
	    	...
	    	commonstore.load({
				...
				callback: function(records, operation, success) {
					if (success) {
						for(var i = 0; i < records.length ; i++ ){
							grideStore.insert(i, [{codeName : records[i].data.codeName , code : records[i].data.code}]);
			...
			anotherstore.load({
				params: {
					...
					ptnrCode : searchString
				},
				callback: function(records, operation, success) {
					if (success) {
						grideStore.removeAll();
						for(var i = 0; i < records.length ; i++ ){
							grideStore.insert(i, [{codeName : records[i].data.ptnrName ,code:records[i].data.ptnrCode}]);
						}
					}
				}
			});
			...
			callback: function(records, operation, success) {
				if (success) {
					if(screenType == 'TYPE_MANPOWER' || screenType =='TYPE_STEVEDORE'){
			    		...
			    		store.removeAll();
			    		store.insert(1, [{codeName : 'Hatch' ,code:CodeConstants.MT_LOCDIV1_HTC }]);
			    		store.insert(2, [{codeName : 'Warehouse' ,code:CodeConstants.MT_LOCDIV1_WHO }]);
			    		store.insert(3, [{codeName : 'Break Bulk Wharf' ,code:CodeConstants.MT_LOCDIV1_WRF}]);
			    	}	
				}
			}
		});
    		...
    		if(divCd == CodeConstants.MT_LOCDIV1_HTC){
    			commonCodeListStore.load({
    				...
    				callback: function(records, operation, success) {
    					if (success) {
    						for(var i = 0; i < records.length ; i++ ){
    							grideStore.insert(i, [{codeName : records[i].data.scdNm , code:records[i].data.scd}]);
    						}
    					}
    				}
    			});
    			...
    			locationCodeListStore.load({
    				...
    				callback: function(records, operation, success) {
    					if (success) {
							...
							for(var i = 0; i < 11; i++){
								grideStore.insert(i+records.length, [{code: 'H' + (i+1),codeName: 'H' + (i+1)}])
							}
