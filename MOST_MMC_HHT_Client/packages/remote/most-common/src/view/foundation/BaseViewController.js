...
//	},
	...
	*/
	...
	onConnectWs: function(){
		...
		var refs = me.getReferences();
		if(!me.WSCONNECT){
			me.WSCONNECT = Ext.create(me.WSUTIL);
		}else {
			me.WSCONNECT.setUrl(urlWS);
		}
		
		var task = {
			identifyId: 'checkStatusTask',
			run: function() {
				var milliseconds = me.formatDate(new Date(), 'yyyy-MM-dd HH:mm:ss');
				console.log('Check connection - ' + milliseconds);
				me.connectWebSocket();
			},
			interval: me.WS_CHECKTIME,
		}
	...
	fn_handleMessage: function(me, wsClient, frame, messageOutput) {
		console.log("###### Message #####");
		me.processBaseWsMsg(messageOutput.body);
	},
