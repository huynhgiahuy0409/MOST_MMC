Ext.define('TSB.ux.menu.TreeMenu', {
    ...
    initComponent: function() {
		...
		//added by Brian (2023/03/20)
    	var itemTemplate = new Ext.XTemplate(
		    '<tpl for=".">',
		        '<div "position: absolute; left: 20px; top: 450px;>',
		          '<span>{menuScreenName}</span>',
		        '</div>',
		    '</tpl>'
    	);
		Ext.apply(me, {
		    items: [{
				xtype: 'textfield',
				reference: 'refSearchMenu',
				itemId: 'txtSearchMenuName',
				text: 'Search',
				emptyText: 'Search',
		   	    //hidden: true,
				enableKeyEvents: true,
				listeners: {
					change  : 'onSearchMenuItem',
					keydown: 'keyDown'
				}
		    },{
		    	
				xtype: 'dataview',
				reference: 'refSearchDataView',
				itemId: 'txtSearchMenuId',
				height: 200,
		   	    width: 150,
...
//        		if(record.data.upperMenuId != 'ROOT'){
        		if(record.data.menuTypeCode != 'FOLDER'){
        			...
        			/**
        			 * Brian Park.2021.11.22
        			 * Do not delete 3-level
        			 */
