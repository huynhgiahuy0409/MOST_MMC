...
//    FIRST_LOAD_VIEW_NAME : 'app-screenauthority',
...
//										});
    ...
    createTab: function(prefix, rec, cfg, isConfigChanged) {
        ...
        //added by Brian (2024/01/12) Screen Authority
        var authStore = me.getStore("retrieveAuthList");
        
        authStore.load({
			params: {
				pgmId: rec.data.menuId,
			...
			callback: function(authItems, operation, success){
		        ...
		        //added by Brian (apply screen authority, 2024/01/10)
				var componentList = new Array();
				var authItems = authItems.filter((value) => value.get("screenType") == "L")
					   .filter((value) => value.get("pgmId") == rec.data.menuId);
				...
				for(var i = 0; i < componentList.length; i++){
					var fieldList = Ext.ComponentQuery.query('[xtype="'+componentList.at(i)+'"]',tab);
...
//                    }else{
//                        me.onLoadView('app-vesselscheduleexternal');
//                    }
...
//			branchCode: me.lookupReference('branch').getValue()
		...
		item.save({
            callback: function(record, operation, success) {
            	if(success) {
            		...
            		localStorage.setItem('branch', CONSTANTS.DEFAULT_BRANCH);
            		location.reload();
            	}else{
					var proxy = item.getProxy();
	...
	alerting: function(){
		...
		if(refs.refAlerting.iconCls == 'x-fa fa-bell-o'){
			var win = refs.refAlertingWin;
			if (!win) {
	   			win = Ext.create('Ext.window.Window', {
		        	reference: 'refAlertingWin',
		        	title : 'Alert List',
		        	layout: 'fit',
		        	width: 320,
		            height: 600,
