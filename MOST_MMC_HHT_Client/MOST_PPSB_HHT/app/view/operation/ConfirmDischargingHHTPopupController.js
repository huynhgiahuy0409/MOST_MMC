Ext.define('MOST.view.operation.ConfirmDischargingHHTPopupController', {
	...
	/**
	 * WEBSOCKET CONSTANT START
	*/
	WSUTIL: 'MOST.websocket.WebSocketClient',
	TOPICID: '',
	TOPIC_ALL: '/ws/topic/all',	
	YARD_TRUCK_NO: '',
	WSCONNECT: null, // MyApp.websocket.WebSocketClient,
	WS_CHECKTIME: 15000,
	TASK_LISTTASK: ['checkStatusTask'],
	/**
	 * WEBSOCKET CONSTANT END
	 */
	...
	onLoad: function(){
		...
		modeOfOprCombo.load();
		me.setComboBoxWithLocalCache(CacheServiceConstants.APFP_COMBOBOX, 'confirmDischargingHatchDrtCombo');
	...
	// Start Process received Message:
	processMsg: function(msg) {
		var me = this;
		var wsContent = JSON.parse(msg).content;
		
		try {
			var dataItem = JSON.parse(wsContent);
			if(!dataItem){
				console.log("-- [DS.processMsg] ---  No Data !!!");
				return;
			}
			console.log('--- [DS.processMsg] sender = ' + dataItem.sender);
			
			if(dataItem.type == CodeConstants.WS_TYPE_OPERATE
					&& dataItem.sender == CodeConstants.WS_SENDER_DISCHARGING){
					
					me.onRefresh();
			}
			
		} catch (error) {
			console.log('--- [DS.processMsg] --- wsContent Not a Json: ' + wsContent);
			return;
		}
	},// End Start Process received Message.
...
//		}
	...
	onOpenTruckPopup: function(){
		...
		if(isVA){
			...
			popupAlias = 'app-equipmentCdPopupHHT';
			eqDivCd = 'YT'
		}else{
	...
	onOpenEquipmentPopup: function(field){
		var me = this;
		const targetCtl = field.getReference();
		var popupAlias = 'app-equipmentCdPopupHHT';
			title = '',
			eqDivCd = ''
		
		if(targetCtl === 'refTxtCfmDischargingForklift'){
			title = 'Forklift List';
			eqDivCd = 'FL'
		}else if(targetCtl === 'refTxtCfmDischargingPrimeMover'){
			title = 'Prime Mover List';
			eqDivCd = 'PM'
		}
	...
	onEquipmentChange: function (field, event, eOpts) {
		var me = this;
		var refs = me.getReferences();
		...
		me.debounceTimer = setTimeout(() => {
			...
			var params = {
				searchType: 'equipmentcode',
				likeYN: 'Y',
				eqFacNo: refs[reference].getValue()
			};
				...
				callback: function (records, operation, success) {
					...
					if (success) {
						if (records.length > 0) {
							...
							MessageUtil.warning('warning_msg', 'Invalid Equipment Code');
							field.setInputValue('')
						}
					}
				}
	...
	onEquipmentCollapse: function (field) {
		var me = this;
		var store = field.getStore();
	...
	isExistedRecord: function (store, field) {
		...
		var foundItem = store.findBy(function (record) {
			return record.get(field.getValueField()).toLowerCase() === inputValue.toLowerCase();
		});
	...
	setControlSetting: function(){
		...
		var lorryNo = recvData.get('lorryNo');
		var gateTxnNo = recvData.get('gateTxnNo');
     	
     	var isBBK = false;
		var isDBK = false;
		var cgTpCd = recvData.get('cgTpCd');

		isBBK = (cgTpCd === CodeConstants.MT_CGTP_BBK);
		isDBK = (cgTpCd === CodeConstants.MT_CGTP_DBN || cgTpCd === CodeConstants.MT_CGTP_DBE);

		if(lorryNo && gateTxnNo){
			delvTpCd = 'D';
			detailItem.set('delvTpCd', delvTpCd);
			detailItem.set('lorryNo', lorryNo);
			detailItem.set('gateTxnNo', gateTxnNo);
		}
		
		if(!isBBK && !isDBK) {			
			MessageUtil.warning('warning_msg', 'ConfirmDischarging_can_not_operation_msg', cgTpCd);
			var window = me.getView().up('window');
			window.close();
		}

		if(delvTpCd == CodeConstants.MT_DELVTP_I || recvData.get('isYardTruck')) {
			//Indirect case
	...
	afterSetCodePopupData:function(xtype, targetControl, returnValue){
		...
		if(targetControl === 'refConfirmDischargingLorryNo'){
			if(returnValue) {
				var isInternalTruck = refs.refRadioCfmDischargingVA.getChecked()
				if(isInternalTruck){
					detailItem.set('lorryNo',returnValue.item.get('eqFacNo'));
				}else{
		...
		}else if(targetControl === 'refBtnCfmDischargingDamage'){
		...
		}else if(targetControl === 'refTxtCfmDischargingForklift'){
			if(returnValue) {
				var me = this;
				var store = me.getStore('equipmentListPopup')
				var refs = me.getReferences();
				store.setData([returnValue.item.data])
				var field = refs[targetControl];
				field.setValue(returnValue.item.get('eqFacNo'));
			}
		}
	},
...
//			}
	...
	checkFinal: function (ref) {
		...
		if(isVG){
			...
			if(cgTpCd == CodeConstants.MT_CGTP_BBK && loadAbQty >= aprBalQty){
				isFinalLoad = true;
			} else if(isDBK && loadAbMt >= aprBalMt){
				isFinalLoad = true;
			}
		}
	...
	save: function () {
		...
		if (cgTpCd === CodeConstants.MT_CGTP_BBK) {
			...
			let isOverAmount = (loadMt >= balMt  || loadM3 > balM3);
			if(isOverAmount){
				MessageUtil.questionModern('info_msg', 'confirmdischarging_isfinalope_msg', null,
						function (button) {
							finalYN = (button == 'ok' ? 'Y' : 'N');
			...
			callback: function (record, operation, success) {
				...
				if (success) {
					...
					if(tblDtlDamageCheckStore.getCount() > 0){
						me.processDamage(record.data.jobNo)
					}else{
						detailItem.commit();
						me.setMaskedForm(false);
	...
	processDamage: function(jobNo){
		...
		tblDtlDamageCheckStore.each(function(record) {
			var isNewItem = record.get('workingStatus') == WorkingStatus.INSERT
			if(isNewItem) record.set('jobNo', jobNo)
		});
		tblDtlDamageCheckStore.commitChanges()
		...
		if(uploadedFileDamageStore.getCount() > 0){
			uploadedFileDamageStore.each(function(record, index){
				var isNewItem = record.get('workingStatus') == WorkingStatus.INSERT
				if(isNewItem) formData.append(record.data.fileName, record.data.fileStream);
			});
			uploadedFileDamageStore.commitChanges()
	...
	saveDamageCheckProcess: function () {
        ...
        for( targetRecord of tblDtlDamageCheckStore.data.items) {
            ...
            copyRecord.set({
				...
				,stevedoreId: targetRecord.get('stevedoreId')
				,agentId: targetRecord.get('agentId')
				,pkgNo: targetRecord.get('pkgNo')
            });
        ...
        if(currentUploadStore.data.length > 0) {
			for(var i = 0; i < currentUploadStore.data.length; i++) {
				...
				recordUpload.set('pgmId', uploadItem.get('pgmId'));
	...
	amountValidation: function () {
		...
		if (isVG) {
			if (isBBK && loadVgQty <= 0) {
				MessageUtil.warning('warning_msg', 'hht_discharging_bbk_zero_amount');
				return false;
			} else if (isDBK && loadVgMt <= 0) {
				MessageUtil.warning('warning_msg', 'hht_discharging_dbn_zero_amount');
				return false;
			}
		}
		else if (isVA) {
			if (isBBK && loadVaQty <= 0) {
				MessageUtil.warning('warning_msg', 'hht_discharging_bbk_zero_amount');
				return false;
			} else if (isDBK && loadVaMt <= 0) {
				MessageUtil.warning('warning_msg', 'hht_discharging_dbn_zero_amount');
				return false;
			}
		}
		else if (isVB) {
			if (isBBK && loadVbQty <= 0) {
				MessageUtil.warning('warning_msg', 'hht_discharging_bbk_zero_amount');
				return false;
			} else if (isDBK && loadVbMt <= 0) {
				MessageUtil.warning('warning_msg', 'hht_discharging_dbn_zero_amount');
				return false;
			}
		}
			...
			if (isBBK && loadAbQty <= 0) {
				MessageUtil.warning('warning_msg', 'hht_discharging_bbk_zero_amount');
				return false;
			} else if (isDBK && loadAbMt <= 0) {
				MessageUtil.warning('warning_msg', 'hht_discharging_dbn_zero_amount');
				return false;
			}
		}
	...
	onTabDamage: function(){
		...
		var refs = me.getReferences();
		var recvData = me.prevData;
		ViewUtil.openHhtPopup(this, 'app-damageofcargo', 'refsBtnDamage', recvData);
	},
	...
	onTabDimension: function(){
		...
		var refs = me.getReferences();
		var recvData = me.prevData;
		ViewUtil.openHhtPopup(this, 'app-dimensionofcargohht', 'refsBtnDimension', recvData);
	},
	...
	onOpenTblDamageCheck: function() {
		...
		ViewUtil.openHhtPopup(me, 'app-damagecheckhhtpopup', 'refBtnCfmDischargingDamage', detailItem);
	},
	// TOPICID: '',
	// TOPIC_ALL: '/ws/topic/all',
	...
	// 	var me = this;
    //  	var refs = me.getReferences();
    //  	urlWS = refs.txtWsUrl.getValue();
     	
    //  	if(!me.WSCONNECT){
	// 		me.WSCONNECT = Ext.create('MyApp.websocket.WebSocketClient');
	// 	}else {
	// 		me.WSCONNECT.setUrl(urlWS);
	// 	}
	...
	// 		let status = webSocketUtil.getClient().connected;
	// 		if(status){
	// 			me.setStatus(webSocketUtil.getClient().connected);
	// 		}else{
	// 			console.log('Connection Status Ok');
	// 		}
	// 		return;
	// 	}else {
	// 		webSocketUtil.fn_ws_connect_default_(this, me.TOPICID, this.fn_connected, this.fn_handleMessage, this.fn_connectErr);
	// 	}
	// },
	
	// fn_connected: function(me, wsClient, frame) {
	// 	me.setStatus(true);
	// 	me.processCtl(wsClient.client.connected, frame);
	// },
	...
	// stopTasks: function(){
	// 	var me = this;
	// 	for (let i = 0; i < me.TASK_LISTTASK.length; i++) {
	// 		var task = Ext.TaskManager.tasks.find(obj => obj.identifyId == me.TASK_LISTTASK[i]);
	// 		if(task){
	// 			console.log('Close task: '+ task.identifyId);
	...
	// processCtl: function(isConnected, frame) {
	// 	var me = this;
	// 	var refs = me.getReferences();
	// },
	...
	// 	var me = this;
	// 	var refs = me.getReferences();
	// 	refs.refWsStatus.setValue(connected);
	// },
	
	// formatDate: function(inputDate, format) {
	//     if (!inputDate) return '';
	...
	//  	var me = this;
	// 	var refs = me.getReferences();
	// 	refs.refLblClock.setHtml(Ext.Date.format(new Date(), 'g:i:s A'));
	// },
