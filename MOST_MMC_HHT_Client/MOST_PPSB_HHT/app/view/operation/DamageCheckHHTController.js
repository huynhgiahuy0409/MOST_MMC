Ext.define('MOST.view.controller.DamageCheckHHTController', {
    ...
    onTblLoadDetail: function () {
        var  me = this
            ,refs = me.getReferences()
            ,recvData = me.getView().recvData.clone()
            ,theDetail = null
        ...
        if (theDetail.data.isOperationScreen) {
            var catgCd = theDetail.get('catgCd')

            refs.refVslCallId.setValue(theDetail.data.vslCallId);
            refs.refTblDtlCheckedDt.setValue(theDetail.data.checkedDt);
            refs.refTblDetailLocationRadioGroup.setDisabled(true)
            ...
            if (catgCd == 'I' || catgCd == 'T') {
                refs.refDtlCboMasterBL.setValue(theDetail.data.mfDocId);
                refs.refDtlCboSubBL.setValue(theDetail.data.blNo);
            } else if (catgCd == 'E' || catgCd == 'S'|| catgCd == 'R') {
                refs.refDtlCboBookingNo.setValue(theDetail.data.mfDocId);
                refs.refDtlCboSN.setValue(theDetail.data.shipgNoteNo);
            }
            ...
            if (theDetail.data.locCd == 'VSL') {
                refs.refRadioVessel.setChecked(true);
            } else if (theDetail.data.locCd == 'YARD') {
    ...
    setTblDetailComboStore: function () {
        ...
        pkgNoCombo.load({
            params: {
                ...
                ,shipgNoteNo: theDetail.get('snNo')
           }
        });
...
;
    ...
    onTblSelectDetailCgNoCombo: function ( combo , newValue, eOpts )  {
        var  me = this
            ...
            ,sltRecord = refs[combo.reference].getSelection()
        ;
        if(!sltRecord){
            return
        }
        var cgTpCd = refs[combo.reference].getSelection().data.cgTpCd || theDetailParm.get('cgTpCd');
        me.onLoadPackageNoCombo()
    ...
    onLoadPackageNoCombo: function(){
        var me = this
        , refs = me.getReferences()
        , pkgNoCombo  = me.getStore('pkgNoCombo')
        , vslCallId = me.getViewModel().get('tblDamageCheckDetail').get('vslCallId')
        , blNo = refs.refDtlCboSubBL.getValue()
        , snNo = refs.refDtlCboSN.getValue()
        ;
        pkgNoCombo.load({
    ...
    setTblDetailGridStore : function () {
        var  me = this 
            ...
            ,blSnNo = theDetail.get('blNo') ? theDetail.get('blNo') : theDetail.get('snNo')
            ,params = {
                vslCallId: theDetail.get('vslCallId')
                ,catgCdFile: [theDetail.get('vslCallId'), blSnNo].join('|')
                ,catgCd: theDetail.get('catgCd')
                ,blNo: theDetail.get('blNo')
                ,snNo: theDetail.get('snNo')
                ,unitNo: theDetail.get('unitNo')
                ,cgNo: theDetail.get('snNo') ?  theDetail.get('snNo') : theDetail.get('blNo')
                ,pgmId : 'mpct239'
            }
    ...
    onTblAddDetailGrid: function( button, e, eOpts ) {
        ...
        if (!stevedoreId) {
            MessageUtil.warning('warning_msg', 'mandatoryField_msg', refs.refStevedoreId.getLabel());
			return;
        }else if(!agentId){
            MessageUtil.warning('warning_msg', 'mandatoryField_msg', refs.refAgentId.getLabel());
			return;
        }
        ...
        record.set({
            ...
            ,agentId
            ,workingStatus: WorkingStatus.INSERT
            ,scn: theDetail.get('scn')
            ,pkgNo: theDetail.get('pkgNo')
            ,catgCd: theDetail.get('catgCd')
        });
    ...
    onTblDetailClear: function(button, e, eOpts) {
        ...
        refs.refStevedoreId.clearValue();
        refs.refAgentId.clearValue();
        refs.refDamageCheckPackageNo.clearValue();
    },
    ...
    onSelectGridDamageCheckHHT: function () {
        ...
        refs.refStevedoreId.setValue(selected.data.stevedoreId);
        refs.refAgentId.setValue(selected.data.agentId);
        refs.refDamageCheckPackageNo.setValue(selected.data.pkgNo);
    ...
    fileSelected: function (field) {
        var me = this,
            ...
            theDmg = me.getViewModel().get('tblDamageCheckDetail')

        for(var i = 0; i < files.length; i++) {
            var record = Ext.create('MOST.model.common.FileUpload');
            var file = files[i];
            var blSnNo = theDmg.get('blNo')? theDmg.get('blNo') : theDmg.get('snNo')
            record.set('catgCd', [theDmg.get('vslCallId'), blSnNo].join("|"));
