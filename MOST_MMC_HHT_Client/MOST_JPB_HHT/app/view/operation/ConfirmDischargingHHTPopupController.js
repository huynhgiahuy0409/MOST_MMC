			...
			callback: function(record, operation, success) {
				if(success){
					if(record != null && record.length > 0){
						...
						refs.refConfirmDischargingLorryNo.setValue('ABC')
						refs.refTxtCfmDischargingPkgNo.setValue('ABC')
					}
				}
			}
		});
...
//			}
	...
	onPassedTerminalHoldValidation: function(){
		var me = this;
		var detailItem = me.getViewModel().get('theDetail');
		var isDirect = detailItem.get('jobPurpCd') === 'VG';
		var isIndirect = detailItem.get('jobPurpCd') === 'VA';
		if(isDirect){
			if(detailItem.get('domesticChk') == 'N' && Token.getCustomHoldChk() === 'Y') {
				if(detailItem.get('custMode') !== me.CUST_RELEASE) {
					MessageUtil.warning('warning_msg', 'confirmdischarging_customrealeaseblock_msg');
					return
				} else {
					me.save();
				}
	...
	checkDamageCheck: function(jobNo){
		...
		if(tblDtlDamageCheckStore.getCount() > 0){
			...
			uploadedFileDamageStore.each(function(record) {
				var a = record.data.workingStatus
			});
			tblDtlDamageCheckStore.commitChanges()
